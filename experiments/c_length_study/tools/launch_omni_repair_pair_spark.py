...
from experiments.c_length_study.tools.launch_length_training_spark import run_job, save, verified_source
from experiments.c_length_study.tools.launch_omni_repair_spark import metrics, continuation_screen
...
def assert_source_pair(manifests):
    """Only the deliberate training-plan delta may differ between branches."""
    if len(manifests) != 2 or set(manifests[0]) != set(manifests[1]):
        raise ValueError('Paired source manifest coverage differs')
    plan = 'robot/hexapod_mkii_length_study/training_plan.json'
    if plan not in manifests[0]:
        raise ValueError('Paired sources must bind the exact training plan')
    changed = {key for key in manifests[0] if manifests[0][key] != manifests[1][key]}
    if changed != {plan}:
        raise ValueError('Paired sources must differ only in their training plan')
...
def checked_report(path, expected):
    ...
    maxima = [r['windows']['all']['applied_torque_abs_max_nm'] for r in report['scenarios']]
    if any(not math.isfinite(value) or value < 0 or value > 1.60001 for value in maxima):
        raise RuntimeError('Nonfinite or out-of-bounds applied motor torque')
...
def main():
    ...
    for source in a.sources:
        ...
        manifests.append(json.loads((source/'campaign_source_hashes.json').read_text()))
        plans.append(json.loads((source/'robot/hexapod_mkii_length_study/training_plan.json').read_text()))
    assert_source_pair(manifests)
