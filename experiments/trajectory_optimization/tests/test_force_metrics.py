...
from experiments.paper_walk import evaluate
from experiments.paper_walk.env_config import JOINT_NAMES, MODEL_SHA256
from experiments.trajectory_optimization.force_metrics import report_for, run_batch, sha, summarize, write_summary
from experiments.paper_walk.tests.test_evaluate import FakeCapture, FakeNative
...
class ForceMetricsTests(unittest.TestCase):
    def test_fresh_pack_binds_and_imports_load_reporter_without_repository(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(manifest['force_metrics.py'], sha(source/'force_metrics.py'))
            result = subprocess.run([sys.executable, '-c',
                'from source.force_metrics import MODEL_SHA256; print(MODEL_SHA256)'],
                cwd=output, text=True, capture_output=True, check=True)
