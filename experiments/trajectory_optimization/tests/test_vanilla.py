...
from experiments.trajectory_optimization.model import ROOT, digest
from experiments.trajectory_optimization.prepare_vanilla import prepare
from experiments.trajectory_optimization.vanilla import TrainingLoads, VanillaVecEnv
...
class VanillaTests(unittest.TestCase):
    ...
    def test_fresh_pack_keeps_admitted_physics_and_refuses_reuse(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in ('env.py', 'env_config.py', 'task.py', 'evaluate.py', 'launch_spark.py', 'reservation.py'):
                self.assertEqual(digest(source/name), digest(ROOT/'experiments/paper_walk'/name))
