...
read the same numbers. Every v1 module carries ``SCHEMA_VERSION = 1``. A v1
value is never edited: a changed interface is a new module or a new constant
beside the old one, because the deployed checkpoint's meaning is frozen with
it. :mod:`.joints_v2` is the first such module: the runtime joint order of
asset v1 (the CAD assembly), standing beside the mock's :mod:`.joints`.
"""

from __future__ import annotations

from . import action, actuator, command, frames, joints, joints_v2, observation
