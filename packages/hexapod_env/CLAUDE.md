...
the pure reward/gait primitives under `rewards/`, the robot-asset descriptions
under `assets/` (`spec.py` is stdlib-only: one `HexapodAssetSpec` per robot
model; `articulation.py` builds the Isaac Lab `ArticulationCfg` from a spec),
and the gym registration in `register.py`. Adding a robot model means a new
spec, one env config that reads it (see `HexapodMkiiV1FlatEnvCfg`), and a new
task ID; existing task IDs keep loading the model they were trained on. It is
consumed today via `PYTHONPATH` (the Spark container adds
