...
from .capture import CONTACT_CATEGORIES, DiagnosticGeometry, Rs05DiagnosticCapture, capture_row
from .config import HexapodMkiiRs05FlatEnvCfg
...
class HexapodMkiiRs05Env(DirectRLEnv):
    ...
    def _get_rewards(self):
        ...
        components, total = task_math.reward_terms(
            ...
            linear_body=tensor(data.root_lin_vel_b),
    ...
    def _get_observations(self):
        ...
        critic = task_math.critic_observation(policy, tensor(self._robot.data.root_lin_vel_b))
    ...
    def _close_capture_row(self):
        ...
        applied_readback = tensor(self._motors.applied_effort).index_select(-1, self._canonical_index)
        ...
        row = capture_row(
            ...
            native_input_pre_nm=applied_readback.cpu().numpy(),
            native_input_post_nm=applied_readback.cpu().numpy(),
        ...
        self._capture.record(row)

    def _contact_channels(self, link_pose):
        """Toe and non-toe ground contact of one substep, in numpy."""

        feet = np.zeros((self.num_envs, 6, 3))
        shaft = np.zeros((self.num_envs, 3))
        distal = np.zeros((self.num_envs, 6), dtype=bool)
        for index, (leg, sensor) in enumerate(zip(self._leg_names, self._feet_contact_sensors)):
            force = tensor(sensor.data.force_matrix_w)[:, 0, 0].cpu().numpy()
            point = tensor(sensor.data.contact_pos_w)[:, 0, 0].cpu().numpy()
            loaded = np.linalg.norm(force, axis=-1) > 1.0
            body_index = self._body_names.index(f"{leg}_tibia")
            finite = np.isfinite(point).all(-1)
            inside, _ = self._capture_geometry.cap_contains_batch(
                f"{leg}_tibia", np.where(finite[:, None], point, link_pose[:, body_index, :3]),
                link_pose[:, body_index],
            )
            toe = loaded & finite & inside
            feet[:, index] = np.where(toe[:, None], force, 0.0)
            shaft += np.where((loaded & ~toe)[:, None], force, 0.0)
            distal[:, index] = toe
        other = np.zeros((self.num_envs, len(CONTACT_CATEGORIES), 3))
        other[:, CONTACT_CATEGORIES.index("shaft")] = shaft
        nonfoot = np.linalg.norm(shaft, axis=-1) > 1.0
        for name, sensor in self._body_contact_sensors.items():
            if name.endswith("_tibia"):
                continue
            force = tensor(sensor.data.net_forces_w)[:, 0].cpu().numpy()
            category = "body" if name == self.cfg.body_names[0] else "coxa" if name.endswith("_coxa") else "femur"
            other[:, CONTACT_CATEGORIES.index(category)] += force
            nonfoot |= np.linalg.norm(force, axis=-1) > 1.0
        return {"distal_contact": distal, "distal_force_world_n": feet,
                "nonfoot_contact": nonfoot, "nonfoot_force_world_n": other}
