...
**PAUSED FOR JAMES — 11 September 2026:** the user has paused research and requested a complete Git handoff to James. This supersedes all earlier instructions to continue overnight, keep the GPU busy or automatically restart agents/jobs. Perform no new research, validation, training or recording until James or the user explicitly resumes work. The saved Codex continuation is paused. Preserve the external Spark reservation; this pause does not release competing jobs. Handoff/publication and read-only integrity checks are allowed. Start with [docs/JAMES_HANDOFF.md](docs/JAMES_HANDOFF.md) and STATUS.
...
**Complete external automation block, 11 September 2026:** the user explicitly ordered external Spark automated jobs stopped completely, including their triggers, with HEXAPOD-only priority. Preserve the forecast/Ollama masks, reconstruction queue lock and protected launcher entry described in [docs/OPERATIONS.md §3.1](docs/OPERATIONS.md#31-complete-external-automation-block). Do not re-enable them through a repository installer, per-job cleanup or a sharing request. Keep original unit/source backups and queued/completed outputs; restore only after explicit user release. Keep CI and Pages enabled; HEXAPOD's continuation remains paused under the handoff instruction above. Verify the actual masks and queue-lock owner before allocation; the historical inline worker gate alone was replaced by an external upload and is insufficient. These operational controls never waive native admission or change robot/controller/quality contracts.
