...
def main():
    ...
    baseline = {name: evaluate(learner, REQUEST[key]) for name, key in [('train', 'prior'), ('same_cycle_holdout', 'heldout')]}
    ...
    try:
        learner.pretrain_bc(REQUEST['prior'], REQUEST['bc_steps'], REQUEST['batch_size'])
    finally:
        hook.remove()
    fit_seconds = time.monotonic()-started
    assert steps == learner.bc_steps == 1000
    after = {name: evaluate(learner, REQUEST[key]) for name, key in [('train', 'prior'), ('same_cycle_holdout', 'heldout')]}
    ...
    report = {
        ...
        'scope': 'Offline imitation fit on original accepted steady native rows only; no walking or native handoff claim',
        ...
        'no_native_actions': True, 'onset_or_stop_examples_added': False,
    ...
    with np.load(REQUEST['prior'], allow_pickle=False) as data:
        obs = torch.from_numpy(data['observations'].copy())
