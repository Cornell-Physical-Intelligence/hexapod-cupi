"""Dispatch only this fresh standing and four bounded directional cases under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_directional_source_001'
OUTPUT = BASE / 'reference_directional_001'
PAUSE = BASE / 'forecast_pause_041'
UNIT = 'hexapod-reference-directional-001-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '373c9ea08406f6f3593279fa9fda24badf56ae86bd938e140dba7d734b5ee919':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-pair-load-transfer-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_040/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then four cold directional reference cases; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-041', '--on-active=60m',
        ...
        for fd in reversed(locks):
            os.close(fd)
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=3120',
