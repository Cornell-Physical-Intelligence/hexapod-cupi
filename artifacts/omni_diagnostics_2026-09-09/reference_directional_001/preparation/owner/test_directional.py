...
class MotionTests(unittest.TestCase):
 def test_correct_signed_reverse_strafe_turn_arc(self):
  for case in CASES:
 ...
 def test_same5mm_pose_rate_gate_rejects_despite_good_direction(self):
  d,r=fixture('reverse');d['velocity_world_mps'][200:1400,0,0]+=.001
  g=score_direction(d,r,case='reverse',joint_names=[f'joint{i}' for i in range(18)]);self.assertFalse(g['passed'])
 ...
 def test_command_time_and_admitted_and_actual_channels_bound(self):
  for key in ['requested_command','reference_admitted_target_twist','actual_executed_body_navigation_twist']:
   d,r=fixture('reverse');d[key][200,0,1]+=.001
   with self.assertRaises(ValueError):score_direction(d,r,case='reverse',joint_names=[f'joint{i}' for i in range(18)])
 def test_timebase_mismatch_rejected(self):
  d,r=fixture('reverse');d['time_s'][201,0]+=.01
  with self.assertRaises(ValueError):score_direction(d,r,case='reverse',joint_names=[f'joint{i}' for i in range(18)])
...
class HostTests(unittest.TestCase):
 ...
 def test_exact_fresh_standing_and_wave_required(self):
  with tempfile.TemporaryDirectory() as td:
   p=Path(td)/'receipt.json';a=SimpleNamespace(mode='directional',case='reverse',num_envs=1,steps=2400,admission=p);identity={'source':'new_directional'}
   ...
   with patch.object(contract,'standing_preflight',return_value=(identity,{})),patch.object(contract,'check_wave'):
    p.write_text(json.dumps(good));self.assertEqual(preflight(a,Path(td))[0]['case'],'reverse')
