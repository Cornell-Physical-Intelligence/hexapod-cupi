...
HERE=Path(__file__).resolve().parent;SOURCE=HERE/'source_directional_001';sys.path.insert(0,str(SOURCE/'tools'))
...
identity,geometry=standing(a,SOURCE);a.mode='directional';a.num_envs=1;a.steps=2400;a.case='reverse';a.admission=HERE.parent/'reference_physics_results_009/run/standing/admission.json'
try:preflight(a,SOURCE)
except ValueError:old_rejected=True
else:raise AssertionError('Actual009 admission accepted for new source')
...
r={'runtime':runtime,'source_manifest_sha256':host.digest(SOURCE/'campaign_source_hashes.json'),'source_files':930,'actual009_old_admission_rejected':old_rejected,'failed_quiet_synthetic_receipt_rejected':quiet_rejected,'all_named_cases_preflight':cases,'synthetic_receipts_are_CPU_only':True,'physics_admitted':False}
