#!/usr/bin/env python3
"""Fresh standing then four named low-speed directional reference cases; no PPO."""
import argparse
import fcntl
import hashlib
import json
...
def command(source, output, name, phase):
    if phase != "standing" and phase not in CASES:
        raise ValueError("Only fresh standing and four named directional cases exist")
...
def main():
    ...
    try:
        ...
        for case in CASES:
            ...
            save(args.output / 'campaign.json', report)
        check_source(args.source)
        report.update(status='completed', all_four_declared_reference_cases_passed=True,
