#!/usr/bin/env python3
"""Fresh standing then one bounded RR preload left-strafe diagnostic; no PPO."""
import argparse
import fcntl
import hashlib
import json
...
def command(source, output, name, phase):
    if phase != "standing" and phase not in CASES:
        raise ValueError("Only fresh standing and the RR-diagnostic left_strafe case exist")
