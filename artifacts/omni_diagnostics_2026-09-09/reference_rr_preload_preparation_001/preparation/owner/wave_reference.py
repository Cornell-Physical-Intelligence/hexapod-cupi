...
class WaveConfig:
    ...
    lift_m: float = .007
    horizontal_duration_fraction: float = .80
...
class AdvancedHorizontalSwing(Swing):
    """Preserve the vertical arc and endpoint; finish horizontal motion earlier.

    This is a time parameterization change, not a changed foothold, body command,
    or contact gate. Both components meet position/velocity/acceleration at their
    endpoints. It is only used for a new swing that starts at zero velocity and acceleration.
    """
    def __init__(self, t0, duration, p0, p1, lift, horizontal_fraction):
        if (isinstance(horizontal_fraction, bool) or not math.isfinite(horizontal_fraction)
                or not .5 <= horizontal_fraction <= 1. or not math.isfinite(duration) or duration <= 0
                or not math.isfinite(t0)):
            raise ValueError('Finite positive swing duration and horizontal fraction in [0.5, 1] required')
        super().__init__(t0, duration, p0, p1, lift)
        self.horizontal_fraction = horizontal_fraction
        self.horizontal = Swing(t0, duration*horizontal_fraction, p0, p1, 0.)

    def sample(self, time_s):
        vertical = super().sample(time_s)
        if time_s >= self.horizontal.end_time:
            horizontal = (self.end.copy(), np.zeros(3), np.zeros(3))
        else:
            horizontal = self.horizontal.sample(time_s)
        for output, xy in zip(vertical, horizontal):
            output[:2] = xy[:2]
        return vertical
...
class WaveContactReference:
    ...
    def __init__(self, joint_names_runtime, config=WaveConfig(), geometry=None):
        ...
        if (not .5 <= config.horizontal_duration_fraction <= 1. or config.swing_s <= 0 or config.contact_hold_s < .02 or config.contact_confirm_steps < 1
        ...
        if asdict(config) != asdict(WaveConfig()):
            raise ValueError("RR diagnostic requires the exact original wave005 configuration")
        self.cfg = config
        self.ready = False
        self.failure = None
    ...
    def reset(self, snapshot):
        ...
        self.raw_force_free_samples = 0; self.raw_force_free_runs = 0; self.unqualified_contact_returns = 0
        self.last_unqualified_return_time = None; self.last_unqualified_run_samples = 0; self.last_unqualified_lift_m = None
        ...
        self.rr_preload_diagnostic = RRFirstLandingPreload()
    ...
    def _output(self, q, v, a, measured):
        ...
        state = dict(mode=self.mode, current_leg=None if self.current_leg is None else LEGS[self.current_leg],
            ...
            flight_qualification_semantics='two_consecutive_force_free_samples_and_measured_minimum_lift_before_apex',
            raw_force_free_samples=self.raw_force_free_samples, raw_force_free_runs=self.raw_force_free_runs,
            unqualified_contact_returns=self.unqualified_contact_returns,
            last_unqualified_return_time_s=self.last_unqualified_return_time,
            last_unqualified_run_samples=self.last_unqualified_run_samples,
            last_unqualified_lift_m=self.last_unqualified_lift_m,
            ...
            swing=None if self.swing is None else dict(start_s=self.swing.t0, duration_s=self.swing.duration,
                ...
                horizontal_duration_fraction=self.swing.horizontal_fraction,
                horizontal_coefficients=self.swing.horizontal.coeff.copy()))
        return dict(valid=np.array([self.failure is None]), failure_reason=self.failure,
            ...
            diagnostics={**self.last_diagnostics, 'configuration': asdict(self.cfg),
                         'rr_preload_diagnostic': self.rr_preload_diagnostic.state(),
    ...
    def step(self, snapshot, requested_forward_left_yaw, dt=.02):
        ...
        try:
            ...
            if not (np.array_equal(requested, np.zeros(3)) or np.array_equal(requested, np.array([0., .005, 0.]))):
                raise ValueError('RR diagnostic accepts only exact left_strafe0.005 or zero command')
            ...
            self.rr_preload_diagnostic.require_contact(measured['contact'][5], measured['reference_point_world_m'][5])
            ...
            if self.current_leg is not None:
                ...
                if not measured['contact'][i]:
                    if self.flight_count == 0:self.raw_force_free_runs += 1
                    self.raw_force_free_samples += 1
                    self.flight_count += 1; self.contact_count = 0
                    self.flight_peak_z = max(self.flight_peak_z, actual_z)
                    if (self.flight_count >= 2
                            and self.flight_peak_z-self.flight_baseline_z >= self.cfg.minimum_measured_lift_m):
                        self.flight_seen = True
                        if self.landing is None:self.mode = 'swing'
                elif not self.flight_seen:
                    if self.flight_count:
                        self.unqualified_contact_returns += 1
                        self.last_unqualified_return_time = self.time
                        self.last_unqualified_run_samples = self.flight_count
                        self.last_unqualified_lift_m = self.flight_peak_z-self.flight_baseline_z
                ...
                if not self.flight_seen and self.time >= self.swing.t0+.5*self.swing.duration:
                    ...
                    if measured['contact'][i]:
                        ...
                        if self.time >= end-1e-8:
                            ...
                            if self.contact_count >= self.cfg.contact_confirm_steps:
                                ...
                                self.rr_preload_diagnostic.arm(i, self.touchdowns, self.time, self.cfg.contact_hold_s,
                                    self.anchors[i], self.measured_anchors[i], self.swing.end)
            ...
            if self.current_leg is None and self.time >= self.hold_until and speed > 1e-6:
                ...
                self.swing = AdvancedHorizontalSwing(self.time, self.cfg.swing_s, self.anchors[i], endpoint,
                    self.cfg.lift_m, self.cfg.horizontal_duration_fraction)
                self.current_leg = i; self.mode = 'unloading'; self.flight_seen = False; self.flight_count = 0; self.contact_count = 0
                self.raw_force_free_samples = 0; self.raw_force_free_runs = 0; self.unqualified_contact_returns = 0
                self.last_unqualified_return_time = None; self.last_unqualified_run_samples = 0; self.last_unqualified_lift_m = None
                self.landing = None; self.landing_trigger_s = None; self.landing_contact_origin = None
                self.landing_endpoint_correction_m = None; self.landing_contact_gap_steps = 0
            ...
            finite_stop = (np.linalg.norm(requested) == 0 and self.current_leg is None
                           and not self.rr_preload_diagnostic.active
            ...
            self.time += dt
            self.rr_preload_diagnostic.apply(self.time, self.anchors, self.measured_anchors, self.preload_world)
            ...
            if (self.current_leg is None and np.max(np.abs(self.command)) < 1e-14 and np.max(np.abs(self.command_rate)) < 1e-12
                    and not self.rr_preload_diagnostic.changed_this_step):
                q = self.q.copy()
            ...
            self.rr_preload_diagnostic.abort(self.failure)
            return self._output(self.q, self.v, np.zeros_like(self.q), measured)
