"""Dispatch only this fresh standing and one bounded opposing-pair forward case under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_pair_motion_source_001'
OUTPUT = BASE / 'reference_pair_motion_001'
PAUSE = BASE / 'forecast_pause_050'
UNIT = 'hexapod-pair-motion-001-20260910.service'
...
def main():
    ...
    if hashlib.sha256((SOURCE/'tools/launch_pair_motion_spark.py').read_bytes()).hexdigest() != '60d51f1e8d71924390add188460f12e157eb114aa3dae617ce254c90fe4d021e':
        raise RuntimeError('Wrong reviewed paired-motion host')
    sys.path.insert(0, str(SOURCE / 'tools'))
    from launch_pair_motion_spark import check_source
    from launch_length_study_spark import preflight
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '216224f3cb727549d8f70bfc2836f32bdf472f892b37b0c89fa471dc71b23932':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-moving-ppo-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_049/restored.json').is_file():
        raise RuntimeError('Previous reference forecasting restoration not yet recorded')
    prior_pins = {'reference_moving_ppo_001/campaign.json': '05b31fa503f46021ee71abd4bf6bb51653c0e340640a77bf04ef3c2820f7d919', 'forecast_pause_049/restored.json': '67b4c8f9b8a6c035c39f2c5b72ea00ade2495cf1b2c66bf92c4f2ccaa866d6b6', 'reference_moving_ppo_001/jobs/standing.json': 'd8b5bbe44a77d48fc45ba8eb78dacd6e99d0035a0d5abff4c7ee36ec5b77be45', 'reference_moving_ppo_001/jobs/calibration.json': '71fb3096abd65a273ec9e5faa509a596beaf78025143db69be08e8b1abad8df9', 'reference_moving_ppo_001/jobs/profile_32.json': 'bafc020bcaac2b166ab427747ab603cce44ea8b99cb2251b763bb882057ce633'}
    ...
    for phase in ('standing','calibration','profile_32'):
        job=json.loads((BASE/'reference_moving_ppo_001/jobs'/(phase+'.json')).read_text())
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then one full48s opposing-pair forward-stop diagnostic; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-050', '--on-active=30m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_pair_motion_spark.py'),
