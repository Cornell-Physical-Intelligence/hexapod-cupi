...
if (H/'report.json').exists():raise FileExistsError('Preserve existing evidence')
...
packet=pack(baseline[-1]);schema=packet['schema'];schema['sha256']=packet['schema_sha256'];(H/'STATE_SCHEMA.json').write_text(json.dumps(schema,indent=2)+'\n')
...
np.savez_compressed(H/'baseline_targets.npz',**values,joint_names=np.asarray(baseline[-1]['joint_names_runtime']))
sources={str(p.relative_to(H)):sha(p) for p in sorted(H.rglob('*.py'))}
report={'scope':'CPU-only paired contact-aware controller with ideal synthetic measured pose/contact streams; no actual physics, torque or speed qualification',
 ...
 'tests':{'count':13,'log_sha256':sha(H/'tests.log'),'initial_failed_expectation_log_sha256':sha(H/'tests_initial.log'),'zero_residual_full_forward_and_stop':True,'each_foot_independently_qualified':True,'JSON_controller_motor_state_and_history_replay_exact':True,'new_reset_epoch_clears_all_history_slots':True},
