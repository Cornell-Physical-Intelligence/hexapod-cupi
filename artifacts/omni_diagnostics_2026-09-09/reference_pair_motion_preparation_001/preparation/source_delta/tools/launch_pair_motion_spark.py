#!/usr/bin/env python3
"""Fresh standing then one paired forward/stop physics discriminator; no PPO."""
import argparse
import fcntl
import hashlib
import json
...
from pair_motion_contract import PROTOCOL, CASES, check_pair
...
def command(source, output, name, phase):
    if phase != "standing" and phase not in CASES:
        raise ValueError("Only fresh standing and paired_forward exist")
    common = ["docker", "compose", "--env-file", "docker/.env.base", "-f", "docker/docker-compose.yaml",
        ...
        "/workspace/hexapod/tools/" + ("run_reference_physics.py" if phase == "standing" else "run_pair_motion.py"), "--mode", "standing" if phase == "standing" else "paired_motion",
...
def main():
    ...
    report = dict(status="preparing", runtime_binding=runtime, started_unix=time.time(),
        ...
        source_manifest_sha256=digest(args.source / "campaign_source_hashes.json"), automatic_continuation=False, solver_comparison=SOLVER_PROTOCOL, paired_motion_protocol=PROTOCOL)
    ...
    try:
        ...
        for case in CASES:
            ...
            if (case_identity.get('standing_identity') != identity or case_identity.get('paired_motion_protocol') != PROTOCOL
                ...
                raise RuntimeError('Paired-motion result differs from its exact named-case/fresh standing contract')
            ...
            if actual_solver.get('protocol') != SOLVER_PROTOCOL or actual_solver.get('actual_scene_attributes') != comparison.get('actual_scene_attributes'):
                raise RuntimeError('Paired-motion solver readback differs from fresh standing admission')
            if tree_hashes(package) != before:
                raise RuntimeError('Paired-motion case changed its read-only admitted asset')
            case_states[case] = result
            report.update(status='running_paired_motion_case', completed_cases=list(case_states), untested_cases=[k for k in CASES if k not in case_states])
            save(args.output / 'campaign.json', report)
        check_source(args.source)
