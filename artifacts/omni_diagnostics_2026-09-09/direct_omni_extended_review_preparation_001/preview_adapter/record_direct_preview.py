...
def execute(args, provenance):
    ...
    def prepare_shutdown():
        from shutdown_integrity import seal_before_shutdown
        try:return seal_before_shutdown(args,provenance,namespace['app'],verify_inputs)
    ...
    namespace={'__name__':'__main__','__file__':str(entry),'__package__':None,
        ...
        '_preview_record':recording,'_preview_prepare_shutdown':prepare_shutdown}
...
def main(argv=None):
    ...
    try:
        ...
        # Default SimulationApp.close terminates the process; this branch is optional.
        from shutdown_integrity import validate_shutdown_seal
        validate_shutdown_seal(args.output,provenance)
        if verify_inputs(args)!=provenance:raise ValueError('Inputs changed after a returning App close')
    except BaseException as exc:
        preserve_failure(args.output,exc)
        raise
