...
parser.add_argument("--direct-allocation", choices=("smoke", "pilot", "extended"))
parser.add_argument("--direct-branch", choices=("curriculum", "caps", "quiet_priority"))
...
from candidate_asset_audit import audit_candidate_usd as repair_and_verify
...
def main():
    ...
    if plan["omni"].get("direct_recovery_training") != protocol():
    ...
    state["direct_selection"] = direct_selection
    ...
    # Only allocation differs; inherited physical cfg assignments stay exact.
    ...
    if omni:
        ...
        from omni_flat_env import configure_omni
        configure_omni(cfg, omni.get("overrides"))
        if "diagnostics" in omni:
            from omni_diagnostics import diagnostic_options, diagnostic_scenarios
    ...
    if omni:
        from omni_flat_env import OmniFlatEnv
        if args.mode == "train":
        ...
        from length_reference_env import ReferenceGaitEnv
    ...
    if args.mode=="validate":
        ...
        runner_cfg.max_iterations = direct_selection.get("updates", plan["training_iterations"])
        ...
        if args.mode=="train":
            ...
            from direct_training import learn_and_verify
            ...
            runner.load(str(args.checkpoint))
            if omni:
                from omni_flat_evaluation import evaluate_omni, record_omni
                function = evaluate_omni
