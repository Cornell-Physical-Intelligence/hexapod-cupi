...
class StructuralTests(unittest.TestCase):
    def test_exact_native_ast_round_trip(self):
        ...
        self.assertEqual(counts,dict(app=1,save=1,config=1,render=1,dispatch=1,shutdown=1))
        class Undo(ast.NodeTransformer):
            def visit_Try(self,node):
                self.generic_visit(node)
                if len(node.body)==1 and entry.same(node.body[0],'_preview_prepare_shutdown()') and len(node.finalbody)==1 and entry.same(node.finalbody[0],'app.close()'):
                    return node.finalbody[0]
                return node
...
class ReceiptTests(unittest.TestCase):
    def setUp(self):
        host=patch.object(contract,'HOST','a'*64);host.start();self.addCleanup(host.stop)
    def fixture(self,root):
        pilot=root/'pilot';pilot.mkdir();checkpoint=root/'selected.pt';checkpoint.write_bytes(b'exact trained final')
        bound=contract.sha(checkpoint);selected={'updates':500}
        ...
        identity={'schema':'direct315_extended_native_v4','source_manifest_sha256':contract.SOURCE,'plan_sha256':contract.PLAN,'checkpoint_sha256':contract.ORIGINAL,
        ...
        campaign={'status':'completed','terminal_inputs_unchanged':True,'host_freeze_sha256':contract.HOST,
            'identity':identity,'allocation':'extended','branch':'caps','planned_phases':phases,'accepted_phases':receipts,'PPO_updates_completed':500,'bounded_campaign_complete':True,'last_completed_phase':'final_stop','Stage2_complete':False}
    ...
    def test_all_six_phases_and_exact_selected_checkpoint_required(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            self.assertEqual(receipt['completed_updates'],500)
...
class LoopTests(unittest.TestCase):
    def run_loop(self,terminal=None):
        ...
        p={'checkpoint_sha256':'x','pilot':{'branch':'caps','completed_updates':500}}
