...
def instrument(text):
    ...
    tree=ast.parse(text);counts={'app':0,'save':0,'config':0,'render':0,'dispatch':0,'shutdown':0}
    class Rewrite(ast.NodeTransformer):
        ...
        def visit_Expr(self,node):
            self.generic_visit(node)
            if same(node,'app.close()'):
                counts['shutdown']+=1
                return ast.copy_location(ast.parse('try:\n    _preview_prepare_shutdown()\nfinally:\n    app.close()').body[0],node)
    ...
    if counts!={'app':1,'save':1,'config':1,'render':1,'dispatch':1,'shutdown':1}:raise ValueError('Unexpected render seam counts '+str(counts))
