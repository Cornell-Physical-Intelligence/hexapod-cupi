...
class Tests(unittest.TestCase):
 ...
 def test_complete_and_partial_recording_result(self):
  with tempfile.TemporaryDirectory() as t:
   d=Path(t);identity={k:k for k in ('source_manifest_sha256','plan_sha256','native_contract_freeze_sha256','adapter_freeze_sha256','checkpoint_sha256')};identity.update(pilot={'completed_updates':500},rendering_overrides={'num_envs':1})
   ...
   h.save(d/'state.json',dict(status='completed',checkpoint_sha256=identity['checkpoint_sha256'],runtime_binding={'runtime_tree_sha256':h.LEGACY_RUNTIME_TREE},recording_provenance=identity));h.save(d/'video.json',v);h.save(d/'pre_shutdown_integrity.json',{'schema':'direct_preview_pre_shutdown_v1','scope':'before_native_app_close','process_exit_verified':False,'passed':True,'provenance':identity,'shutdown_runtime':{'source_sha256':'6af5372bb0cdda6e665a30cd285ff724b3e5bd012dbdd4df458e0726173fc903','fast_shutdown':True,'close_return_required':False},'output_hashes':h.tree_hashes(d)})
