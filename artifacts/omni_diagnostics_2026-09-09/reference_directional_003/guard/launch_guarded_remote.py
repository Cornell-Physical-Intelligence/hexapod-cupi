"""Dispatch only this fresh standing and two bounded directional cases under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_directional_source_003'
OUTPUT = BASE / 'reference_directional_003'
PAUSE = BASE / 'forecast_pause_046'
UNIT = 'hexapod-reference-directional-003-20260910.service'
...
def main():
    ...
    if hashlib.sha256((SOURCE/'tools/launch_directional_physics_spark.py').read_bytes()).hexdigest() != '5cb1840e69c63752e1a8b75978107044b0c9c8df7543bb4be9e93f9cd0aed101':
        raise RuntimeError('Wrong reviewed directional host')
    sys.path.insert(0, str(SOURCE / 'tools'))
    from launch_directional_physics_spark import check_source
    from launch_length_study_spark import preflight
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'e36fb225c1b1235c7a1389a37f491f71430dee9fefac52605e6302c8c34af650':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-mild-fixtures-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_045/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then two cold directional reference cases; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-046', '--on-active=40m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1920',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_directional_physics_spark.py'),
