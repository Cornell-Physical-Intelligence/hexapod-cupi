"""Dispatch only this fresh standing and three bounded directional cases under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_directional_source_002'
OUTPUT = BASE / 'reference_directional_002'
PAUSE = BASE / 'forecast_pause_044'
UNIT = 'hexapod-reference-directional-002-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '7c64602cae91478aebd4b03adcc79f56084db7529fd1c54c0439d061075a1c58':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-residual-ppo-002-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_043/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then three cold directional reference cases; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-044', '--on-active=50m',
