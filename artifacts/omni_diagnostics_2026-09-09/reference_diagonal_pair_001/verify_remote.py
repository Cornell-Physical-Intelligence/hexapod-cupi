...
source=base/'reference_diagonal_pair_source_001';run=base/'reference_diagonal_pair_001';pause=base/'forecast_pause_047'
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
assert sha(source/'campaign_source_hashes.json')=='26ff2b7f8e57b3a6ea69dae09be58b26c2b0967e92103ed3f2673c0f32ae9625'
m=json.loads((source/'campaign_source_hashes.json').read_text());assert len(m)==934
...
unit=subprocess.check_output(['systemctl','--user','show','hexapod-diagonal-pairs-001-20260910.service','-p','ActiveState','-p','SubState','-p','Result'],text=True)
