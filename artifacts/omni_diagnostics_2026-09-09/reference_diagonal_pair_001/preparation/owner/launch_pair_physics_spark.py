#!/usr/bin/env python3
"""Fresh full-C standing then two cold diagonal-pair static-transfer diagnostics; no wave/PPO."""
import argparse
import fcntl
import hashlib
import json
...
from pair_screen_contract import PAIR_PROTOCOL, PAIR_CASES
...
def command(source, output, name, phase):
    if phase not in ("standing", *PAIR_CASES):
        raise ValueError("Only fresh standing and bounded opposing-pair diagnostic phases exist")
    common = ["docker", "compose", "--env-file", "docker/.env.base", "-f", "docker/docker-compose.yaml",
        ...
        "/workspace/hexapod/tools/" + ("run_reference_physics.py" if phase == "standing" else "run_pair_physics.py"), "--mode", "standing" if phase == "standing" else "pair",
        "--num-envs", "32" if phase == "standing" else "1",
        "--steps", "1000" if phase == "standing" else "1300",
        "--geometry-reference", "/workspace/hexapod/" + GEOMETRY_REFERENCE]
    if phase in PAIR_CASES:
        common += ["--pair-case", phase, "--admission", "/outputs/standing/admission.json"]
...
def main():
    ...
    report = dict(status="preparing", runtime_binding=runtime, started_unix=time.time(),
        ...
        source_manifest_sha256=digest(args.source / "campaign_source_hashes.json"), automatic_continuation=False, solver_comparison=SOLVER_PROTOCOL, pair_protocol=PAIR_PROTOCOL)
    ...
    try:
        ...
        report['cases'] = {}
        for case in PAIR_PROTOCOL['case_order']:
            paired = run_owned(args, case)
            pair_identity = paired.get('identity', {})
            if (pair_identity.get('standing_identity') != identity or pair_identity.get('pair_protocol') != PAIR_PROTOCOL
                    or pair_identity.get('pair_case') != case or pair_identity.get('pair_partition') != PAIR_CASES[case]
                    or paired.get('pair_case') != case or paired.get('pair_partition') != PAIR_CASES[case]
                    or pair_identity.get('standing_admission_sha256') != digest(args.output / 'standing/admission.json')
                    or paired.get('proposed_diagnostic_criteria_met') is not True or paired.get('control_steps') != 1300
                    or paired.get('diagnostic_controls') != 1100
                    or paired.get('diagnostic_result', {}).get('proposed_criteria_met') is not True):
                raise RuntimeError('Diagonal diagnostic does not meet its distinct named/source-bound proposed criteria')
            pair_solver = json.loads((args.output / case / 'solver_comparison.json').read_text())
            if pair_solver.get('protocol') != SOLVER_PROTOCOL or pair_solver.get('actual_scene_attributes') != comparison.get('actual_scene_attributes'):
                raise RuntimeError('Pair solver identity/readback differs from standing admission')
            if tree_hashes(package) != before:
                raise RuntimeError('Pair diagnostic changed its read-only admitted asset')
            check_source(args.source)
            report['cases'][case] = {'state_sha256':digest(args.output / case / 'state.json'),'proposed_criteria_met':True}
            save(args.output / 'campaign.json', report)
        report.update(status='completed', proposed_pair_diagnostic_criteria_met=True, physics_qualification=False,
