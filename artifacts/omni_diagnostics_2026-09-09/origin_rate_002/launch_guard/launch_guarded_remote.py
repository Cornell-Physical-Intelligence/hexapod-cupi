...
SOURCE = BASE / 'reference_origin_source_002'
OUTPUT = BASE / 'origin_rate_002'
PAUSE = BASE / 'forecast_pause_038'
UNIT = 'hexapod-origin-rate-002-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'c028664ae782b053f0e126ace1e7a24e6eb8a89cf8dbaa9fd8b5628d28560160':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-origin-rate-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_037/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-038', '--on-active=75m',
