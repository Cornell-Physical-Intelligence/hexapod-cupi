...
def _same_tensor_view(a,b):
 import torch
 return (isinstance(a,torch.Tensor) and isinstance(b,torch.Tensor)
  and a.device==b.device and a.dtype==b.dtype and tuple(a.shape)==tuple(b.shape)
  and a.storage_offset()==b.storage_offset() and tuple(a.stride())==tuple(b.stride())
  and a.untyped_storage().data_ptr()==b.untyped_storage().data_ptr())


def _view_description(value):
 return {'shape':list(value.shape),'stride':list(value.stride()),'storage_offset':value.storage_offset(),
  'device':str(value.device),'dtype':str(value.dtype)}


def synchronize_origin_views(env,xy):
 """Synchronize separate reset-origin stores, preserving original Z and pose quaternion."""
 import torch
 if int(env._sim_step_counter)!=0:raise RuntimeError('Origin metadata may only change inside cold reset')
 terrain=env._terrain.env_origins;scene=env.scene.env_origins
 if any(not isinstance(v,torch.Tensor) or tuple(v.shape)!=(1,3) for v in (terrain,scene)):
  raise RuntimeError('Explicit writable single-replica Torch origin views required')
 terrain_before=terrain.clone();scene_before=scene.clone()
 default_pose=getattr(env.scene,'_default_env_pose',None)
 if default_pose is not None and (not isinstance(default_pose,torch.Tensor) or tuple(default_pose.shape)!=(1,7)):
  raise RuntimeError('Unexpected scene default pose metadata; preserve before dispatch')
 pose_before=None if default_pose is None else default_pose.clone()
 expected_xy=torch.as_tensor(xy,dtype=terrain.dtype,device=terrain.device).reshape(1,2)
 terrain[:,:2]=expected_xy
 scene[:,:2]=expected_xy.to(device=scene.device,dtype=scene.dtype)
 after_terrain=env._terrain.env_origins;after_scene=env.scene.env_origins
 checks={'cold_reset_counter0':int(env._sim_step_counter)==0,
  'terrain_written_view_persists':_same_tensor_view(terrain,after_terrain),
  'scene_written_view_persists':_same_tensor_view(scene,after_scene),
  'terrain_xy_exact':torch.equal(after_terrain[:,:2],expected_xy),
  'scene_xy_exact':torch.equal(after_scene[:,:2],expected_xy.to(after_scene)),
  'terrain_z_preserved':torch.equal(after_terrain[:,2:],terrain_before[:,2:]),
  'scene_z_preserved':torch.equal(after_scene[:,2:],scene_before[:,2:]),
  'default_pose_z_quaternion_preserved':default_pose is None or torch.equal(default_pose[:,2:],pose_before[:,2:])}
 report={'checks':checks,'passed':all(checks.values()),'terrain_view':_view_description(after_terrain),
  'scene_view':_view_description(after_scene),'terrain_and_scene_same_view_storage':_same_tensor_view(after_terrain,after_scene),
  'terrain_before_m':array(terrain_before).tolist(),'scene_before_m':array(scene_before).tolist(),
  'terrain_after_m':array(after_terrain).tolist(),'scene_after_m':array(after_scene).tolist(),
  'scene_terrain_registered':getattr(env.scene,'_terrain',None) is not None,
  'default_pose_before':None if pose_before is None else array(pose_before).tolist(),
  'default_pose_after':None if default_pose is None else array(default_pose).tolist()}
 env._matched_origin_written_views=(terrain,scene)
 env.matched_origin_origin_sync=report
 if not report['passed']:raise RuntimeError('Origin write/re-read storage or metadata preservation failed: '+repr(report))
 return report
...
def matched_class(base,payload,case):
 ...
 class MatchedOriginEnvironment(base):
  def _reset_idx(self,env_ids):
   ...
   synchronize_origin_views(self,CASES[case])
...
def initial_readback(env,payload,case):
 ...
 checks['origin_sync_verified']=getattr(env,'matched_origin_origin_sync',{}).get('passed') is True
 checks['scene_origin_written_view_retained']=_same_tensor_view(env.scene.env_origins,env._matched_origin_written_views[1])
 checks['terrain_origin_written_view_retained']=_same_tensor_view(env._terrain.env_origins,env._matched_origin_written_views[0])
 checks['scene_origin_stable_view']=_same_tensor_view(env.scene.env_origins,env.scene.env_origins)
 checks['terrain_origin_stable_view']=_same_tensor_view(env._terrain.env_origins,env._terrain.env_origins)
 checks['terrain_origins']=np.array_equal(array(env._terrain.env_origins),np.array([[*CASES[case],0]],dtype=np.float32))
 ...
 result={'case':case,'checks':checks,'passed':all(checks.values()),'actual':{k:v.tolist() for k,v in actual.items()},
  ...
  'origin_storage_sync':env.matched_origin_origin_sync,'reset_injections':getattr(env,'matched_origin_reset_injections',0),'body_pose_written_after_first_step':False}
 return result
