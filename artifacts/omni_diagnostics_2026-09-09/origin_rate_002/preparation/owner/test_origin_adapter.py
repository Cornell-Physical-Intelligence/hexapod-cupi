...
from matched_origin import expected_initial,matched_class,initial_readback,validate_ground_rows,_same_tensor_view
...
class OriginTests(unittest.TestCase):
 ...
 def test_synchronized_storage_replacement_rejected(self):
...
class PublicFallbackScene:
 def __init__(self):
  self._terrain=None;self._default_env_pose=torch.tensor([[0.,0.,0.,0.,0.,0.,1.]])
 @property
 def env_origins(self):return self._default_env_pose[:,:3]

class FallbackBase(Base):
 def __init__(self):super().__init__();self.scene=PublicFallbackScene()

class OriginViewRegression(unittest.TestCase):
 def test_distinct_stores_public_slicing_view_and_preserved_pose_metadata(self):
  e=matched_class(FallbackBase,P,'far')()
  self.assertIsNot(e.scene.env_origins,e.scene.env_origins)
  self.assertTrue(_same_tensor_view(e.scene.env_origins,e.scene.env_origins))
  self.assertFalse(_same_tensor_view(e.scene.env_origins,e._terrain.env_origins))
  old=e.scene._default_env_pose.clone();e._reset_idx(None)
  np.testing.assert_array_equal(e.scene.env_origins.numpy(),[[30.,-50.,0.]])
  np.testing.assert_array_equal(e._terrain.env_origins.numpy(),[[30.,-50.,0.]])
  self.assertTrue(torch.equal(old[:,2:],e.scene._default_env_pose[:,2:]))
  self.assertTrue(initial_readback(e,P,'far')['passed'])
  self.assertFalse(e.matched_origin_origin_sync['terrain_and_scene_same_view_storage'])
 def test_copy_returning_getter_fails_even_when_origin_values_are_zero(self):
  class CopyScene(PublicFallbackScene):
   @property
   def env_origins(self):return self._default_env_pose[:,:3].clone()
  e=matched_class(FallbackBase,P,'origin_a')();e.scene=CopyScene()
  with self.assertRaises(RuntimeError):e._reset_idx(None)
 def test_nonzero_original_z_is_preserved_then_rejected_by_exact_plan(self):
  e=matched_class(FallbackBase,P,'near')();e.scene._default_env_pose[0,2]=.25
  e._reset_idx(None);self.assertEqual(e.scene.env_origins[0,2].item(),.25)
  self.assertFalse(initial_readback(e,P,'near')['passed'])
 def test_actual001_alias_only_failure_is_retained(self):
  raw=HERE.parent/'reference_origin_results_001/run/origin_a/initial_readback.json'
  r=json.loads(raw.read_text());self.assertEqual([k for k,v in r['checks'].items() if not v],['scene_origin_alias'])
  self.assertEqual(r['actual_scene_origins_m'],[[0.,0.,0.]])
