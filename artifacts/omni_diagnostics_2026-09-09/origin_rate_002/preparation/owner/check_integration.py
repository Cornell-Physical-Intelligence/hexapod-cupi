...
HERE=Path(__file__).resolve().parent;source=HERE/'source_origin_002'
...
args.mode='origin';args.num_envs=1;args.case='origin_a';args.admission=HERE.parent/'reference_origin_results_001/run/standing/admission.json'
try:preflight(args,source)
except ValueError:old_rejected=True
else:raise AssertionError('Actualorigin001 admission cannot authorize a fresh origin source')
...
result={'runtime':runtime,'source_manifest_sha256':host.digest(source/'campaign_source_hashes.json'),'source_files':len(json.loads((source/'campaign_source_hashes.json').read_text())),
 'actual_origin001_old_admission_rejected':old_rejected,'failed_quiet_synthetic_receipt_rejected':failed_quiet_rejected,'declared_cases':checks,
