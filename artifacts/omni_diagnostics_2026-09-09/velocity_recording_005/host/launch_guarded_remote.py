...
OUTPUT = BASE / 'omni_velocity_recording_005'
PAUSE = BASE / 'forecast_pause_024'
UNIT = 'hexapod-omni-velocity-recording-005-20260910.service'
...
def main():
    ...
    validate_inputs(SimpleNamespace(source=SOURCE,probe=BASE/'omni_velocity_probe_003',
        pilot=BASE/'omni_velocity_pilot_003',adapter=BASE/'omni_velocity_recording_adapter_005'))
    previous = call(['systemctl','--user','show','hexapod-omni-velocity-recording-004-20260910.service','-p','ActiveState'])
    if 'ActiveState=active' in previous or 'ActiveState=activating' in previous:
        raise RuntimeError('Previous recorder owner has not exited; do not overlap recording')
    if not (BASE/'forecast_pause_023/restored.json').is_file():
        raise RuntimeError('Previous recorder forecasting restoration not yet recorded')
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Bounded actual final50-update PPO recording with labels; no training or qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-024', '--on-active=20m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=720',
            ...
            '/usr/bin/python3', str(Path(__file__).resolve().parent / 'launch_recording_spark.py'),
            '--source', str(SOURCE), '--probe', str(BASE/'omni_velocity_probe_003'),
            '--pilot', str(BASE/'omni_velocity_pilot_003'), '--adapter', str(BASE/'omni_velocity_recording_adapter_005'),
            '--output', str(OUTPUT)]
