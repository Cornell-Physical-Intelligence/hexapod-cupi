...
SOURCE = BASE / 'reference_physics_source_005'
OUTPUT = BASE / 'reference_physics_005'
PAUSE = BASE / 'forecast_pause_031'
UNIT = 'hexapod-reference-physics-005-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'c557b71636c8b1e10883ab8a6b2b49f40b661988f9b09faeaefe4821a72844b4':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-004-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_030/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Isolated full-C standing solver comparison; no wave, PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-031', '--on-active=25m',
