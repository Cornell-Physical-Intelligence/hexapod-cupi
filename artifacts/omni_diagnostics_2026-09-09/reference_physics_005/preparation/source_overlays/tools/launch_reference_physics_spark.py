#!/usr/bin/env python3
"""Standing-only full-C TGS solver comparison; no wave, PPO or automatic continuation."""
import argparse
import fcntl
import hashlib
import json
...
def command(source, output, name, phase):
    if phase != "standing":
        raise ValueError("Source005 permits only the standing solver comparison")
...
def main():
    ...
    if args.output.exists():
        parser.error("Use a fresh output; standing diagnostic never resumes automatically")
    ...
    report = dict(status="preparing", runtime_binding=runtime, started_unix=time.time(),
        ...
        source_manifest_sha256=digest(args.source / "campaign_source_hashes.json"), automatic_continuation=False,
        solver_comparison=PROTOCOL, bounded_wave_physics_passed=False)
    ...
    try:
        ...
        if (identity.get("solver_comparison") != PROTOCOL or comparison.get("protocol") != PROTOCOL
            ...
            raise RuntimeError("Standing solver comparison identity/readback mismatch")
        report.update(status="completed", source_unchanged=True, admitted_asset_unchanged=True,
                      standing_solver_comparison_completed=True, bounded_wave_physics_passed=False,
                      solver_readback_sha256=digest(args.output / "standing/solver_comparison.json"),
                      substep_review_sha256=digest(args.output / "standing/physics_substep_review.json"))
    except Exception as exc:
        report.update(status="stopped" if isinstance(exc, InterruptedError) else "failed", error=repr(exc))
        raise
    finally:
