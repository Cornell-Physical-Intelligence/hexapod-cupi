"""Explicit ideal synthetic measured pose/contact fixture, never actual physics evidence.
Body follows virtual command by assumption; forces are illustrative equal weight shares.
"""
import unittest
...
class IdealMeasuredFixture:
    def __init__(self, names=None, preload=.0):
        ...
        self.joint_deflection=self.q-self.qtarget[self.to_leg].reshape(6,3)
    ...
    def advance(self, output):
        ...
        self.qtarget = output['q_ref'][0].copy(); self.q = self.qtarget[self.to_leg].reshape(6, 3)+self.joint_deflection
        self.v = output['v_ref'][0].copy(); self.time = output['target_time_s']
        self.p=np.asarray(output['state']['desired_position_world_m']).copy()
        self.R=np.asarray(output['state']['initial_rotation_world_from_body']).copy()
        self.cmd=np.asarray(output['admitted_command']).copy()
