...
class PairMotionTests(unittest.TestCase):
 ...
 def test_first_confirmed_foot_must_remain_supported_until_partner_finishes(self):
  def mutate(k,g,f,row):
   if 3.96<=k*.02<4.26:force_contact(row,4,False)
   if g.active_pair and g.foot_cycles[1].current_leg is None:force_contact(row,1,False)
  r,rows,g,f=evaluate(duration=5,mutate=mutate);self.assertIsNotNone(r['failure']);self.assertIn('before pair handoff',r['failure']['reason']);self.assertEqual(g.completed_pairs,0)
