...
from pair_motion_measurements import check_sensor_row,torque_window,measured_support
...
def replay_pair_reference(data,references,joint_names,physics=None,support_checks=None):
 from pair_motion import PairContactReference
 g=PairContactReference(joint_names);max_difference=0.;count=0;reset_seen=False;postchecks=0
 supplied={} if support_checks is None else {r['physical_step']:r for r in support_checks}
 for row in references:
  ...
  if 'result' in row and step<len(data['time_s']) and step in supplied:
   try:
    torques=torque_window([{k:v[i] for k,v in physics.items()} for i in range(step*8+1,(step+1)*8+1)])
    check=measured_support(g,{k:v[step] for k,v in data.items()})
    reproduced={'physical_step':step,'support':check,'torque':torques}
   except ValueError as error:
    reproduced={'physical_step':step,'failure':str(error)}
   difference=numeric_tree_difference(reproduced,supplied[step]);max_difference=max(max_difference,difference)
   if difference>PROTOCOL['numerical_replay_absolute_tolerance']:raise ValueError('Independent raw poststep support/torque replay differs')
   postchecks+=1
 return {'reset_seen':reset_seen,'reference_controls_replayed':count,'max_absolute_difference':max_difference,
         'absolute_tolerance':PROTOCOL['numerical_replay_absolute_tolerance'],'complete_state_including_each_foot_replayed':True,'poststep_checks_independently_replayed':postchecks,
...
def score_pair_motion(data,references,physics,support_checks,*,joint_names,failure=None,dt=.02):
 ...
 replay=replay_pair_reference(data,references,joint_names,physics,support_checks)
