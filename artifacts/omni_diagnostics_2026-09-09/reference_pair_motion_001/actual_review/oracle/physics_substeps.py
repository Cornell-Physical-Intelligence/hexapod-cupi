...
def measured_substep(env):
    ...
    return {'root_link_position_world_m':array(d.root_link_pos_w),
            ...
            'joint_position_rad':array(d.joint_pos)}
...
class PhysicsSubstepRecorder:
    ...
    def __init__(self,env,*,reader=measured_substep):
        ...
        self.identity={'articulation_data_class':type(d).__module__+'.'+type(d).__qualname__,
                       ...
                       'joint_kinematics_schema':'actual_named_joint_position_rad_and_velocity_rad_s_each_update',
    ...
    def _capture(self,control,substep):
        ...
        if list(self.env._robot.joint_names)!=self.identity['joint_names_runtime']:
            raise RuntimeError('Articulation joint order changed during substep observation')
