...
ROOT=Path('tmp/direct_omni_train_pilot_caps_results_001');ROOT.mkdir(exist_ok=False)
AUDIT=Path('tmp/direct_omni_train_pilot_caps_terminal_audit_001.json');a=json.loads(AUDIT.read_text());BASE='/home/orionh/HEXAPOD_runs/mock_length_study_20260909'
...
needed=sum(a['raw_sizes_bytes'][k] for k in selected);free=shutil.disk_usage(ROOT).free;reserve=512<<20
...
for key in sorted(selected,key=lambda k:(priority(k),a['raw_sizes_bytes'][k],k)):
 ...
 prefix,relative=key.split('/',1);remote=BASE+'/'+('direct_omni_train_pilot_caps_001' if prefix=='run' else 'forecast_pause_057')+'/'+relative
