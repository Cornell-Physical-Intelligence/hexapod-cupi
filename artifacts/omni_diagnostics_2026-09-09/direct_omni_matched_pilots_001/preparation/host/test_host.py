...
ROOT=HERE.parents[1]
CONTRACT=ROOT/'tmp/direct_omni_recovery_001/native_003'
...
def args(out):return SimpleNamespace(source=Path('/source-real'),checkpoint=Path('/original.pt'),contract=CONTRACT,supervisor_source=PARENT,output=out,isaaclab=Path('/IsaacLab'),host_freeze_sha256='host',allocation='smoke',branch='caps',smoke=None)
class HostTests(unittest.TestCase):
 ...
 def test_smoke_keeps_final_diagnostics_and_requires_caps(self):
  ...
  a.branch='caps';a.allocation='pilot'
 ...
 def _campaign(self,fail=None,alter=False,allocation='smoke'):
  temp=tempfile.TemporaryDirectory();out=Path(temp.name);a=args(out);a.allocation=allocation
  if allocation=='pilot':a.smoke=out/'prior_smoke';a.smoke.mkdir();(a.smoke/'receipt.json').write_text('{}')
  a.checkpoint=out/'original.pt';a.checkpoint.write_bytes(b'original');identity={'x':1};calls=[]
  class Contract:
  ...
  def run(a,phase):
   ...
   if phase=='train':
    ...
    (out/phase/'training_receipt.json').write_text(json.dumps({'complete':True,'final_checkpoint_sha256':h.sha(cp),'updates_completed':2 if allocation=='smoke' else 50}))
