"""Explicit independent CAPS smoke / curriculum or CAPS pilot, with cold diagnostics."""
from pathlib import Path
from types import SimpleNamespace
import argparse,hashlib,importlib.util,json,signal,sys,time
sys.dont_write_bytecode=True
SOURCE_MAP='64144b2c2466f17c809554ef53f82b736b4199136cc821451d7ac2788752bd2e'
LEGACY_RUNTIME_TREE='abe4e3542c7af7093f4a203b0db3631706a0b981f77befb40b243559b7a69280'
CONTRACT_FREEZE='20fbcd40a869b39d8e3143a6c9715ec62ada5cc6211eaf473f60f8b7167678cb'
...
def selected_phases(args):
 if args.allocation not in PHASES or args.branch not in ('caps','curriculum'):raise ValueError('Explicit bounded allocation/branch required')
 if args.allocation=='smoke' and args.branch!='caps':raise ValueError('Only CAPS smoke is allocated')
 if args.allocation=='pilot' and getattr(args,'smoke',None) is None:raise ValueError('Pilot requires separate completed smoke proof')
...
def command(args,name,phase):
 ...
 if args.allocation=='pilot':cli+=['-v',str(args.smoke)+':/smoke:ro']
...
def run_campaign(args,identity):
 ...
 smoke_map=tree_hashes(args.smoke) if args.allocation=='pilot' else None
 def unchanged():
 ...
 try:
  for phase in phases:
   ...
   if phase=='train':report['PPO_updates_completed']=2 if args.allocation=='smoke' else 50
...
def main():
 ...
 p.add_argument('--allocation',choices=tuple(PHASES),required=True);p.add_argument('--branch',choices=('curriculum','caps'),required=True)
