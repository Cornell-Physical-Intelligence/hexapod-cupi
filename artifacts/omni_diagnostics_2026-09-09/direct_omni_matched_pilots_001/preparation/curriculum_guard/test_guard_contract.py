...
class GuardTests(unittest.TestCase):
 def setUp(self):
  ...
  fixtures={
   ...
   'forecast_pause_055/pause.json':dict(unit=g.PREVIOUS_UNIT,units={'weather.timer':'ActiveState=active','other.timer':'ActiveState=inactive'}),
  ...
  for name,value in [('BASE',self.base),('SMOKE',self.base/'direct_omni_train_smoke_002'),('PRIOR_PINS',pins)]:
 ...
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_train_smoke_002/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_055/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_full_preflight_identity_and_exact_pilot_selection(self):
  identity=dict(source_manifest_sha256=g.SOURCE_SHA256,checkpoint_sha256=g.CHECKPOINT_SHA256,actor_width=315,critic_width=318,smoke_campaign_sha256=g.PRIOR_PINS['direct_omni_train_smoke_002/campaign.json'],selection={'schema':'direct315_stand_stop_caps_native_v2','allocation':'pilot','branch':'curriculum','replicas':1024,'controls_per_update':24,'updates':50,'caps':{'temporal_weight':0.,'spatial_weight':0.,'noise_scale':1.,'noise_seed':1157}})
  ...
  args=host.verify_inputs.call_args.args[0];self.assertEqual(args.supervisor_source,g.SUPERVISOR_SOURCE);self.assertEqual((args.allocation,args.branch,args.smoke),('pilot','curriculum',g.SMOKE))
 def test_pending_actual_prior_hash_rejects_even_with_other_bindings_complete(self):
  with patch.object(g,'PRIOR_PINS',{**g.PRIOR_PINS,'direct_omni_train_smoke_002/jobs/train.json':'PENDING_TERMINAL_SMOKE'}):
 ...
 def test_exact_prior_seven_receipts_required(self):
  with patch.object(g,'PRIOR_PINS',{k:v for k,v in g.PRIOR_PINS.items() if k!='direct_omni_train_smoke_002/jobs/final_stop.json'}):
 ...
 def test_failed_train_or_missing_final_diagnostics_reject(self):
  ...
  for change in [{'PPO_updates_completed':1},{'allocation':'pilot'},{'branch':'curriculum'},{'planned_phases':['standing','train']},{'accepted_phases':{'standing':{}}},{'identity':{'source_manifest_sha256':'a'*64}}]:
   path.write_text(json.dumps({**original,**change}));pins={**g.PRIOR_PINS,'direct_omni_train_smoke_002/campaign.json':g.sha(path)}
 ...
 def test_missing_prior_job_cleanup_rejects(self):
  name='direct_omni_train_smoke_002/jobs/final_stop.json';path=self.base/name;d=json.loads(path.read_text());d['cleanup_checked']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_six_phase_pilot_cli_and_bounds(self):
  ...
  self.assertFalse(set(cmd)&{'learn','--decision-receipt','--phase-group'});self.assertEqual(cmd[cmd.index('--allocation')+1],'pilot');self.assertEqual(cmd[cmd.index('--branch')+1],'curriculum');self.assertEqual(cmd[cmd.index('--smoke')+1],str(g.SMOKE))
...
class RuntimePendingTests(unittest.TestCase):
 ...
 def test_actual_frozen_host_selects_exact_six_phases(self):
  ...
  self.assertEqual(host.selected_phases(SimpleNamespace(allocation='pilot',branch='curriculum',smoke=g.SMOKE)),('standing','initial_constant','initial_stop','train','final_constant','final_stop'))
  with self.assertRaisesRegex(ValueError,'completed smoke'):host.selected_phases(SimpleNamespace(allocation='pilot',branch='curriculum',smoke=None))
