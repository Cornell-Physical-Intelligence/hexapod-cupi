"""External pause056 guard: bounded direct315 curriculum pilot after verified smoke."""
...
OUTPUT=BASE/'direct_omni_train_pilot_curriculum_001'
PAUSE=BASE/'forecast_pause_056'
UNIT='hexapod-direct-omni-train-pilot-curriculum-001-20260910.service'
...
def require_final_bindings():
    ...
    if set(PRIOR_PINS)!={'direct_omni_train_smoke_002/campaign.json',
...
def validate_train_inputs(train_host):
    ...
    args=SimpleNamespace(source=SOURCE,checkpoint=CHECKPOINT,contract=CONTRACT,supervisor_source=SUPERVISOR_SOURCE,output=OUTPUT,isaaclab=Path('/home/orionh/IsaacLab'),host_freeze_sha256=HOST_FREEZE_SHA256,allocation='pilot',branch='curriculum',smoke=SMOKE)
    ...
    expected={'schema':'direct315_stand_stop_caps_native_v2','allocation':'pilot','branch':'curriculum','replicas':1024,'controls_per_update':24,'updates':50,'caps':{'temporal_weight':0.,'spatial_weight':0.,'noise_scale':1.,'noise_seed':1157}}
    if identity.get('selection')!=expected or (identity.get('actor_width'),identity.get('critic_width'))!=(315,318):raise RuntimeError('Host selection differs from bounded fifty-update curriculum pilot')
    if identity.get('smoke_campaign_sha256')!=PRIOR_PINS['direct_omni_train_smoke_002/campaign.json']:raise RuntimeError('Host admitted different prior smoke campaign')
    return identity
...
def main():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Bounded direct315 curriculum pilot: fresh standing, original-policy constant and stop diagnostics,50 updates with1024 replicas and24 controls per update, final constant and stop diagnostics; no automatic continuation or Stage2 performance admission',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-056', '--on-active=70m',
    ...
    try:
        cmd=['systemd-run','--user','--unit='+UNIT,'--property=RuntimeMaxSec=3720',
            ...
            '--allocation','pilot','--branch','curriculum','--smoke',str(SMOKE)]
