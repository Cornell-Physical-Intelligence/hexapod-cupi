...
class GuardTests(unittest.TestCase):
 def setUp(self):
  ...
  fixtures={
   'direct_omni_train_smoke_002/campaign.json':dict(status='completed',terminal_inputs_unchanged=True,bounded_campaign_complete=True,allocation='smoke',branch='caps',PPO_updates_completed=2,planned_phases=['standing','train','final_constant','final_stop'],accepted_phases={k:{} for k in ['standing','train','final_constant','final_stop']},host_freeze_sha256=g.HOST_FREEZE_SHA256,identity={'source_manifest_sha256':g.SOURCE_SHA256}),
   'forecast_pause_055/pause.json':dict(unit=g.SMOKE_UNIT,units={'weather.timer':'ActiveState=active','other.timer':'ActiveState=inactive'}),
   'forecast_pause_055/restored.json':dict(timers=['weather.timer'],restored_unix=123)}
  for i,phase in enumerate(('standing','train','final_constant','final_stop')):
   fixtures['direct_omni_train_smoke_002/jobs/'+phase+'.json']=dict(status='completed',phase=phase,cleanup_checked=True,container_name='test-only-'+phase,container_id=str(i+1)*64)
  pins={}
  ...
  for name,value in [('BASE',self.base),('SMOKE',self.base/'direct_omni_train_smoke_002'),('SMOKE_PINS',pins)]:
   p=patch.object(g,name,value);p.start();self.addCleanup(p.stop)
  for name,value in [('PREVIOUS_ATTEMPTED_PHASES',('standing',)),('PRIOR_PINS',{'direct_omni_train_pilot_curriculum_001/campaign.json':'a'*64,'forecast_pause_056/pause.json':'b'*64,'forecast_pause_056/restored.json':'c'*64,'direct_omni_train_pilot_curriculum_001/jobs/standing.json':'d'*64})]:
   p=patch.object(g,name,value);p.start();self.addCleanup(p.stop)
  self.unit='ActiveState=failed\nSubState=failed\nInvocationID='+g.SMOKE_INVOCATION
 ...
 def test_exact_completed_previous_eight_absences(self):
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run',side_effect=self.absent) as run:g.verify_smoke_owner()
  self.assertEqual(run.call_count,8)
 def test_collected_unit_with_exact_receipts_allowed(self):
  with patch.object(g,'call',return_value='ActiveState=inactive\nInvocationID='),patch.object(g.subprocess,'run',side_effect=self.absent):g.verify_smoke_owner()
 def test_active_or_conflicting_invocation_fails(self):
  for value in [self.unit.replace('ActiveState=failed','ActiveState=active'),self.unit.replace(g.SMOKE_INVOCATION,'a'*32)]:
   with patch.object(g,'call',return_value=value),patch.object(g.subprocess,'run') as run:
    with self.assertRaises(RuntimeError):g.verify_smoke_owner()
    run.assert_not_called()
 def test_changed_restoration_fails(self):
  (self.base/'forecast_pause_055/restored.json').write_text('{}')
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'receipt changed'):g.verify_smoke_owner()
   run.assert_not_called()
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_train_smoke_002/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.SMOKE_PINS,name:g.sha(path)}
  with patch.object(g,'SMOKE_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'not complete'):g.verify_smoke_owner()
   run.assert_not_called()
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_055/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.SMOKE_PINS,name:g.sha(path)}
  with patch.object(g,'SMOKE_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'timer restoration'):g.verify_smoke_owner()
   run.assert_not_called()
 def test_container_live_or_unknown_not_absence(self):
  for r in [subprocess.CompletedProcess([],0,'x /owner false',''),subprocess.CompletedProcess([],1,'','daemon unavailable')]:
   with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run',return_value=r):
    with self.assertRaisesRegex(RuntimeError,'not proven absent'):g.verify_smoke_owner()
 ...
 def test_full_preflight_identity_and_exact_pilot_selection(self):
  identity=dict(source_manifest_sha256=g.SOURCE_SHA256,checkpoint_sha256=g.CHECKPOINT_SHA256,actor_width=315,critic_width=318,smoke_campaign_sha256=g.SMOKE_PINS['direct_omni_train_smoke_002/campaign.json'],selection={'schema':'direct315_stand_stop_caps_native_v2','allocation':'pilot','branch':'caps','replicas':1024,'controls_per_update':24,'updates':50,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157}})
  ...
  with patch.object(g,'sha',side_effect=lambda p:d[p]),patch.object(g,'HOST_SHA256','a'*64),patch.object(g,'verify_frozen') as verify:
   ...
   with self.assertRaisesRegex(RuntimeError,'fifty-update'):g.validate_train_inputs(host)
   identity['selection']['updates']=50;host.selected_phases.return_value=('standing','train')
   with self.assertRaisesRegex(RuntimeError,'phase order'):g.validate_train_inputs(host)
  args=host.verify_inputs.call_args.args[0];self.assertEqual(args.supervisor_source,g.SUPERVISOR_SOURCE);self.assertEqual((args.allocation,args.branch,args.smoke),('pilot','caps',g.SMOKE))
 def test_pending_actual_prior_hash_rejects_even_with_other_bindings_complete(self):
  with patch.object(g,'SMOKE_PINS',{**g.SMOKE_PINS,'direct_omni_train_smoke_002/jobs/train.json':'PENDING_TERMINAL_SMOKE'}):
 ...
 def test_exact_prior_seven_receipts_required(self):
  with patch.object(g,'SMOKE_PINS',{k:v for k,v in g.SMOKE_PINS.items() if k!='direct_omni_train_smoke_002/jobs/final_stop.json'}):
 ...
 def test_failed_train_or_missing_final_diagnostics_reject(self):
  path=self.base/'direct_omni_train_smoke_002/campaign.json';original=json.loads(path.read_text())
  for change in [{'PPO_updates_completed':1},{'allocation':'pilot'},{'branch':'curriculum'},{'planned_phases':['standing','train']},{'accepted_phases':{'standing':{}}},{'identity':{'source_manifest_sha256':'a'*64}}]:
   path.write_text(json.dumps({**original,**change}));pins={**g.SMOKE_PINS,'direct_omni_train_smoke_002/campaign.json':g.sha(path)}
   with self.subTest(change=change),patch.object(g,'SMOKE_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
    with self.assertRaisesRegex(RuntimeError,'allocation/source/phase'):g.verify_smoke_owner()
    run.assert_not_called()
 def test_missing_prior_job_cleanup_rejects(self):
  name='direct_omni_train_smoke_002/jobs/final_stop.json';path=self.base/name;d=json.loads(path.read_text());d['cleanup_checked']=False;path.write_text(json.dumps(d));pins={**g.SMOKE_PINS,name:g.sha(path)}
  with patch.object(g,'SMOKE_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run',side_effect=self.absent):
   with self.assertRaisesRegex(RuntimeError,'cleanup proof'):g.verify_smoke_owner()
 ...
 def test_six_phase_pilot_cli_and_bounds(self):
  ...
  self.assertIn('--property=RuntimeMaxSec=3720',cmd);self.assertIn('--property=TimeoutStopSec=180',cmd)
  self.assertEqual(cmd[cmd.index('--checkpoint')+1],str(g.CHECKPOINT))
  self.assertFalse(set(cmd)&{'learn','--decision-receipt','--phase-group'});self.assertEqual(cmd[cmd.index('--allocation')+1],'pilot');self.assertEqual(cmd[cmd.index('--branch')+1],'caps');self.assertEqual(cmd[cmd.index('--smoke')+1],str(g.SMOKE))
  self.assertIn('--on-active=70m',[n.value for n in ast.walk(t) if isinstance(n,ast.Constant)])
...
class RuntimePendingTests(unittest.TestCase):
 def test_real_unbound_draft_fails_before_any_external_effect(self):
  if all(len(v)==64 for v in [*g.SMOKE_PINS.values(),*g.PRIOR_PINS.values()]):
   g.require_final_bindings()
   for name,bound in g.SMOKE_PINS.items():self.assertEqual(g.sha(HERE/'smoke_owner'/name),bound)
   return
 ...
 def test_actual_frozen_host_selects_exact_six_phases(self):
  spec=importlib.util.spec_from_file_location('actual_pilot_host_review',HERE.parent/'direct_omni_train_host_002/launch_train_spark.py')
  host=importlib.util.module_from_spec(spec);spec.loader.exec_module(host)
  self.assertEqual(host.selected_phases(SimpleNamespace(allocation='pilot',branch='caps',smoke=g.SMOKE)),('standing','initial_constant','initial_stop','train','final_constant','final_stop'))
  with self.assertRaisesRegex(ValueError,'completed smoke'):host.selected_phases(SimpleNamespace(allocation='pilot',branch='caps',smoke=None))
