...
class NativeTests(unittest.TestCase):
    def test_shared_allocation_and_branch(self):
        ...
        for allocation,n,updates in [('smoke',32,2),('pilot',1024,50)]:
            for branch in ['curriculum','caps']:
                ...
                self.assertEqual(result['algorithm']['caps_options']['temporal_weight'],.1 if branch=='caps' else 0.)
    ...
    def test_scope_command_checkpoint_mounts(self):
        for phase in ['standing','train','final_constant','final_stop']:
            argv=runtime_arguments(phase,'smoke','caps')
        ...
        for phase in ['initial_constant','initial_stop']:
            with self.assertRaises(ValueError): runtime_arguments(phase,'smoke','caps')
    ...
    def test_frozen_behavior_bytes_reused(self):
        ...
        for name in ['caps.py','caps_ppo.py','curriculum.py']:
    ...
    def test_actual_late_app_flags_parse_before_launch(self):
        ...
        old=sys.argv
        try:
            sys.argv=['entry',*runtime_arguments('train','smoke','caps')[1:],'--info']
    ...
    def test_completed_smoke_campaign_boundary_not_just_phase_states(self):
        ...
        identity={'source':'bound','selection':selection('train','smoke','caps',None,None)}
        phases=['standing','train','final_constant','final_stop']
        receipts={p:{'phase':p,'checkpoint_sha256':'c'*64} for p in phases}
        campaign={'status':'completed','terminal_inputs_unchanged':True,'allocation':'smoke','branch':'caps','identity':identity,'planned_phases':phases,'accepted_phases':receipts}
