...
class GuardTests(unittest.TestCase):
 ...
 def test_changed_restoration_fails(self):
  (self.base/'forecast_pause_054/restored.json').write_text('{}')
 ...
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_train_smoke_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_054/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_exact_failed_train_phase_not_relabelled_completed(self):
  name='direct_omni_train_smoke_001/jobs/train.json';path=self.base/name;d=json.loads(path.read_text());d['status']='completed';path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_prior_observed_updates_and_failed_reload_scope_preserved(self):
  name='direct_omni_train_smoke_001/campaign.json';path=self.base/name;original=json.loads(path.read_text())
  for change in [{'observed_updates_completed':1},{'training_receipt_complete':True}]:
   d={**original,'observed_training':{**original['observed_training'],**change}};path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
   with self.subTest(change=change),patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
    with self.assertRaisesRegex(RuntimeError,'finalizer failure'):g.verify_previous_owner()
    run.assert_not_called()
 def test_prior_wrong_phase_inventory_rejects_even_when_hash_matches(self):
  name='direct_omni_train_smoke_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['accepted_phases']['train']={};path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'phase/allocation'):g.verify_previous_owner()
   run.assert_not_called()
 def test_prior_failed_source_hash_is_separate_from_successor_pending_source(self):
  name='direct_omni_train_smoke_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['identity']['source_manifest_sha256']='f'*64;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
