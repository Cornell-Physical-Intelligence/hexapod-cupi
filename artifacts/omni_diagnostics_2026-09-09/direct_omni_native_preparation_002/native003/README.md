# Native003: inference-buffer checkpoint finalizer correction

This successor changes only `direct_training.verify_reload`. Actual smoke001 completed two PPO updates, then its finalizer failed when strict loading copied into normalizer `_std` buffers created by RSL's inference-mode rollout update. The failed campaign remains unadmitted; final constant/stop evaluations never ran.

After learning, this finalizer clones only registered actor/critic buffers flagged as inference tensors into ordinary tensors under an explicit non-inference context. Values, dtype, device, parameters and optimizer objects are preserved. Strict checkpoint loading and exact actor/critic/normalizer/Adam/deterministic-output comparison still run. Checkpoint bytes and RNG state remain unchanged. This is a load into the existing runner after buffer normalization, not a fresh-runner-load claim.

Three focused tests call the actual RSL normalizer update without PPO or physics steps: original failure reproduction, exact fixed reload, ordinary-buffer object preservation, and rejection of an altered Adam checkpoint. The complete 16-test suite passes; the earlier two-update CPU learning regression is not repeated. Sources and counts are bound in `SUCCESSOR_PROVENANCE.json`.

Build with the unchanged `build_source.py` against the exact cold589 parent into a fresh source directory. The only generated runtime difference from native002/source598 is `tools/direct_training.py`; `source_origin.json` and the source manifest record that change. Plan, physics, original checkpoint, curriculum, CAPS loss, allocations, stop/constant scoring and admission gates remain byte-identical. Bind the new native/source hashes in a new host; do not resume or reuse smoke001 admission.

```
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tmp/reference_residual_ppo_001/_deps:tmp/direct_omni_recovery_001/native_003 .venv/bin/python3 -m unittest discover -s tmp/direct_omni_recovery_001/native_003 -p 'test_*.py'
```

The following native002 README is preserved as historical design context; its old freeze names and test counts refer to the parent.
