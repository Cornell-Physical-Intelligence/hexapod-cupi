...
def terminal(fields):
 ...
 require(fields.get('InvocationID')==INVOCATION,'Wrong or unavailable invocation identity')
...
def main():
 ...
 unit=dict(line.split('=',1) for line in unittext.splitlines());terminal(unit)
 ...
 print(json.dumps({'verified_unix':time.time(),'unit':unit,'campaign_status':campaign['status'],
    'campaign_sha256':initial_campaign_sha,'PPO_updates_completed':0,'input_maps':inputs,
