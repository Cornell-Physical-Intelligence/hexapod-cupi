...
PARENT = ROOT/'tmp/direct_omni_train_analysis_003/analyze.py'
...
class OptimizerSummaryTests(unittest.TestCase):
    ...
    def test_fifty_updates_sparse_inventory_and_native_contract_agree(self):
        ...
        config_tree=ast.parse((NATIVE/'direct_config.py').read_text()); scope={'math':math}
        exec(compile(ast.Module(body=[n for n in config_tree.body if isinstance(n,ast.FunctionDef) and n.name=='gradient_updates'],type_ignores=[]),'<frozen milestones>','exec'),scope)
        exec(compile(ast.Module(body=functions, type_ignores=[]), '<frozen contract>', 'exec'), scope)
        receipt = fixture(50)
