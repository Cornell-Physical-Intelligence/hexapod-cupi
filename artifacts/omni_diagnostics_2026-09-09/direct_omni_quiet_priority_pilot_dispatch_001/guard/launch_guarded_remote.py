"""External pause060 guard: bounded direct315 quiet-priority pilot after verified smoke."""
...
SMOKE=BASE/'direct_omni_train_smoke_003'
OUTPUT=BASE/'direct_omni_train_pilot_quiet_priority_001'
PAUSE=BASE/'forecast_pause_060'
UNIT='hexapod-direct-omni-train-pilot-quiet-priority-001-20260910.service'
HOST=BASE/'direct_omni_train_host_003/launch_train_spark.py'
SOURCE_SHA256='ef962070bd770d03553eb824fb6caccada7d6a06ac26d2f180bad068f0a9cc62'
SUPERVISOR_SHA256='04942c62de1f557d8f117f648f990b6dac69443d61c546529d05a4e2a7f0e01e'
CONTRACT_SHA256='1ab678beeba6bf74978c210c4e38f13655e25728fe66e3007e3b1db5e19b6b20'
CHECKPOINT_SHA256='1971b782327408f8446b3271cc665dad8c9c515523cf8c87303141dc487d06e8'
HOST_SHA256='d2b5298aa444f49b46611e377e63ad388dbaa11404bfb9c5b5024ed80cb6f672'
HOST_FREEZE_SHA256='df815208f340a9c8c0c2ce8c9fcfad7fd28791f745961ab714f96c0508454efc'
PREVIOUS_UNIT='hexapod-direct-omni-train-smoke-003-20260910.service'
PREVIOUS_INVOCATION='0882531a12724963a64901721d87dc96'
PRIOR_PINS={'direct_omni_train_smoke_003/campaign.json': 'fc6e962d83920180c46445aee7fa5808caa7ea509f2f9455dfd72f304dbfb3b5', 'direct_omni_train_smoke_003/jobs/standing.json': '1f1d8dcae5c851a588a5d45a67c90471856a3e2b8c2950853ed17f19837d20bb', 'direct_omni_train_smoke_003/jobs/train.json': 'ff688f5758b27a1b7f0abdf8656541f769777b125e848a528ae2971c1e27749f', 'direct_omni_train_smoke_003/jobs/final_constant.json': 'c14435b66a2de3f4fee259edd6b3e7b3a91fa2b7720eb9b8c0305278731b7dc5', 'direct_omni_train_smoke_003/jobs/final_stop.json': '3f67b5a8891cdfbc10cd403a31bafa3811cbaca5ad34649dc03d8ffafecd5bd7', 'forecast_pause_059/pause.json': '30425d6c20574d59c6221ab676d4c1f297d05397b8019fc3d39f88e55e781349', 'forecast_pause_059/restored.json': 'd7d78f206093b5ccad2900879dff4eb370f40e9044bb4521e897f0a641cb795c'}
...
def require_final_bindings():
    ...
    if set(PRIOR_PINS)!={'direct_omni_train_smoke_003/campaign.json',
        'direct_omni_train_smoke_003/jobs/standing.json','direct_omni_train_smoke_003/jobs/train.json',
        'direct_omni_train_smoke_003/jobs/final_constant.json','direct_omni_train_smoke_003/jobs/final_stop.json',
        'forecast_pause_059/restored.json','forecast_pause_059/pause.json'}:raise RuntimeError('Exact completed smoke owner/restoration receipts required')
...
def verify_previous_owner():
    ...
    previous_pause=json.loads((BASE/'forecast_pause_059/pause.json').read_text())
    restored=json.loads((BASE/'forecast_pause_059/restored.json').read_text())
...
def validate_train_inputs(train_host):
    ...
    args=SimpleNamespace(source=SOURCE,checkpoint=CHECKPOINT,contract=CONTRACT,supervisor_source=SUPERVISOR_SOURCE,output=OUTPUT,isaaclab=Path('/home/orionh/IsaacLab'),host_freeze_sha256=HOST_FREEZE_SHA256,allocation='pilot',branch='quiet_priority',smoke=SMOKE)
    ...
    expected={'schema':'direct315_quiet_priority_native_v3','allocation':'pilot','branch':'quiet_priority','replicas':1024,'controls_per_update':24,'updates':50,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157,'quiet_temporal_weight':1.0}}
    if identity.get('selection')!=expected or (identity.get('actor_width'),identity.get('critic_width'))!=(315,318):raise RuntimeError('Host selection differs from bounded fifty-update quiet-priority pilot')
    if identity.get('smoke_campaign_sha256')!=PRIOR_PINS['direct_omni_train_smoke_003/campaign.json']:raise RuntimeError('Host admitted different prior smoke campaign')
    return identity
...
def main():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Bounded direct315 quiet-priority pilot: fresh standing, original-policy constant and stop diagnostics,50 updates with1024 replicas and24 controls per update, final constant and stop diagnostics; no automatic continuation or Stage2 performance admission',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-060', '--on-active=70m',
    ...
    try:
        cmd=['systemd-run','--user','--unit='+UNIT,'--property=RuntimeMaxSec=3720',
            ...
            '--allocation','pilot','--branch','quiet_priority','--smoke',str(SMOKE)]
