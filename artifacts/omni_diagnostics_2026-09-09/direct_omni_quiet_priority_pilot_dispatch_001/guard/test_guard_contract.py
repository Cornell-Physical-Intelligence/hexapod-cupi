...
class GuardTests(unittest.TestCase):
 def setUp(self):
  ...
  fixtures={
   'direct_omni_train_smoke_003/campaign.json':dict(status='completed',terminal_inputs_unchanged=True,bounded_campaign_complete=True,allocation='smoke',branch='quiet_priority',PPO_updates_completed=2,planned_phases=['standing','train','final_constant','final_stop'],accepted_phases={k:{} for k in ['standing','train','final_constant','final_stop']},host_freeze_sha256=g.HOST_FREEZE_SHA256,identity={'source_manifest_sha256':g.SOURCE_SHA256}),
   'forecast_pause_059/pause.json':dict(unit=g.PREVIOUS_UNIT,units={'weather.timer':'ActiveState=active','other.timer':'ActiveState=inactive'}),
   'forecast_pause_059/restored.json':dict(timers=['weather.timer'],restored_unix=123)}
  for i,phase in enumerate(('standing','train','final_constant','final_stop')):
   fixtures['direct_omni_train_smoke_003/jobs/'+phase+'.json']=dict(status='completed',phase=phase,cleanup_checked=True,container_name='test-only-'+phase,container_id=str(i+1)*64)
  pins={}
  ...
  for name,value in [('BASE',self.base),('SMOKE',self.base/'direct_omni_train_smoke_003'),('PRIOR_PINS',pins)]:
 ...
 def test_changed_restoration_fails(self):
  (self.base/'forecast_pause_059/restored.json').write_text('{}')
 ...
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_train_smoke_003/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_059/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_full_preflight_identity_and_exact_pilot_selection(self):
  identity=dict(source_manifest_sha256=g.SOURCE_SHA256,checkpoint_sha256=g.CHECKPOINT_SHA256,actor_width=315,critic_width=318,smoke_campaign_sha256=g.PRIOR_PINS['direct_omni_train_smoke_003/campaign.json'],selection={'schema':'direct315_quiet_priority_native_v3','allocation':'pilot','branch':'quiet_priority','replicas':1024,'controls_per_update':24,'updates':50,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157,'quiet_temporal_weight':1.0}})
  ...
  args=host.verify_inputs.call_args.args[0];self.assertEqual(args.supervisor_source,g.SUPERVISOR_SOURCE);self.assertEqual((args.allocation,args.branch,args.smoke),('pilot','quiet_priority',g.SMOKE))
 def test_pending_actual_prior_hash_rejects_even_with_other_bindings_complete(self):
  with patch.object(g,'PRIOR_PINS',{**g.PRIOR_PINS,'direct_omni_train_smoke_003/jobs/train.json':'PENDING_TERMINAL_SMOKE'}):
 ...
 def test_exact_prior_seven_receipts_required(self):
  with patch.object(g,'PRIOR_PINS',{k:v for k,v in g.PRIOR_PINS.items() if k!='direct_omni_train_smoke_003/jobs/final_stop.json'}):
 ...
 def test_failed_train_or_missing_final_diagnostics_reject(self):
  path=self.base/'direct_omni_train_smoke_003/campaign.json';original=json.loads(path.read_text())
  for change in [{'PPO_updates_completed':1},{'allocation':'pilot'},{'branch':'caps'},{'planned_phases':['standing','train']},{'accepted_phases':{'standing':{}}},{'identity':{'source_manifest_sha256':'a'*64}}]:
   path.write_text(json.dumps({**original,**change}));pins={**g.PRIOR_PINS,'direct_omni_train_smoke_003/campaign.json':g.sha(path)}
 ...
 def test_missing_prior_job_cleanup_rejects(self):
  name='direct_omni_train_smoke_003/jobs/final_stop.json';path=self.base/name;d=json.loads(path.read_text());d['cleanup_checked']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_six_phase_pilot_cli_and_bounds(self):
  ...
  self.assertFalse(set(cmd)&{'learn','--decision-receipt','--phase-group'});self.assertEqual(cmd[cmd.index('--allocation')+1],'pilot');self.assertEqual(cmd[cmd.index('--branch')+1],'quiet_priority');self.assertEqual(cmd[cmd.index('--smoke')+1],str(g.SMOKE))
...
class RuntimePendingTests(unittest.TestCase):
 ...
 def test_actual_frozen_host_selects_exact_six_phases(self):
  spec=importlib.util.spec_from_file_location('actual_pilot_host_review',HERE.parent/'direct_omni_train_host_003/launch_train_spark.py')
  host=importlib.util.module_from_spec(spec);spec.loader.exec_module(host)
  self.assertEqual(host.selected_phases(SimpleNamespace(allocation='pilot',branch='quiet_priority',smoke=g.SMOKE)),('standing','initial_constant','initial_stop','train','final_constant','final_stop'))
  with self.assertRaisesRegex(ValueError,'completed smoke'):host.selected_phases(SimpleNamespace(allocation='pilot',branch='quiet_priority',smoke=None))
