"""Reproduce all-environment005→007 standing comparison with unchanged quiet scorer."""
...
def main():
 ...
 for trial,path in [('005',args.baseline_run),('007',args.candidate_run)]:
  ...
  if trial=='007':
   for e,row in enumerate(rows):
    ...
    for name,jm in row['joints'].items():
     ...
     failures.append({'env':e,'joint':name,'failed_bounds':row['failed_bounds'],'reported_joint_RMS_rad_s':float(np.sqrt(np.mean(v*v))),'position_delta_50Hz_RMS_rad_s':float(np.sqrt(np.mean(fd*fd))),'position_range_rad':float(np.ptp(q)),'net_position_delta_rad':float(q[-1]-q[0]),'integrated_reported_joint_velocity_rad':float(v[1:].sum()*.02),'baseline005_reported_joint_RMS_rad_s':quiet['005']['quiet_rows'][e]['joints'][name]['velocity_rms_rad_s']})
 a=results['005']['post_settle'];b=results['007']['post_settle'];comparison={}
 for frame in ['link','com']:
  ...
  for integrator in a[frame]['velocity_integrals']:
   ...
   comparison[frame][integrator]={'baseline005_every_env_m':av.tolist(),'comparison007_every_env_m':bv.tolist(),'007_minus005_every_env_m':(bv-av).tolist(),'005_worst_m':float(av.max()),'007_worst_m':float(bv.max()),'improved_envs':int((bv<av).sum()),'increased_envs':int((bv>av).sum()),'005_count_above_5mm':int((av>.007).sum()),'007_count_above_5mm':int((bv>.007).sum())}
 ...
 print(json.dumps({'quiet_pass005':quiet['005']['pass_count'],'quiet_pass007':quiet['007']['pass_count'],'output':str(args.output)},indent=2))
