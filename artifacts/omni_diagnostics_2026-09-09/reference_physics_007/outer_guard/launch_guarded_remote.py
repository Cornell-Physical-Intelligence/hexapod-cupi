...
SOURCE = BASE / 'reference_physics_source_007'
OUTPUT = BASE / 'reference_physics_007'
PAUSE = BASE / 'forecast_pause_033'
UNIT = 'hexapod-reference-physics-007-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '666571ed6e37a73b857179323573ae20cdef0eee53aa25908108bd4e29465465':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-006-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_032/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-033', '--on-active=25m',
