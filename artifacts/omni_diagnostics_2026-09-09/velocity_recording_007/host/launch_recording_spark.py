...
def command(args,name):
    return ["docker","compose","--env-file","docker/.env.base","-f","docker/docker-compose.yaml",
        ...
        "-e","PYTHONDONTWRITEBYTECODE=1","-e","PYTHONUNBUFFERED=1","-e",
    ...
    try:
        ...
        with (args.output / "logs" / (phase + ".log")).open("w") as log:
            ...
            while process.poll() is None:
                ...
                if time.monotonic() - phase_started > 90:
                    current_log = (args.output / "logs" / (phase + ".log")).read_text(errors="replace")
                    if "APPLAUNCHER READY" not in current_log:
                        raise TimeoutError("Pre-AppReady deadline90s exceeded; preserve startup evidence")
