...
def main():
    ...
    print('RECORDER007 PREFLIGHT START', flush=True)
    parser = parser_base()
    early, _ = parser.parse_known_args()
    # Match the executed training entrypoint: pin the runtime and import
    # AppLauncher before candidate_runner imports Torch. Strict metadata/asset
    # preflight still completes before the simulator constructor is called.
    if not early.preflight_only:
        root = verify_source(early.source_root)
        sys.path.insert(0, str(root / 'tools'))
    ...
    app = env = None
    try:
        print('RECORDER007 APPLAUNCHER START', flush=True)
        app = AppLauncher(args).app
        print('RECORDER007 APPLAUNCHER READY', flush=True)
