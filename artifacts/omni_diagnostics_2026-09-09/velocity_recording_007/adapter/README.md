...
## Recorder007: match validated import ordering and bound startup

Recorder006 traced the repeated stall to native Kit startup (`SimulationApp._start_app`, line576), before any GPU context or policy load. This matches the historical2026-08-26 pre-AppReady incident. No shared cache, volume or unrelated workload was changed. This version pins the runtime and imports AppLauncher before candidate_runner/Torch, matching the actually executed training entrypoint; all strict preflight checks still precede constructing the application. The host uses unbuffered Python output and rejects a missing AppLauncher-ready marker after90seconds. This is a tested diagnostic intervention, not proof of the native root cause. All prior render sources/results remain unchanged.
