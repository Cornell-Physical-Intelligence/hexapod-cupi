...
class GuardTests(unittest.TestCase):
 ...
 def test_exact_recording_previous_two_absences(self):
 ...
 def test_changed_restoration_fails(self):
  (self.base/'forecast_pause_058/restored.json').write_text('{}')
 ...
 def test_missing_prior_receipt_rejects_before_container_checks(self):
  (self.base/'direct_omni_preview_001/jobs/recording.json').unlink()
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_preview_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_058/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_full_preflight_identity_and_exact_smoke_selection(self):
  identity=dict(source_manifest_sha256=g.SOURCE_SHA256,checkpoint_sha256=g.CHECKPOINT_SHA256,actor_width=315,critic_width=318,selection={'schema':'direct315_quiet_priority_native_v3','allocation':'smoke','branch':'quiet_priority','replicas':32,'controls_per_update':24,'updates':2,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157,'quiet_temporal_weight':1.0}})
 ...
 def test_completed_native_recording_not_relabelled_failed(self):
  name='direct_omni_preview_001/jobs/recording.json';path=self.base/name;d=json.loads(path.read_text());d['status']='failed';path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
 ...
 def test_recording_cleanup_is_not_a_ppo_or_quality_admission(self):
  name='direct_omni_preview_001/campaign.json';path=self.base/name;original=json.loads(path.read_text())
  for change in [{'policy_training_started':True},{'stage2_complete':True},{'recording_only':False}]:
 ...
 def test_prior_wrong_phase_inventory_rejects_even_when_hash_matches(self):
  name='direct_omni_preview_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['planned_phases']=['recording','train'];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'scope differs'):g.verify_previous_owner()
   run.assert_not_called()
 def test_prior_failed_source_hash_is_separate_from_successor_pending_source(self):
  name='direct_omni_preview_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['identity']['source_manifest_sha256']='f'*64;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_embedded_restorer_identical_reviewed_parent(self):
  ...
  self.assertEqual(embedded(HERE/'launch_guarded_remote.py'),embedded(HERE.parents[0]/'direct_omni_train_smoke_guard_003/launch_guarded_remote.py'))
