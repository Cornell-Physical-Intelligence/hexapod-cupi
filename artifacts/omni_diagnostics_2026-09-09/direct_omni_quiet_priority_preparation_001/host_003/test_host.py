...
ROOT=HERE.parents[1]
CONTRACT=ROOT/'tmp/direct_omni_recovery_001/native_004'
 ...
 def _campaign(self,fail=None,alter=False,allocation='smoke'):
  ...
  class Contract:
   def validate_result(self,path,phase,i,expected_checkpoint_sha256=None):
    ...
    if phase=='train':result['optimizer_diagnostics']={'schema':'direct315_actor_gradients_v1','updates':2 if allocation=='smoke' else 50,'minibatches':40 if allocation=='smoke' else 1000,'sparse_actor_gradient_rows':2 if allocation=='smoke' else 8}
    return result
  def run(a,phase):
