"""External pause059 guard: quiet-priority smoke003 after exact preview001 cleanup."""
...
OUTPUT=BASE/'direct_omni_train_smoke_003'
PAUSE=BASE/'forecast_pause_059'
UNIT='hexapod-direct-omni-train-smoke-003-20260910.service'
...
PREVIOUS_UNIT='hexapod-direct-omni-preview-001-20260910.service'
PREVIOUS_INVOCATION='7121f177c458460e81de0ef7f4a967f3'
PRIOR_PINS={'direct_omni_preview_001/campaign.json': 'f7352f3deaede92d9e52ba4a9f3a4d00c4ae60f66ef4308a877b49571c073983', 'direct_omni_preview_001/jobs/recording.json': '38c8a9fbea2fcba0f9f004eae3241a29b60e1576c461a19d265e297c74c6f988', 'forecast_pause_058/pause.json': '7265a0e179a65be9d125deaa123500ba4eda9aadec454ea6dfe2b9f111786d28', 'forecast_pause_058/restored.json': 'a2cd701484f2b8977c8e7ee3eec4dbef42e9317e304f7924fe0cf8b29dd46060'}
...
def require_final_bindings():
    ...
    if set(PRIOR_PINS)!={'forecast_pause_058/pause.json', 'direct_omni_preview_001/jobs/recording.json', 'direct_omni_preview_001/campaign.json', 'forecast_pause_058/restored.json'}:raise RuntimeError('Exact preview owner/restoration receipts required')
...
def verify_previous_owner():
    ...
    campaign=json.loads((BASE/'direct_omni_preview_001/campaign.json').read_text())
    if campaign.get('status')!='failed' or campaign.get('terminal_inputs_unchanged') is not True:raise RuntimeError('Prior preview campaign not terminal and immutable')
    if campaign.get('planned_phases')!=['recording'] or campaign.get('recording_only') is not True or campaign.get('policy_training_started') is not False or campaign.get('stage2_complete') is not False:raise RuntimeError('Prior preview scope differs')
    if campaign.get('host_freeze_sha256')!='640ea3891d8e0d6e83f173f5bc5a2206ccdd2d0ef74c9a324d62ecda4fd9594e' or campaign.get('identity',{}).get('source_manifest_sha256')!='64144b2c2466f17c809554ef53f82b736b4199136cc821451d7ac2788752bd2e':raise RuntimeError('Prior preview source/host identity differs')
    previous_pause=json.loads((BASE/'forecast_pause_058/pause.json').read_text())
    restored=json.loads((BASE/'forecast_pause_058/restored.json').read_text())
    ...
    job=json.loads((BASE/'direct_omni_preview_001/jobs/recording.json').read_text())
    if job.get('status')!='completed' or job.get('phase')!='recording' or job.get('exit_code')!=0 or job.get('cleanup_checked') is not True or not job.get('container_id') or not job.get('container_name'):raise RuntimeError('Prior recording lacks exact terminal cleanup proof')
...
def validate_train_inputs(train_host):
    ...
    expected={'schema':'direct315_quiet_priority_native_v3','allocation':'smoke','branch':'quiet_priority','replicas':32,'controls_per_update':24,'updates':2,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157,'quiet_temporal_weight':1.0}}
...
def main():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Bounded direct315 quiet-priority smoke003: fresh standing, two updates with32 replicas, final constant and stop diagnostics; preview001 cleanup is ancestry evidence only, no pilot or automatic continuation',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-059', '--on-active=45m',
