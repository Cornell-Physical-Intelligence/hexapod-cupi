...
class GuardTests(unittest.TestCase):
 ...
 def test_exact_six_jobs_twelve_absences(self):
 ...
 def test_active_or_conflicting_invocation_fails(self):
  for value in [self.unit.replace('ActiveState=inactive','ActiveState=active'),self.unit.replace(g.PREVIOUS_INVOCATION,'a'*32)]:
 ...
 def test_changed_restoration_fails(self):
  (self.base/'forecast_pause_060/restored.json').write_text('{}')
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   ...
   run.assert_not_called()
 def test_missing_prior_receipt_rejects_before_container_checks(self):
  (self.base/'direct_omni_train_pilot_quiet_priority_001/jobs/train.json').unlink()
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaises(FileNotFoundError):g.verify_previous_owner()
   run.assert_not_called()
 def test_semantically_incomplete_prior_campaign_rejects_even_with_matching_hash(self):
  name='direct_omni_train_pilot_quiet_priority_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['terminal_inputs_unchanged']=False;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'not terminal'):g.verify_previous_owner()
   run.assert_not_called()
 def test_exact_timer_restore_required_even_when_receipt_hash_matches(self):
  name='forecast_pause_060/restored.json';path=self.base/name;d=json.loads(path.read_text());d['timers']=[];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
 ...
 def test_full_preflight_identity_and_exact_smoke_selection(self):
  identity=dict(source_manifest_sha256=g.SOURCE_SHA256,checkpoint_sha256=g.CHECKPOINT_SHA256,actor_width=315,critic_width=318,selection={'schema':'direct315_extended_native_v4','allocation':'smoke','branch':'quiet_priority','replicas':32,'controls_per_update':24,'updates':2,'caps':{'temporal_weight':.1,'spatial_weight':.1,'noise_scale':1.,'noise_seed':1157,'quiet_temporal_weight':1.0}})
  ...
  args=host.verify_inputs.call_args.args[0];self.assertEqual(args.supervisor_source,g.SUPERVISOR_SOURCE);self.assertEqual((args.allocation,args.branch,args.smoke),('smoke','quiet_priority',None))
 def test_completed_training_job_not_relabelled_failed(self):
  name='direct_omni_train_pilot_quiet_priority_001/jobs/train.json';path=self.base/name;d=json.loads(path.read_text());d['status']='failed';path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run',side_effect=self.absent):
   with self.assertRaisesRegex(RuntimeError,'terminal cleanup'):g.verify_previous_owner()
 def test_prior_pilot_scope_cannot_change(self):
  name='direct_omni_train_pilot_quiet_priority_001/campaign.json';path=self.base/name;original=json.loads(path.read_text())
  for change in [{'PPO_updates_completed':49},{'Stage2_complete':True},{'automatic_continuation':True},{'allocation':'smoke'},{'branch':'caps'}]:
   d={**original,**change};path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
   with self.subTest(change=change),patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
    with self.assertRaisesRegex(RuntimeError,'scope differs'):g.verify_previous_owner()
    run.assert_not_called()
 def test_prior_wrong_phase_inventory_rejects_even_when_hash_matches(self):
  name='direct_omni_train_pilot_quiet_priority_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['planned_phases']=['standing','train'];path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'scope differs'):g.verify_previous_owner()
   run.assert_not_called()
 def test_prior_source_hash_is_separate_from_successor_pending_source(self):
  name='direct_omni_train_pilot_quiet_priority_001/campaign.json';path=self.base/name;d=json.loads(path.read_text());d['identity']['source_manifest_sha256']='f'*64;path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
  with patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'source/host'):g.verify_previous_owner()
   run.assert_not_called()
 ...
 def test_four_phase_smoke_cli_and_bounds(self):
  ...
  self.assertFalse(set(cmd)&{'pilot','learn','--decision-receipt','--phase-group','--smoke'});self.assertEqual(cmd[cmd.index('--allocation')+1],'smoke');self.assertEqual(cmd[cmd.index('--branch')+1],'quiet_priority')
 ...
 def test_embedded_restorer_identical_reviewed_parent(self):
  ...
  self.assertEqual(embedded(HERE/'launch_guarded_remote.py'),embedded(HERE/'parent_guard_004.py'))

 def test_all_failed_quiet_rows_do_not_block_integration_scope(self):
  campaign=json.loads((self.base/g.PREVIOUS_OUTPUT/'campaign.json').read_text())
  self.assertEqual(campaign['accepted_phases']['final_stop']['quiet_failed_env_ids'],list(range(48)))
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run',side_effect=self.absent):g.verify_previous_owner()
 def test_missing_accepted_phase_and_false_completion_reject(self):
  name=g.PREVIOUS_OUTPUT+'/campaign.json';path=self.base/name;original=json.loads(path.read_text())
  for modification in ['missing','incomplete','wrong_phase']:
   d=json.loads(json.dumps(original))
   if modification=='missing':del d['accepted_phases']['initial_stop']
   elif modification=='incomplete':d['accepted_phases']['final_stop']['complete']=False
   else:d['accepted_phases']['train']['phase']='standing'
   path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
   with self.subTest(modification=modification),patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
    with self.assertRaises(RuntimeError):g.verify_previous_owner()
    run.assert_not_called()
 def test_prior_native_identity_is_required(self):
  (self.base/'direct_omni_train_preparation_003/FREEZE_SHA256.json').write_text('{}')
  with patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
   with self.assertRaisesRegex(RuntimeError,'native/host'):g.verify_previous_owner()
   run.assert_not_called()
 def test_no_six_job_duplicate_or_missing_identifier(self):
  name=g.PREVIOUS_OUTPUT+'/jobs/final_stop.json';path=self.base/name;original=json.loads(path.read_text())
  other=json.loads((self.base/g.PREVIOUS_OUTPUT/'jobs/standing.json').read_text())
  for replacement in [None,other['container_id']]:
   d={**original,'container_id':replacement};path.write_text(json.dumps(d));pins={**g.PRIOR_PINS,name:g.sha(path)}
   with self.subTest(replacement=replacement),patch.object(g,'PRIOR_PINS',pins),patch.object(g,'call',return_value=self.unit),patch.object(g.subprocess,'run') as run:
    with self.assertRaises(RuntimeError):g.verify_previous_owner()
    run.assert_not_called()
 def test_prior_nonzero_exit_or_pid_or_failed_unit_rejects(self):
  for value in [self.unit.replace('MainPID=0','MainPID=77'),self.unit.replace('ExecMainStatus=0','ExecMainStatus=1'),self.unit.replace('ActiveState=inactive','ActiveState=failed')]:
   with self.subTest(value=value),patch.object(g,'call',return_value=value),patch.object(g.subprocess,'run') as run:
    with self.assertRaises(RuntimeError):g.verify_previous_owner()
    run.assert_not_called()
 def test_full_prior_pin_set_required(self):
  with patch.object(g,'SOURCE_SHA256','a'*64),patch.object(g,'CONTRACT_SHA256','b'*64),patch.object(g,'HOST_SHA256','c'*64),patch.object(g,'HOST_FREEZE_SHA256','d'*64):
   g.require_final_bindings()
   with patch.object(g,'PRIOR_PINS',dict(list(g.PRIOR_PINS.items())[:-1])):
    with self.assertRaisesRegex(RuntimeError,'nine'):g.require_final_bindings()
