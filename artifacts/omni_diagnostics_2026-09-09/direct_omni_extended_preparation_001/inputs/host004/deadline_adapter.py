"""Exact source009 run_owned adaptation for explicit native005 extended campaigns.
...
SCHEMA = 'direct315_extended_supervisor_deadlines_v1'
...
HELPER_NAME = '_direct_extended_phase_deadline_seconds'
SEAMS = (
    ('deadline_seconds=600', 'deadline_seconds=_direct_extended_phase_deadline_seconds(args, phase)'),
    ('deadline = time.monotonic() + 600', 'deadline = time.monotonic() + _direct_extended_phase_deadline_seconds(args, phase)'),
    ('raise TimeoutError("Standing phase exceeded ten-minute bound")',
     'raise TimeoutError(f"{phase} phase exceeded {_direct_extended_phase_deadline_seconds(args, phase)}-second bound")'),
)


def phase_deadline_seconds(args, phase):
    if getattr(args, 'allocation', None) != 'extended' or phase not in PHASES:
        raise ValueError('Deadline adapter is restricted to explicit extended phases')
    return 1800 if phase == 'train' else 600
...
def inspect_supervisor(path):
    ...
    return {
        ...
        'phase_deadline_seconds': {phase: 1800 if phase == 'train' else 600 for phase in PHASES},
...
def install(module, path):
    ...
    # Compile only the exact original function definition; no source imports,
    # main(), defaults with side effects or runtime statements are executed.
    code = compile(ast.Module(body=[copy.deepcopy(proof['_original_node'])], type_ignores=[]), str(path), 'exec')
    ...
    if HELPER_NAME in vars(module) or 'DIRECT_DEADLINE_ADAPTER' in vars(module):
    ...
    metadata = {k: v for k, v in proof.items() if k not in ('original_function_source', 'adapted_function_source', '_original_node')}
    # Install only after all source/code checks succeed.
    setattr(module, HELPER_NAME, phase_deadline_seconds)
    module.DIRECT_DEADLINE_ADAPTER = metadata
