...
class CapsPPO(PPO):
    def __init__(self,*args,caps_options,gradient_diagnostics=True,gradient_update_milestones=None,**kwargs):
        ...
        milestones=(1,10,25,50) if gradient_update_milestones is None else tuple(gradient_update_milestones)
        if milestones not in ((1,10,25,50),(1,10,25,50,100,250,500)) or any(type(x) is not int for x in milestones):
            raise ValueError('Only declared sparse diagnostic milestones are supported')
        self.gradient_update_milestones=milestones
    ...
    def _caps_update(self) -> dict[str, float]:
        ...
        for minibatch_index,batch in enumerate(generator,1):
            ...
            sparse=self.gradient_diagnostics and self.current_update in self.gradient_update_milestones and minibatch_index in (1,self.num_learning_epochs*self.num_mini_batches)
