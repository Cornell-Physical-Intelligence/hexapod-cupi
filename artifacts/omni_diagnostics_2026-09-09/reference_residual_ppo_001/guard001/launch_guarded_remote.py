...
OUTPUT = BASE / 'reference_residual_ppo_001'
PAUSE = BASE / 'forecast_pause_042'
UNIT = 'hexapod-reference-residual-ppo-001-20260910.service'
HOST = BASE / 'reference_residual_ppo_launch_001/launch_residual_ppo_spark.py'
RUN = BASE / 'reference_physics_009'
BRIDGE = BASE / 'reference_device_smoke_adapter_001'
CONSUMER = BASE / 'reference_residual_ppo_source_001'
...
def main():
    ...
    if hashlib.sha256(HOST.read_bytes()).hexdigest() != '3201525025e59042f85fdb9cad0ad02dc32b4d272293b5b246899da81f51e61d':
        raise RuntimeError('Wrong reviewed bounded PPO host')
    host_manifest = HOST.parent / 'FREEZE_SHA256.json'
    if hashlib.sha256(host_manifest.read_bytes()).hexdigest() != '54b16b7f75d7b5c6fb65a8040d0535edab7cfab5e8d2e3333e202be0fbfa3985':
    ...
    previous = call(['systemctl','--user','show','hexapod-reference-directional-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_041/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-042', '--on-active=50m',
