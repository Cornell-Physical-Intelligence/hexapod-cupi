...
def source(name):
 if name.startswith('run/'):return base+'reference_residual_ppo_001/'+name[4:]
 if name.startswith('forecast_pause/'):return base+'forecast_pause_042/'+name[len('forecast_pause/'):]
...
def fetch(item):
 ...
 subprocess.run(['scp','-q','-o','BatchMode=yes','spark:'+source(name),str(p)],check=True,timeout=60)
