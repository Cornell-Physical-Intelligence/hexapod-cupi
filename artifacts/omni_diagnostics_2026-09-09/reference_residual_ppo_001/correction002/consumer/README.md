...
## Parser-only successor002

The first actual campaign passed fresh standing, then failed before smoke
AppLauncher or calibration: the early parser treated the late `--device cuda:0`
flag as an abbreviation for `--device-run`, overwriting the proof path. No PPO
updates occurred. This successor disables argparse abbreviation. No physics,
learner, reward, exploration setting, observation, action or gate changes.

`test_late_app_launcher_flags.py` executes the actual full preflight CLI,
including `--device cuda:0`, `--headless`, `--info` and the host's `--kit_args`.
It reproduces the old error and checks both flag orders in the corrected CLI.
All subprocesses exit before importing AppLauncher; these are CPU parser/input
checks. The original001 source and evidence remain immutable. Existing RSL and
physical scoring reports retain their original source/CPU scope.
