"""Fresh standing, bounded two-update PPO and matched checkpoint retention."""
...
PHASES=('standing','smoke','evaluate_initial','evaluate_final')
CONSUMER_FREEZE='e9a1d6d687504fd1323d8addbcb962c3025745a258f71b9f076879778eedaa0b'
CONSUMER=None
PARENT=None
...
def consumer_args(args,phase,require_standing):
    return SimpleNamespace(source_root=args.source,run=args.run,device_run=args.device_run,bridge=args.bridge,
        ...
        standing=args.output/'standing/admission.json',output=args.output/phase,mode=phase,
        smoke_output=args.output/'smoke' if phase.startswith('evaluate_') else None)

def validate_inputs(args,phase='smoke',require_standing=False):
    global CONSUMER
    ...
    if CONSUMER is None:
        spec=importlib.util.spec_from_file_location('_bound_residual_physical_contract',args.consumer/'physical_contract.py')
        CONSUMER=importlib.util.module_from_spec(spec);spec.loader.exec_module(CONSUMER)
    if Path(CONSUMER.__file__).resolve()!=args.consumer/'physical_contract.py':raise ValueError('Wrong imported consumer')
...
def command(args,name,phase):
    ...
    cli+=['-v',str(args.output)+':/outputs:rw']
    if phase.startswith('evaluate_'):cli+=['-v',str(args.output/'smoke')+':/outputs/smoke:ro']
    cli+=['--entrypoint','/workspace/isaaclab/_isaac_sim/python.sh','isaac-lab-base','/consumer/run_residual_ppo.py',
        '--mode',phase,'--source-root','/workspace/hexapod','--run','/qualified','--device-run','/device-proof',
        '--bridge','/bridge','--observation-bundle','/observation','--package','/study',
        '--standing','/outputs/standing/admission.json','--output','/outputs/'+phase]
    if phase.startswith('evaluate_'):cli+=['--smoke-output','/outputs/smoke']
...
def run_owned(args, phase):
    ...
    report = dict(status="starting", phase=phase, container_name=name, started_unix=time.time(),
                  deadline_seconds=600, app_ready_deadline_seconds=90, no_policy_loaded=phase == "standing")
...
def run_phases(args,baseline,report):
    expected=None;smoke_hashes=None
    for phase in PHASES:
        ...
        if smoke_hashes is not None and tree_hashes(args.output/'smoke')!=smoke_hashes:raise ValueError('Immutable smoke evidence changed')
        ...
        if phase=='standing':
            ...
            if accepted.get('passed') is not True or accepted.get('mode')!=phase:raise ValueError('Consumer result validation or phase binding did not pass')
            if phase.startswith('evaluate_'):
                which='initial' if phase=='evaluate_initial' else 'final'
                if state.get('checkpoint_sha256')!=sha(args.output/'smoke'/(which+'.pt')):raise ValueError('Retention loaded the wrong immutable checkpoint')
        if phase=='smoke':smoke_hashes=tree_hashes(args.output/'smoke');save(args.output/'smoke_immutable.sha256.json',smoke_hashes)
        ...
        if phase=='smoke':report['PPO_updates_completed']=2
        save(args.output/'campaign.json',report)
    if tree_hashes(args.output/'smoke')!=smoke_hashes:raise ValueError('Final smoke checkpoint/evidence mutation')
...
def main():
    ...
    report=dict(status='preparing',started_unix=time.time(),runtime_binding=runtime,source_manifest_sha256=SOURCE_MAP,
        ...
        PPO_updates_completed=0,Stage2_complete=False,scope='Bounded integration and matched retention; no automatic long training')
    try:
        save(args.output/'campaign.json',report);run_phases(args,baseline,report)
        report.update(status='completed',two_update_integration_and_matched_retention_passed=True)
    except Exception as error:report.update(status='failed',error=repr(error));raise
    finally:
        try:
            smoke_state=args.output/'smoke/state.json'
            if smoke_state.is_file():
                observed=read(smoke_state)
                report['smoke_reported_PPO_updates_completed']=observed.get('PPO_updates_completed')
                report['smoke_reported_policy_training_started']=observed.get('policy_training_started')
