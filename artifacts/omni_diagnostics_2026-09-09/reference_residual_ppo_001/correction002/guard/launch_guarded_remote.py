"""Dispatch only this fresh bounded residual-PPO integration under the user's existing pause authorization."""
...
OUTPUT = BASE / 'reference_residual_ppo_002'
PAUSE = BASE / 'forecast_pause_043'
UNIT = 'hexapod-reference-residual-ppo-002-20260910.service'
HOST = BASE / 'reference_residual_ppo_launch_002/launch_residual_ppo_spark.py'
RUN = BASE / 'reference_physics_009'
BRIDGE = BASE / 'reference_device_smoke_adapter_001'
CONSUMER = BASE / 'reference_residual_ppo_source_002'
...
def main():
    ...
    if hashlib.sha256(HOST.read_bytes()).hexdigest() != '8cda4adba0852283b2a57379222db32634d06c75f08814387acce1b9d8e5569b':
        raise RuntimeError('Wrong reviewed bounded PPO host')
    host_manifest = HOST.parent / 'FREEZE_SHA256.json'
    if hashlib.sha256(host_manifest.read_bytes()).hexdigest() != '2675f3e918185c279f4b71c049262422e73d71df83f88a1104079a236d4b1fcc':
    ...
    previous = call(['systemctl','--user','show','hexapod-reference-residual-ppo-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_042/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh standing and calibrated two-update PPO with quiet and matched retention; no long training',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-043', '--on-active=50m',
