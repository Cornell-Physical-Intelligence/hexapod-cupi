...
SOURCE = BASE / 'reference_physics_source_004'
OUTPUT = BASE / 'reference_physics_004'
PAUSE = BASE / 'forecast_pause_030'
UNIT = 'hexapod-reference-physics-004-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'a433e529d29d5360c828b406a3dfd769e078d69fc9deaf03f4fe110eb6fa7a63':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-003-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_029/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-030', '--on-active=25m',
