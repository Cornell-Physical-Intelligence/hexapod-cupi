"""Verify or reconstruct exact reference004 from immutable reference003 + delta.
...
TARGET='a433e529d29d5360c828b406a3dfd769e078d69fc9deaf03f4fe110eb6fa7a63'
...
def main():
    ...
    if sha(parent/'campaign_source_hashes.json')!=PARENT:raise ValueError('Wrong immutable source003')
    ...
    if set(overlay)!={'tools/wave_reference.py','tools/physics_substeps.py','tools/run_reference_physics.py'}:
        raise ValueError('Only the reviewed lift and observer delta is permitted')
    ...
    if virtual!=final:raise ValueError('Merged source does not equal frozen source004')
