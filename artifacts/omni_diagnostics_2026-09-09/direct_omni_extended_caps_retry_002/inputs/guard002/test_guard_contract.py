...
class GuardTests(unittest.TestCase):
 def setUp(self):
  ...
  fixtures={
   'direct_omni_train_smoke_004/campaign.json':dict(status='completed',terminal_inputs_unchanged=True,bounded_campaign_complete=True,allocation='smoke',branch='quiet_priority',PPO_updates_completed=2,planned_phases=['standing','train','final_constant','final_stop'],accepted_phases={k:{} for k in ['standing','train','final_constant','final_stop']},host_freeze_sha256=g.SMOKE_HOST_FREEZE_SHA256,identity={'source_manifest_sha256':g.SOURCE_SHA256}),
...
class RuntimePendingTests(unittest.TestCase):
 ...
 def test_actual_frozen_host_selects_exact_six_phases(self):
  spec=importlib.util.spec_from_file_location('actual_pilot_host_review',HERE.parent/'direct_omni_train_host_005/launch_train_spark.py')
