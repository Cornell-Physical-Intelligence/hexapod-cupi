...
class ActualMainOrderTests(unittest.TestCase):
 def execute(self,fail_at=None):
  ...
  with contextlib.ExitStack() as stack:
   ...
   stack.enter_context(patch.object(g,'verify_failed_previous_owner',side_effect=lambda:events.append(('failed_prior',pause.exists()))))
 ...
 def test_actual_order_fallback_before_timer_stop_two_checks_then_dispatch(self):
  pause,events,validations=self.execute();self.assertEqual(validations,[False,True]);self.assertEqual([value for kind,value in events if kind=='failed_prior'],[False,True])
  runs=[cmd for kind,cmd in events if kind=='run']
  fallback=next(i for i,c in enumerate(runs) if '--on-active=95m' in c)
  ...
  self.assertEqual(dispatch[-6:],['--allocation','extended','--branch','caps','--smoke',str(g.SMOKE)])
