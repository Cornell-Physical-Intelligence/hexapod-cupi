"""External pause063 retry guard: exact failed extended001 ownership plus verified smoke004."""
...
OUTPUT=BASE/'direct_omni_train_extended_caps_002'
PAUSE=BASE/'forecast_pause_063'
UNIT='hexapod-direct-omni-train-extended-caps-002-20260910.service'
HOST=BASE/'direct_omni_train_host_005/launch_train_spark.py'
...
HOST_FREEZE_SHA256='2db7e44ec40543753a6d1325f6d5dc9ed81be2948931b2ca670deffda68ce959'
SMOKE_HOST_FREEZE_SHA256='b55282c968d64dca218baf10278339ff2013c23e28dc7cbe9a0d0b8dfd05d51f'
...
FAILED_UNIT='hexapod-direct-omni-train-extended-caps-001-20260910.service'
FAILED_INVOCATION='75ddb0b75a8a4f0d8fc4a34c92e87486'
FAILED_OUTPUT=BASE/'direct_omni_train_extended_caps_001'
FAILED_PINS={'forecast_pause_062/pause.json': '72f6ca055830f27a593bd8d8eb6ad9eb84232149d13fe5f4fa8e07c15af2eec4', 'forecast_pause_062/launch.json': 'e6acd9dcae7bcd802caa6a0c1e9eb4569a67f7cb271d834509abec89203df414', 'forecast_pause_062/restored.json': '64d994bf2e4f832ad10b8f1bc7cafcb113ae8e81fac0d04e55ef750e1c4c2f8c'}
FAILED_AUDIT_SHA256='010bae002276f15bf1e32a1bf5cffff10f36e9032a42224c112eb17842c36cfd'
...
def require_final_bindings():
    for name,value in {'source':SOURCE_SHA256,'supervisor':SUPERVISOR_SHA256,'contract':CONTRACT_SHA256,
        'checkpoint':CHECKPOINT_SHA256,'host':HOST_SHA256,'host freeze':HOST_FREEZE_SHA256,'smoke host':SMOKE_HOST_FREEZE_SHA256,'failed audit':FAILED_AUDIT_SHA256,**PRIOR_PINS,**FAILED_PINS}.items():
        if len(value)!=64 or any(c not in '0123456789abcdef' for c in value):raise RuntimeError('Final reviewed binding pending: '+name)
    if len(FAILED_INVOCATION)!=32 or any(c not in '0123456789abcdef' for c in FAILED_INVOCATION):raise RuntimeError('Failed predecessor invocation not bound')
    if set(FAILED_PINS)!={'forecast_pause_062/pause.json','forecast_pause_062/launch.json','forecast_pause_062/restored.json'}:raise RuntimeError('Exact three failed-owner receipts required')
...
def verify_failed_previous_owner():
    previous=call(['systemctl','--user','show',FAILED_UNIT,'-p','ActiveState','-p','SubState','-p','InvocationID','-p','MainPID','-p','ExecMainStatus','-p','Result'])
    fields=dict(line.split('=',1) for line in previous.splitlines() if '=' in line)
    expected={'ActiveState':'failed','SubState':'failed','InvocationID':FAILED_INVOCATION,'MainPID':'0','ExecMainStatus':'1','Result':'exit-code'}
    if any(fields.get(k)!=v for k,v in expected.items()):raise RuntimeError('Failed predecessor owner/status is unknown, changed or active')
    # Only ENOENT establishes absent output; permission/IO errors and dangling
    # links are not evidence that no previous campaign was created.
    try:FAILED_OUTPUT.lstat()
    except FileNotFoundError:pass
    else:raise RuntimeError('Failed predecessor output appeared; preserve it and review')
    for name,bound in FAILED_PINS.items():
        if sha(BASE/name)!=bound:raise RuntimeError('Failed predecessor receipt changed: '+name)
    pause=json.loads((BASE/'forecast_pause_062/pause.json').read_text())
    launch=json.loads((BASE/'forecast_pause_062/launch.json').read_text())
    restored=json.loads((BASE/'forecast_pause_062/restored.json').read_text())
    if pause.get('unit')!=FAILED_UNIT or pause.get('output')!=str(FAILED_OUTPUT) or pause.get('source')!=str(SOURCE):raise RuntimeError('Failed predecessor pause identity differs')
    command=launch.get('command',[])
    if '--unit='+FAILED_UNIT not in command or '--output' not in command or command[command.index('--output')+1]!=str(FAILED_OUTPUT):raise RuntimeError('Failed predecessor launch identity differs')
    active={n for n,state in pause['units'].items() if n.endswith('.timer') and 'ActiveState=active' in state}
    if set(restored.get('timers',[]))!=active or not isinstance(restored.get('restored_unix'),(int,float)) or restored['restored_unix']<launch['launched_unix'] or launch['launched_unix']<pause['created_unix']:raise RuntimeError('Failed predecessor restoration not proved')
    if restored.get('owned_cleanup_checked')!=[]:raise RuntimeError('Unexpected container history for pre-campaign failure')
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-063', '--on-active=95m',
