...
OUTPUT = BASE / 'reference_physics_009'
PAUSE = BASE / 'forecast_pause_035'
UNIT = 'hexapod-reference-physics-009-20260910.service'
...
def main():
    ...
    previous = call(['systemctl','--user','show','hexapod-reference-physics-008-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_034/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-035', '--on-active=25m',
