...
import numpy as np
import torch
from batch_wave import BatchWave005, MODES, SNAP_BOOL, SNAP_FLOAT
...
def assert_row(test,out,i,scalar,atol=3e-10):
    ...
    for b,s in [('flight_seen','flight_seen'),('flight_count','flight_count'),('contact_count','contact_count'),
                ...
                ('descent_seen','actual_descent_seen'),('order','next_wave_order_index'),('raw_force_free_samples','raw_force_free_samples'),
                ('raw_force_free_runs','raw_force_free_runs'),('unqualified_contact_returns','unqualified_contact_returns'),
                ('last_unqualified_run_samples','last_unqualified_run_samples')]:
...
class WaveBatchTests(unittest.TestCase):
    ...
    def test_actual004_prefix_preserves_rejection_and_not_physical_pass(self):
        data=np.load(HERE/'parent/inputs/actual004_trace.npz');names=tuple(data['joint_names'])
        ...
        batch=BatchWave005(names,1);batch.reset(pack([initial]),torch.tensor([True]),torch.tensor([0]))
        rejected=None
    ...
    def test_reset_named_order_preload_and_inference_mode_lifecycle(self):
        ...
        with torch.inference_mode():
            b=BatchWave005(names,1);out=b.reset(pack([f.snapshot()]),torch.tensor([True]),torch.tensor([0]))
