"""Versioned batched wave005/residual002 observation and reset-safe history.
...
class ObservationBuilder:
    def __init__(self,joint_names,num_envs,*,device='cpu'):
        for module,relative in [('batch_wave','reference/batch_wave.py'),('tensor_kernel','reference/parent/kernel/tensor_kernel.py')]:
            if Path(sys.modules[module].__file__).resolve()!=(HERE/relative).resolve():raise ValueError('Previously imported module belongs to a different source: '+module)
    ...
    def _encode(self,m,ref,c,requested,world_up,fd,fd_valid):
        ...
        if (ref.get('source_identity',{}).get('scalar_controller_sha256')!=CONTRACT['scalar_reference_sha256']
                or ref.get('source_identity',{}).get('binding')!='wave005_7mm_horizontal080_qualified_flight'
                or ref.get('source_identity',{}).get('implementation')!='batched_qualified_contact_reference_prototype_v2'):
        ...
        for key in ('flight_count','contact_count','liftoffs','touchdowns','landing_gap','raw_force_free_samples','raw_force_free_runs','unqualified_contact_returns','last_unqualified_run_samples'):valid &= state[key]>=0
        valid &= ~state['land_active']|state['landing_metadata_valid']
        valid &= (state['raw_force_free_samples']>=state['flight_count'])&(state['raw_force_free_runs']>=state['unqualified_contact_returns'])
        valid &= state['raw_force_free_samples']>=state['last_unqualified_run_samples']
        valid &= state['unqualified_return_valid']==(state['unqualified_contact_returns']>0)
        valid &= ~state['unqualified_return_valid']|(state['unqualified_lift']>=0)
        valid &= (state['mode']!=8)|(state['sw_active']&~state['flight_seen'])
        ...
        for key,tail in FLOAT_FIELDS.items():
            ...
            elif key in ('stop_time','quiet_time','landing_trigger','unqualified_return_time'):
                presence={'stop_time':'stop_valid','quiet_time':'quiet_valid','landing_trigger':'landing_metadata_valid','unqualified_return_time':'unqualified_return_valid'}[key]
                age=state['time']-value;valid &= ~state[presence]|(age>=-1e-7)
                value=torch.where(state[presence],age.clamp_min(0)/10.,0.)
            elif key=='unqualified_lift':value=torch.where(state['unqualified_return_valid'],value*100.,0.)
