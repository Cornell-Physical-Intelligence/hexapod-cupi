...
sys.path.insert(0,str(HERE/'source_diagonal_pair_001/tools'))
...
class CompletePairWiringTests(unittest.TestCase):
 def run_case(self,pair,names=None):
  f=Fixture(names=names);f.time=4.;g=PairLoadTransfer(f.names,pair_leg_names=pair);start=f.snapshot();g.reset(start)
  ...
  scored=score_transfer(data,refs,substeps=subdata,pair_leg_names=pair)
  ...
  self.assertEqual(refs[-1]['state']['pair_leg_names'],list(pair))
  self.assertIsNone(scored['force_review']['nominal_static_four_corner_reference_nm'])
  self.assertTrue(all(ref['state']['required_support_leg_names']==refs[0]['state']['required_support_leg_names'] for ref in refs))
  for k in ('q_ref','v_ref','a_ref'):self.assertTrue(all(np.isfinite(ref[k]).all() for ref in refs))
  self.assertLessEqual(max(abs(ref['v_ref']).max() for ref in refs),1.75)
  self.assertLessEqual(max(abs(ref['a_ref']).max() for ref in refs),6.)
  with self.assertRaisesRegex(ValueError,'partition'):score_transfer(data,refs,substeps=subdata,pair_leg_names=('lm','rm'))
 def test_lf_rr_complete_unchanged_schedule_core_and_400Hz_wiring(self):self.run_case(('lf','rr'))
 def test_lr_rf_complete_unchanged_schedule_core_and_400Hz_wiring(self):self.run_case(('lr','rf'))
 def test_reversed_runtime_order_selects_named_diagonal(self):self.run_case(('lf','rr'),tuple(reversed(Fixture().names)))
