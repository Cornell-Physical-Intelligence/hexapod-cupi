...
from load_transfer import partition_for_names
...
def score_transfer(data, references, *, substeps=None, dt=.02, pair_leg_names=('lm','rm')):
    ...
    """
    PAIR, CORNERS = partition_for_names(pair_leg_names)
    ...
    expected_pair=list(pair_leg_names);expected_supports=[name for i,name in enumerate(('lf','lm','lr','rf','rm','rr')) if i in CORNERS]
    for reference in references:
        if reference['state'].get('pair_leg_names')!=expected_pair or reference['state'].get('required_support_leg_names')!=expected_supports:
            raise ValueError('Reference named pair/support partition differs from scored case')
    ...
    if stable.any():
        ...
        force_review=dict(mean_reaction_each_foot_world_n=forces.mean(0).tolist(),
            ...
            nominal_static_four_corner_reference_nm=({'vertical_only':1.12866,'ideal_mu_0p6':.621831} if PAIR==(1,4) else None),
            static_comparison_caveat=('Nominal-pose ideal LP, not a force-control target or prediction at the measured deflected pose' if PAIR==(1,4) else 'No diagonal static LP prediction is supplied; recorded forces are measured evidence'),
