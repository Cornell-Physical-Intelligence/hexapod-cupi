"""Explicit named opposing-pair static-load diagnostic (middle or either diagonal). No physics state writes.
...
def partition_for_names(pair_leg_names):
    """Only the three disjoint opposing pairs; named order is explicit and stable."""
    names=tuple(pair_leg_names)
    if names not in (('lm','rm'),('lf','rr'),('lr','rf')):
        raise ValueError('Exact named opposing pair required')
    pair=tuple(LEGS.index(name) for name in names)
    return pair,tuple(i for i in range(6) if i not in pair)
...
class PairLoadTransfer:
    ...
    """
    def __init__(self, joint_names_runtime, config=TransferConfig(), *, pair_leg_names=('lm','rm')):
        self.pair, self.supports = partition_for_names(pair_leg_names)
    ...
    def reset(self, snapshot):
        ...
        self.flight_baseline_z = self.measured_toes0[list(self.pair), 2].copy()
        ...
        local[list(self.pair)] += self.R0.T @ np.array([0., 0., self.cfg.lift_m])
        ...
        for i in self.supports: qend[i] = self.base._leg(self.q0)[i]
    ...
    def _measure(self, snapshot, m):
        ...
        if not m['contact'][list(self.supports)].all():
        ...
        margin = self.base.helper.support_margin(m['contact_point_world_m'],self.supports,com)
        ...
        drift = np.linalg.norm(m['reference_point_world_m'][list(self.supports)]-self.measured_toes0[list(self.supports)],axis=-1).max()
        ...
        if slip[list(self.supports)].max() > self.cfg.maximum_corner_slip_mps:
        ...
        self.diagnostics = dict(measured_support_margin_m=float(margin),body_displacement_m=float(displacement),
            ...
            max_applied_torque_nm=float(abs(applied).max()),corner_max_slip_mps=float(slip[list(self.supports)].max()))
        return normal
    ...
    def step(self, snapshot, *, stop=False, dt=.02):
        ...
        try:
            ...
            for j,leg in enumerate(self.pair):
            ...
            both_unloaded = (self.flight_seen.all() and not m['contact'][list(self.pair)].any()
                and (np.linalg.norm(normal[list(self.pair)],axis=-1) <= self.cfg.maximum_unloaded_normal_force_n).all()
    ...
    def _output(self,q,v,a):
        state=dict(mode=self.mode,proposal_only=True,config=asdict(self.cfg),
            pair_leg_names=[LEGS[i] for i in self.pair],required_support_leg_names=[LEGS[i] for i in self.supports],
