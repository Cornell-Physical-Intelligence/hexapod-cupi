"""Dispatch only this fresh standing and two bounded diagonal-pair cases under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_diagonal_pair_source_001'
OUTPUT = BASE / 'reference_diagonal_pair_001'
PAUSE = BASE / 'forecast_pause_047'
UNIT = 'hexapod-diagonal-pairs-001-20260910.service'
...
def main():
    ...
    if hashlib.sha256((SOURCE/'tools/launch_pair_physics_spark.py').read_bytes()).hexdigest() != '9e875a7978dfd39dbb7733079fefa0125c9b5b8b59f817a33bf7456b25adcc7c':
        raise RuntimeError('Wrong reviewed diagonal-pair host')
    sys.path.insert(0, str(SOURCE / 'tools'))
    from launch_pair_physics_spark import check_source
    from launch_length_study_spark import preflight
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '26ff2b7f8e57b3a6ea69dae09be58b26c2b0967e92103ed3f2673c0f32ae9625':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-directional-003-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_046/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then two cold diagonal-pair static-transfer cases; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-047', '--on-active=40m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1920',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_pair_physics_spark.py'),
