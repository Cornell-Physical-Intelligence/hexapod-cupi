...
hroot=base/'reference_moving_ppo_launch_001';groot=base/'reference_moving_ppo_guard_001'
h=load('bound_host',hroot/'launch_moving_ppo_spark.py');g=load('bound_guard',groot/'launch_guarded_remote.py')
h.verify_tree(hroot,'FREEZE_SHA256.json','f8cb69359f9a9bbab1b812c603bce25c2de42e8692d44c5552a1374bd6279f49')
h.verify_tree(groot,'FREEZE_SHA256.json','7441201d0ff3833049ed1a4047cf0bbc237139a80c62ee1810c2ce4e9096f57a')
args=SimpleNamespace(source=g.SOURCE,run=g.RUN,device_run=g.DEVICE_RUN,bridge=g.BRIDGE,consumer=g.CONSUMER,observation=g.OBSERVATION,output=g.OUTPUT)
...
unit=subprocess.check_output(['systemctl','--user','show','hexapod-rr-preload-002-20260910.service','-p','ActiveState','-p','SubState','-p','Result','-p','InvocationID'],text=True,timeout=20)
...
assert state.get('InvocationID') in ('','7f32b909f3b5411a8f4a57d0200519ef')
...
for phase in ('standing','left_strafe'):
 j=json.loads((base/'reference_rr_preload_002/jobs'/(phase+'.json')).read_text())
