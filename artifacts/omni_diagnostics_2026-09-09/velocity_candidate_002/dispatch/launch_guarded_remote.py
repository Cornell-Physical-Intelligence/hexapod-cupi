...
SOURCE = BASE / 'omni_velocity_source_002'
OUTPUT = BASE / 'omni_velocity_probe_002'
PAUSE = BASE / 'forecast_pause_019'
UNIT = 'hexapod-omni-velocity-probe-002-20260910.service'
...
def main():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh candidate standing and exploration calibration, then at most two stand-only PPO schema updates; no walking training',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-019', '--on-active=30m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_velocity_probe_spark.py'),
