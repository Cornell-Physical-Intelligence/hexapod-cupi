#!/usr/bin/env python3
"""Isolated new C-study target-velocity lineage; guarded standing and scratch PPO.

Derived asset setup from train_length_study.py; existing tasks are untouched.
No old checkpoint resume; evaluate requires exact new lineage/source identity.
"""
...
parser.add_argument("--runner-smoke", type=Path)
parser.add_argument("--iterations", type=int)
AppLauncher.add_app_launcher_args(parser)
args = parser.parse_args()
from candidate_runner import preflight_candidate
preflight_identity, candidate_config = preflight_candidate(args)
# Fail wrong checkpoint/plan/calibration before starting the simulator.
faulthandler.enable()
...
def validate(env, plan, state):
    ...
    for step in range(steps):
        ...
        if not all(bool(torch.isfinite(value).all()) for value in obs.values()) or not bool(torch.isfinite(reward).all()):
...
def main():
    ...
    env=VelocityCandidateEnv(cfg=cfg,evaluation=args.mode!="train")
    ...
    if omni:
        state.update(architecture=omni["architecture"], observation_actor=495, observation_critic=498,
                     candidate_contract=env.candidate_contract, source_sha256=preflight_identity["source_sha256"], stage2_complete=False)
    save(args.output/"state.json",state)
    if args.mode=="validate":
        gate = validate(env,plan,state)
        state.update(status="completed" if gate["passed"] else "rejected",gate=gate)
    else:
        from candidate_runner import (INITIAL_STD, LEARNING_RATE, initialize_scratch, save_candidate,
                                      load_candidate, calibrate_exploration, run_schema_smoke)
        runner_cfg=HexapodPhase1V5PPORunnerCfg()
        runner_cfg.seed=plan["training_seed"];runner_cfg.device=env.device
        runner_cfg.max_iterations=2 if args.mode=="probe" else (args.iterations or plan["training_iterations"])
        runner_cfg.save_interval=25;runner_cfg.logger="tensorboard"
        ...
        if runner_cfg.clip_actions is not None:raise RuntimeError("Raw candidate actions require unclipped wrapper")
        ...
        if args.mode in ("probe","train"):
            initialization=initialize_scratch(runner,candidate_config)
            save(args.output/"initialization.json",initialization)
        if args.mode=="probe":
            calibration=calibrate_exploration(env,runner,args.output,preflight_identity)
            if calibration["passed"]:
                smoke=run_schema_smoke(env,runner,args.output,candidate_config,preflight_identity)
            else:
                smoke={"complete":False,"passed":False,"stage2_complete":False,"reason":"standing exploration calibration rejected"}
                save(args.output/"runner_smoke.json",smoke)
            save(args.output/"evaluation.json",{"complete":True,"kind":"new_action_admission_probe",
                 "passed":calibration["passed"] and smoke["passed"],"stage2_complete":False})
            state.update(status="completed" if calibration["passed"] and smoke["passed"] else "rejected")
        elif args.mode=="train":
            # This is a new scratch lineage. The stand-only smoke weights are not transferred.
            save_candidate(runner,args.output/"policy"/"initial.pt",candidate_config,preflight_identity)
            with torch.inference_mode():env.reset(seed=plan["training_seed"]);env.episode_length_buf.zero_()
            ...
            saved=save_candidate(runner,checkpoint,candidate_config,preflight_identity)
            state.update(status="completed",iterations=runner_cfg.max_iterations,checkpoint=str(checkpoint),
                         checkpoint_sha256=saved["checkpoint_sha256"],
                         learned_std=runner.alg.actor.distribution.std_param.detach().cpu().tolist())
        else:
            loaded=load_candidate(runner,args.checkpoint,candidate_config,preflight_identity)
            from candidate_diagnostics import evaluate_candidate_diagnostics
            from omni_quiet_review import evaluate_quiet_review
            evaluate_candidate_diagnostics(env,runner,plan,args.output,loaded["checkpoint_sha256"])
            quiet_output=args.output/"quiet_review";quiet_output.mkdir()
            quiet=evaluate_quiet_review(env,runner,plan,quiet_output,loaded["checkpoint_sha256"])
            save(args.output/"evaluation.json",{"complete":True,"kind":"candidate_diagnostics_and_quiet_review",
                 "stage2_complete":False,"checkpoint_sha256":loaded["checkpoint_sha256"],
                 "quiet_review_all_pass":quiet["all_scenarios_pass"],"controller":env.candidate_contract})
            state.update(status="completed",checkpoint_sha256=loaded["checkpoint_sha256"])
