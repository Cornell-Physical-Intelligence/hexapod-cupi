...
spec = importlib.util.spec_from_file_location("terrain_robot_launcher", HERE / "launch_velocity_probe_spark.py")
launcher = importlib.util.module_from_spec(spec)
spec.loader.exec_module(launcher)


class LauncherTests(unittest.TestCase):
    def test_only_flat_asset_copy_is_writable_and_probe_is_not_training(self):
        source, output = Path("/frozen"), Path("/run")
        flat = launcher.command(source, output, "owned", "flat")
        probe = launcher.command(source, output, "owned", "probe")
        self.assertIn("/frozen:/workspace/hexapod:ro", flat)
        self.assertIn("/run/inputs/study:/study:rw", flat)
        self.assertIn("/run/inputs/study:/study:ro", probe)
        self.assertEqual(flat[flat.index("--mode") + 1], "validate")
        self.assertEqual(probe[probe.index("--mode") + 1], "probe")
        self.assertIn("/outputs/flat/admission.json", probe)
        self.assertNotIn("--checkpoint", flat + probe)
        for forbidden in ("train", "evaluate", "terrain"):
            with self.assertRaises(ValueError):
                launcher.command(source, output, "owned", forbidden)
