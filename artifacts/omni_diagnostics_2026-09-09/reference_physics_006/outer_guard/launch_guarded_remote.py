...
SOURCE = BASE / 'reference_physics_source_006'
OUTPUT = BASE / 'reference_physics_006'
PAUSE = BASE / 'forecast_pause_032'
UNIT = 'hexapod-reference-physics-006-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '90d7cf551ccf52f7cc2d224b81547c1bad0ad6191c2db56389bb2d54de0ff193':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-005-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_031/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Isolated full-C standing TGS velocity-iteration comparison; no wave, PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-032', '--on-active=25m',
