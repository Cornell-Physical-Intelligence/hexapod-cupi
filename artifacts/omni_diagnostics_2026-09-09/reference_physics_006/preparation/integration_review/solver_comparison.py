...
PROTOCOL={
    'name':'reference006_standing_TGS_velocity_iterations_one',
...
def validate_iteration_readback(rows,num_envs):
    ...
    for row in bodies:
        if (type(row['position']) is not int or type(row['velocity']) is not int
                or not 0<=row['position']<=16 or not 0<=row['velocity']<=1):
            raise RuntimeError('Unknown or overriding body iterations: '+repr(row))
