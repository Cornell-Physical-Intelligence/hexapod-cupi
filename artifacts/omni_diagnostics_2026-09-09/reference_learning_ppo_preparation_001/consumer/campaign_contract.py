...
def verify_inputs(args,*,require_standing=True):
    ...
    return dict(schema='c_reference_recoverable_moving_training_consumer_v3',consumer_freeze_sha256=own,
...
def validate_learning_integrity(output,state):
    """Stdlib review of a new infrastructure phase, never a physical admission."""
    import math
    output=Path(output);record=read(output/'learning_integrity.json');ledger=read(output/'raw/learning_ledger/summary.json')
    if (record.get('schema')!='moving_learning_recovery_admission_v1' or record.get('identity')!=state.get('identity')
        or record.get('infrastructure_passed') is not True or record.get('physical_admission') is not False
        or record.get('automatic_training_allowed') is not False or record.get('ledger')!=ledger
        or record.get('ledger_sha256')!=sha(output/'raw/learning_ledger/summary.json')):
        raise ValueError('Learning infrastructure receipt differs or claims physical admission')
    if state.get('learning_integrity_sha256')!=sha(output/'learning_integrity.json'):
        raise ValueError('State does not bind final learning evidence')
    if (ledger.get('controls')!=712 or ledger.get('replicas')!=32 or ledger.get('completed_finite_recovery_rows',0)<1
        or ledger.get('finite_reset_events',0)<1 or ledger.get('bootstrap_event_records')!=ledger.get('finite_reset_events')
        or ledger.get('all_records_verified') is not True or ledger.get('session_failure') is not None
        or ledger.get('error') is not None or state.get('learning_recovery_passed') is not True):
        raise ValueError('Actual finite-event recovery proof incomplete')
    files=ledger.get('files_sha256',{})
    actual={str(p.relative_to(output/'raw/learning_ledger')):sha(p) for p in (output/'raw/learning_ledger').rglob('*') if p.is_file() and p.name!='summary.json'}
    if not files or files!=actual:raise ValueError('Actual compact learning ledger changed or incomplete')
    timing=state.get('recovery_control_seconds',[]);wall=state.get('recovery_wall_s')
    if (state.get('recovery_controls')!=512 or len(timing)!=512 or any(isinstance(v,bool) or not isinstance(v,(int,float)) or not math.isfinite(v) or v<=0 for v in timing)
        or isinstance(wall,bool) or not isinstance(wall,(int,float)) or not math.isfinite(wall) or wall+1e-6<sum(timing)):
        raise ValueError('Incomplete actual recovery timing')
    per=state.get('recovery_per_environment',[])
    if len(per)!=32 or any(row.get('row')!=i or row.get('active_controls',-1)<0 or row.get('active_controls',513)>512
        or row.get('excluded_recovery_controls')!=512-row.get('active_controls',0) for i,row in enumerate(per)):
        raise ValueError('Incomplete per-row learning evidence')
    for row in per:
        for key in ('active_controls','excluded_recovery_controls','finite_terminal_transitions','time_limit_transitions','moving_intervals_without_reset_crossing'):
            if type(row.get(key)) is not int or not 0<=row[key]<=512:raise ValueError('Malformed actual per-row count')
        if row['time_limit_transitions'] or row['finite_terminal_transitions']>row['active_controls'] or row['moving_intervals_without_reset_crossing']>row['active_controls']:
            raise ValueError('Impossible recovery-phase episode count')
        for key in ('measured_forward_increment_sum_m','raw_SDK_mean_forward_mps'):
            value=row.get(key)
            if isinstance(value,bool) or not isinstance(value,(float,int)) or not math.isfinite(value):raise ValueError('Nonfinite actual progress evidence')
    if ledger.get('per_environment_episodes')!=[row['finite_terminal_transitions'] for row in per]:
        raise ValueError('Actual episode and event counts disagree')
    if state.get('active_transitions')!=sum(row['active_controls'] for row in per):raise ValueError('Per-row learning counts disagree')
    return record

def validate_learning_decision(args,identity):
    """Shared host/entrypoint guard: only root review can allocate ten updates."""
    receipt_path=getattr(args,'decision_receipt',None)
    if receipt_path is None:raise ValueError('No automatic allocation after recovery admission')
    receipt_path=Path(receipt_path).resolve();campaign=Path(args.output).resolve().parent
    if receipt_path==campaign or campaign in receipt_path.parents:raise ValueError('Review receipt must be outside campaign outputs')
    record=read(receipt_path)
    if (record.get('schema')!='moving_PPO_learning_admission_review_v1' or record.get('identity')!=identity
        or record.get('accepted') is not True or record.get('approved_updates')!=10):
        raise ValueError('Explicit same-source ten-update review required')
    phases=('standing','calibration','learning_recovery_32')
    expected={name:sha(campaign/name/'state.json') for name in phases}
    if record.get('phase_state_sha256')!=expected:raise ValueError('Learning review binds different phase states')
    verify_standing(campaign/'standing/admission.json',campaign)
    for name in phases[1:]:validate_result(campaign/name,identity)
    evidence=sha(campaign/'learning_recovery_32/learning_integrity.json')
    if record.get('learning_integrity_sha256')!=evidence:raise ValueError('Learning review lacks exact actual evidence')
    fields=('event_and_recovery_review','per_environment_progress_review','timing_and_memory_review','raw_evidence_integrity_review','no_gate_relaxation')
    if any(record.get(key) is not True for key in fields):raise ValueError('Per-row event/progress/timing/integrity review remains incomplete')
    return {'decision_sha256':sha(receipt_path),'identity':identity,'approved_updates':10,
            'phase_state_sha256':expected,'learning_integrity_sha256':evidence}
...
def validate_result(output,identity):
    ...
    if (policy.get('lineage')!='c_wave005_recoverable_moving_residual_PPO_v2' or policy.get('scope')!='admitted_bounded_moving_PPO'
    ...
    raw_mode=mode.startswith(('profile','train')) or mode=='learning_recovery_32'
    if mode=='calibrate':controls=2200
    elif mode.startswith('profile') or mode=='learning_recovery_32':controls=712
    ...
    if mode=='learning_recovery_32':
        validate_learning_integrity(output,state)
    ...
    if mode=='train_10':
        decision=state.get('learning_decision',{})
        if decision.get('approved_updates')!=10 or decision.get('identity')!=identity or not decision.get('decision_sha256'):
            raise ValueError('No explicit source-bound learning allocation review')
    if mode.startswith('train'):
        ledger=read(output/'raw/learning_ledger/summary.json')
        if ledger.get('all_records_verified') is not True or ledger.get('session_failure') is not None:
            raise ValueError('Training reset/bootstrap ledger incomplete')
        ...
        for report in updates:
            if (report.get('controls_per_replica')!=256 or report.get('valid_learning_transitions',0)<8
                or report.get('normalization_rows_this_update')!=report.get('valid_learning_transitions')):
