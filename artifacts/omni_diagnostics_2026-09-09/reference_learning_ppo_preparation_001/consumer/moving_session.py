...
from learning_integrity import LearningIntegrity
...
class MovingSession:
    def __init__(self, env, layout, points, stance, *, warp_to_torch, output):
        ...
        self.exported=False
        self.integrity=LearningIntegrity(self)
    ...
    def _reset_rows(self, ended, terminated, truncated):
        # Match installed DirectRLEnv reset_env_ids.nonzero(...).int() exactly.
        ids=torch.nonzero(ended).flatten().to(dtype=torch.int32)
        ...
        integrity_record=self.integrity.before_reset(ids,self.terminal_packet)
        ...
        self.integrity.after_reset(integrity_record)
    ...
    def _advance(self,actions,*,initializing=False):
        ...
        if completed.any():
            ...
            recovery_evidence=self.integrity.before_history_reset(completed)
            self.encoder.reset(completed,self.episodes,m['time_s'])
            self.integrity.after_history_reset(recovery_evidence)
        ...
        if completed.any():self.integrity.recovered(recovery_evidence,final)
    ...
    def export(self):
        ...
        self.integrity.export(self.output/'learning_ledger')
