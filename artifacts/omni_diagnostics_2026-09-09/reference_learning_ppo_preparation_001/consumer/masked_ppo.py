...
class MaskedPPO(PPO):
    ...
    def process_env_step(self, obs, rewards, dones, extras):
        ...
        next_value = self._final_values(final,need)
        i = st.step
        ...
        if self.evidence_callback is not None:
            self.evidence_callback(data,next_value,obs,source='actual_masked_PPO_storage',normalization_rows=self.normalizer_rows)

    def _final_values(self,final,need):
        result=torch.zeros(self.storage.num_envs,device=self.storage.learnable.device)
        if need.any():result[need]=self.critic(final[need]).detach().squeeze(-1)
        if not torch.isfinite(result).all():raise RuntimeError('Nonfinite final critic value')
        return result

    def audit_unoptimized_transition(self,obs,extras):
        # Infrastructure-only probe: same final-value computation, no storage,
        # normalization, policy act, optimizer or learning-iteration update.
        if self.storage.step or self._pending_mask is not None or self.optimizer.state:
            raise RuntimeError('Unoptimized recovery audit cannot reuse learning state')
        data=extras['masked_transition'];need=data['learnable']&~data['terminated']
        if (need&~data['final_observation_valid']).any() or (need&(data['final_episode_id']!=data['episode_id'])).any():
            raise ValueError('Probe final critic packet invalid or from another episode')
        with torch.inference_mode():value=self._final_values(data['final_observation'],need)
        if self.evidence_callback is None:raise RuntimeError('Recovery admission requires actual evidence callback')
        self.evidence_callback(data,value,obs,source='unoptimized_recovery_probe_no_PPO_storage',normalization_rows=self.normalizer_rows)
        return value
