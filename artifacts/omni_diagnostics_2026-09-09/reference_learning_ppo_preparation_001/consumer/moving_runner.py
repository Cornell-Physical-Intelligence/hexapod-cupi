"""New masked moving RSL lineage, with immutable source-bound checkpoints.
...
or an explicitly admitted bounded moving pilot. No old checkpoint is accepted.
"""
...
LINEAGE='c_wave005_recoverable_moving_residual_PPO_v2'
...
SCOPES={'synthetic_masked_CPU_integration','admitted_bounded_moving_PPO'}
...
def contract(scope,bindings):
    ...
    return {'lineage':LINEAGE,'scope':scope,'bindings':dict(bindings),'plan_sha256':digest(HERE/'plan.json'),
        ...
        'maximum_updates':25,'old_checkpoint_transfer_allowed':False,'recovery_controls_excluded_from_learning':True,'timeout_bootstrap':'final_pre_reset_critic_same_episode','prototype_bundle_training_flags_unchanged':True,
        'native_velocity_fidelity_qualified':False,'stage2_complete':False}

def runner_config(device='cpu'):
    p=plan()
    return {'seed':157,'device':device,'num_steps_per_env':256,'max_iterations':25,'empirical_normalization':{},
        ...
        'algorithm':{'class_name':'masked_ppo:MaskedPPO','num_learning_epochs':5,'num_mini_batches':4,'learning_rate':p['learning_rate'],
...
def reload_checkpoint(runner,path,identity):
    """Strict same-source/scope readback; continuation remains bounded by the host decision receipt."""
    ...
    return {'checkpoint_sha256':sidecar['checkpoint_sha256'],'actor_critic_normalizer_and_optimizer_exact':True,
        'inference_buffers_made_writable':repaired,'automatic_continuation_allowed':False}
