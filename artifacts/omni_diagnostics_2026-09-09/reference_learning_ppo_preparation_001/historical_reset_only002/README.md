# Minimal reset-context successor002

This unlaunched successor changes only the inference context around the deferred
original SDK reset. Consumer001 remains immutable. Its actual profile reached
224 controls and rejected: env6 lost the required RR support during the first LF
swing, then reset_buf mutation raised before physical reset. The installed SDK
creates reset_buf inside inference mode; the new context matches its ordinary
automatic-reset execution. The same existing no-event profile would still reject
a real row outcome. This fix grants no training admission or automatic retry.

Two new regressions create the exact inference-mode SDK buffer: the immutable
old reset method reproduces the exception, while the successor completes the
selected reset and 200-control recovery with other histories/clocks intact.
The old method source is byte-bound in inputs/moving_session001.py.txt. All
inherited source, command envelope, reward, policy, profile and evaluation gates
are unchanged. The following parent description remains applicable except its
consumer path; use tmp/reference_moving_ppo_002 when running this successor.
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tmp/reference_residual_ppo_001/_deps \
  .venv/bin/python -m unittest discover -s tmp/reference_moving_ppo_002 -p 'test_*.py'
```
