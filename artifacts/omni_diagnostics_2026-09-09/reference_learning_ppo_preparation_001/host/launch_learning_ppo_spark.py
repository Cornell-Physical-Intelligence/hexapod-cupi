"""Separate actual learning recovery admission from a reviewed ten-update continuation."""
...
PHASES=('standing','calibration','learning_recovery_32','train_10','evaluate_initial','evaluate_010','quiet_010')
MODE={'calibration':'calibrate'}
PHASE_SECONDS={p:(1800 if p=='train_10' else 600) for p in PHASES}
CONSUMER_FREEZE='fd9bef87dda976f9b229e7541408d24e674245ca2c51231dfce9075b81d38334'
CONSUMER=None
PARENT=None
ADMISSION_PHASES=PHASES[:3]
LEARNING_PHASES=PHASES[3:]

def report_path(args):return args.output/('campaign.json' if args.phase_group=='admission' else 'learning_campaign.json')
def selected_phases(args):return ADMISSION_PHASES if args.phase_group=='admission' else LEARNING_PHASES
def verify_prior_files(args):
    if getattr(args,'decision_sha256',None) is not None and sha(args.decision_receipt)!=args.decision_sha256:
        raise ValueError('Decision receipt changed before the next allocation')
    for name,bound in getattr(args,'prior_file_hashes',{}).items():
        if sha(args.output/name)!=bound:raise ValueError('Prior admission evidence changed: '+name)
...
def consumer_args(args,phase,require_standing):
    return SimpleNamespace(source_root=args.source,run=args.run,device_run=args.device_run,bridge=args.bridge,
        ...
        standing=args.output/'standing/admission.json',output=args.output/phase,mode=MODE.get(phase,phase),decision_receipt=getattr(args,'decision_receipt',None))
...
def command(args,name,phase):
    if phase not in selected_phases(args):raise ValueError('Only declared phases in the selected allocation exist')
    ...
    if phase=='train_10':cli+=['-v',str(args.decision_receipt)+':/review/decision.json:ro']
    ...
    if args.phase_group=='learn':
        for name in sorted(getattr(args,'prior_file_hashes',{})):
            parts=Path(name).parts
            if len(parts)==1 or parts[0] in ('jobs','logs'):
                cli+=['-v',str(args.output/name)+':/outputs/'+name+':ro']
    ...
    if phase=='train_10':cli+=['--decision-receipt','/review/decision.json']
...
def run_phases(args,baseline,report):
    expected=None;completed=dict(getattr(args,'completed_prior_phases',{}))
    for phase in selected_phases(args):
        ...
        save(report_path(args),report)
    verify_completed(args,completed)
    verify_prior_files(args)

def tree_hashes(p):return {str(f.relative_to(p)):sha(f) for f in p.rglob('*') if f.is_file()}

def admit_existing_campaign(args, baseline):
    if args.decision_receipt is None or not args.decision_receipt.is_file():raise ValueError('Explicit external learning decision receipt required')
    if args.decision_receipt==args.output or args.output in args.decision_receipt.parents:raise ValueError('Decision receipt must be outside writable campaign output')
    if not args.output.is_dir() or report_path(args).exists():raise ValueError('Exact prior admission and fresh learning report required')
    prior=read(args.output/'campaign.json')
    if (prior.get('status')!='completed' or prior.get('learning_admission_only_complete') is not True
        or prior.get('phase_group')!='admission' or prior.get('PPO_updates_completed')!=0
        or prior.get('consumer_freeze_sha256')!=CONSUMER_FREEZE
        or prior.get('host_freeze_sha256')!=args.host_freeze_sha256
        or prior.get('baseline')!=baseline or prior.get('source_and_inputs_unchanged') is not True
        or set(prior.get('accepted_phases',{}))!=set(ADMISSION_PHASES)):
        raise ValueError('Prior learning admission is incomplete or has different source/host identity')
    for phase in LEARNING_PHASES:
        for path in (args.output/phase,args.output/'jobs'/(phase+'.json'),args.output/'logs'/(phase+'.log'),args.output/(phase+'_accepted.json'),args.output/(phase+'_immutable.sha256.json')):
            if path.exists():raise ValueError('Never overwrite an existing learning phase: '+str(path))
    identity=validate_inputs(args,'train_10',True)
    args.completed_prior_phases={}
    for phase in ADMISSION_PHASES:
        accepted=read(args.output/(phase+'_accepted.json'))
        if accepted!=prior['accepted_phases'][phase] or accepted.get('passed') is not True:
            raise ValueError('Prior accepted phase receipt differs: '+phase)
        files=read(args.output/(phase+'_immutable.sha256.json'))
        if tree_hashes(args.output/phase)!=files:raise ValueError('Prior phase changed: '+phase)
        if phase=='standing':CONSUMER.verify_standing(args.output/'standing/admission.json',args.output)
        else:
            if CONSUMER.validate_result(args.output/phase,identity)!=accepted:raise ValueError('Prior consumer acceptance changed: '+phase)
        args.completed_prior_phases[phase]=files
    decision=CONSUMER.validate_learning_decision(consumer_args(args,'train_10',True),identity)
    assets=read(args.output/'inputs/study_before.sha256.json')
    if tree_hashes(args.output/'inputs/study')!=assets:raise ValueError('Prior admitted study assets changed')
    args.prior_file_hashes=tree_hashes(args.output)
    args.decision_sha256=sha(args.decision_receipt)
...
def main():
    ...
    parser.add_argument('--phase-group',choices=('admission','learn'),required=True)
    ...
    if any(args.output==p or p in args.output.parents for p in inputs):raise ValueError('Output outside immutable inputs required')
    ...
    args.prior_file_hashes={};args.completed_prior_phases={};decision=None
    if args.phase_group=='admission':
        if args.output.exists() or args.decision_receipt is not None:raise ValueError('Fresh admission output without a learning receipt required')
    ...
    else:assets,decision=admit_existing_campaign(args,baseline)
    ...
    report=dict(status='preparing',started_unix=time.time(),runtime_binding=runtime,source_manifest_sha256=SOURCE_MAP,
        ...
        phase_group=args.phase_group,learning_decision=decision,PPO_updates_completed=0,Stage2_complete=False,
        scope='Learning infrastructure admission, then separately reviewed ten-update continuation; no automatic allocation')
    try:
        save(report_path(args),report);run_phases(args,baseline,report)
        report.update(status='completed',learning_admission_only_complete=args.phase_group=='admission',
            ten_update_moving_pilot_and_matched_screens_passed=args.phase_group=='learn',automatic_continuation_allowed=False)
        ...
        try:
            ...
            if args.phase_group=='learn' and training_state.is_file():
            ...
            verify_completed(args,completed);verify_prior_files(args)
            if args.phase_group=='learn' and sha(args.decision_receipt)!=args.decision_sha256:raise ValueError('Decision receipt changed during learning')
        ...
        finally:report['finished_unix']=time.time();save(report_path(args),report)
