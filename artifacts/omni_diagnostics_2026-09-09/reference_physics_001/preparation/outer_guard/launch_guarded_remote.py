...
SOURCE = BASE / 'reference_physics_source_001'
OUTPUT = BASE / 'reference_physics_001'
PAUSE = BASE / 'forecast_pause_027'
UNIT = 'hexapod-reference-physics-001-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'a13f1534f9f802871a2cacbc0be603eae2d80738905381da40691f7cd4beff7c':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-omni-velocity-recording-007-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_026/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh full-C standing then measured-contact zero-residual reference; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-027', '--on-active=25m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_reference_physics_spark.py'),
