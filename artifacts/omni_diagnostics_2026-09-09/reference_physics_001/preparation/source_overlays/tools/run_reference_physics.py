...
from screen_contract import OPTIONS, WAVE, preflight, save, digest
...
def main():
    ...
    try:
        ...
        hold = env.reference_residual_controller.reference_position.clone()
        target_zero = torch.zeros_like(hold)
        valid = torch.ones(args.num_envs, device=env.device, dtype=torch.bool)
        clock = {'step': 0}
        generator = None
        ...
        with pre_reset_capture(env, capture) as captured:
            for step in range(args.steps):
                ...
                if args.mode == 'wave' and step >= WAVE['settle_steps']:
                    ...
                    env.set_reference_targets(hold, target_zero, target_zero, valid)
