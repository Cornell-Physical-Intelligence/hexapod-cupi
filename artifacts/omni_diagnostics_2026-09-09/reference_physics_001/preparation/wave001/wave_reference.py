    ...
    def step(self, snapshot, requested_forward_left_yaw, dt=.02):
        ...
        try:
            ...
            if self.current_leg is not None:
                i = self.current_leg; swing_end = self.swing.end_time
                if not measured['contact'][i]:
                    ...
                    self.flight_count = 0
                elapsed = self.time-self.swing.t0
                if measured['contact'][i] and self.flight_seen:
                    if self.time < swing_end-.20:
                        raise ValueError('Unexpected early touchdown; no unbounded endpoint correction')
                    if self.time >= swing_end-1e-8:
                        # Keep virtual endpoint fixed; latch measured point separately.
                        error = np.linalg.norm(self.swing.end-measured['reference_point_world_m'][i]-self.preload_world[i])
                        speed = np.linalg.norm(measured['reference_point_velocity_world_mps'][i])
                        if error > self.cfg.maximum_touchdown_error_m or speed > .04:
                            raise ValueError('Touchdown position/speed inconsistent with planned endpoint and recorded preload')
                        self.contact_count += 1
                        if self.contact_count >= self.cfg.contact_confirm_steps:
                            self.anchors[i] = self.swing.end.copy()
                            ...
                            self.current_leg = None; self.swing = None; self.mode = 'contact_hold'
                            self.hold_until = self.time+self.cfg.contact_hold_s; self.touchdowns += 1
                if self.current_leg is not None and self.time >= swing_end:
                    self.mode = 'awaiting_contact'
                    if not measured['contact'][i]:
                        target = np.zeros(3); factor = 0.
                    if not self.flight_seen or self.time-swing_end > self.cfg.max_touchdown_delay_s:
            ...
            if self.current_leg is not None:
                points[self.current_leg] = self.swing.sample(self.time)[0]
            body = self._body()
