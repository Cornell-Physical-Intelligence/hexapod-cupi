"""Dispatch only this fresh native reference recording under the user's existing pause authorization."""
...
OUTPUT = BASE / 'reference_progress_recording_001'
PAUSE = BASE / 'forecast_pause_036'
UNIT = 'hexapod-reference-progress-recording-001-20260910.service'
HOST = BASE / 'reference_progress_recording_launch_001/launch_recording_spark.py'
RUN = BASE / 'reference_physics_009'
ADAPTER = BASE / 'reference_progress_recording_adapter_001'
...
def main():
    ...
    if hashlib.sha256(HOST.read_bytes()).hexdigest() != '34f10865703c64f6c17c34edd211792735893d7057b9abfb7090e9a1f5645465':
    ...
    recording_host.validate_inputs(SimpleNamespace(source=SOURCE, run=RUN, adapter=ADAPTER))
    previous = call(['systemctl','--user','show','hexapod-reference-physics-009-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_035/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh native recording of admitted009 stepping reference; no actor, pose forcing or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-036', '--on-active=25m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            ...
            '--source', str(SOURCE), '--run', str(RUN), '--adapter', str(ADAPTER), '--output', str(OUTPUT)]
