"""Dispatch only this fresh standing-then-wave campaign under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_physics_source_008'
OUTPUT = BASE / 'reference_physics_008'
PAUSE = BASE / 'forecast_pause_034'
UNIT = 'hexapod-reference-physics-008-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'a73075ebdf5d6c986728b96117495b8de85adc8db62de1c41ffbe49eb6176d66':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-007-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_033/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh full-C standing and quiet admission followed by bounded zero-residual wave; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-034', '--on-active=25m',
