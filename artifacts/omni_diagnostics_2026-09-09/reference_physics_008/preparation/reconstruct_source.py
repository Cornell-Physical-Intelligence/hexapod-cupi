"""Verify or reconstruct exact reference008 from immutable reference007 + delta.
...
TARGET='a73075ebdf5d6c986728b96117495b8de85adc8db62de1c41ffbe49eb6176d66'
...
def main():
    ...
    if sha(parent/'campaign_source_hashes.json')!=PARENT:raise ValueError('Wrong immutable source007')
    ...
    if set(overlay)!={'tools/solver_comparison.py','tools/screen_contract.py','tools/launch_reference_physics_spark.py','tools/screen_metrics.py','tools/run_reference_physics.py','tools/wave_reference.py'}:
        raise ValueError('Only the reviewed fresh standing and wave delta is permitted')
    ...
    if virtual!=final:raise ValueError('Merged source does not equal frozen source008')
