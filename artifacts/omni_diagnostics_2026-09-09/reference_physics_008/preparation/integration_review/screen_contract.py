...
def preflight(args, source):
    if args.mode not in ('standing', 'wave'):
        raise ValueError('Only fresh standing and bounded wave phases exist')
    if digest(Path(source) / 'tools/wave_reference.py') != PROTOCOL['wave_dependency']['wave_reference_py_sha256']:
        raise ValueError('Exact reviewed horizontal-timing wave dependency required')
    ...
    if args.mode == 'wave':
        ...
        if (admission.get('identity') != identity or admission.get('status') != 'completed' or not admission.get('gate', {}).get('passed')
                or not admission.get('gate', {}).get('all_replica_quiet', {}).get('passed')):
