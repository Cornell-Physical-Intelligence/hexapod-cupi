...
sys.path[:0]=[str(Path(__file__).resolve().parent),str(Path(__file__).resolve().parents[1]/'reference_physics_adapter_007/source_007/tools')]
from screen_contract import preflight, verify_source

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[1]
BASE = ROOT / 'tmp/reference_physics_adapter_007/source_007/robot/hexapod_mkii_length_study'
sys.path.insert(1, str(ROOT / 'tmp/reference_physics_adapter_007/source_007/tools'))
...
def source_fixture(root):
    ...
    (source/'tools').mkdir()
    shutil.copy2(ROOT/'tmp/omni_reference_wave_004/wave_reference.py',source/'tools/wave_reference.py')
    freeze(source)
...
class ScreenContractTests(unittest.TestCase):
    def test_exact_plan_and_source_admit_standing_but_wave_needs_matching_receipt(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            args.admission.write_text(json.dumps({'identity':identity,'status':'completed','gate':{'passed':True,'all_replica_quiet':{'passed':True}}}))
            preflight(args,source)
            changed={**identity,'source_manifest_sha256':'0'*64}
            args.admission.write_text(json.dumps({'identity':changed,'status':'completed','gate':{'passed':True,'all_replica_quiet':{'passed':True}}}))
