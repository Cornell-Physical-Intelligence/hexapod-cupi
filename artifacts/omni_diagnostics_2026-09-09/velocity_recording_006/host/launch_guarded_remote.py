...
OUTPUT = BASE / 'omni_velocity_recording_006'
PAUSE = BASE / 'forecast_pause_025'
UNIT = 'hexapod-omni-velocity-recording-006-20260910.service'
...
def main():
    ...
    validate_inputs(SimpleNamespace(source=SOURCE,probe=BASE/'omni_velocity_probe_003',
        pilot=BASE/'omni_velocity_pilot_003',adapter=BASE/'omni_velocity_recording_adapter_006'))
    previous = call(['systemctl','--user','show','hexapod-omni-velocity-recording-005-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_024/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-025', '--on-active=20m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=720',
            ...
            '--pilot', str(BASE/'omni_velocity_pilot_003'), '--adapter', str(BASE/'omni_velocity_recording_adapter_006'),
            '--output', str(OUTPUT)]
