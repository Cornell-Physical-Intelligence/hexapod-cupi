"""Dispatch only this fresh standing and bounded opposing-pair load transfer under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_pair_source_001'
OUTPUT = BASE / 'pair_load_transfer_001'
PAUSE = BASE / 'forecast_pause_040'
UNIT = 'hexapod-pair-load-transfer-001-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '69a1a23304448d53f7681dfa87ab4fa652ad682ba0e45406e82187a66c4224f0':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-device-smoke-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_039/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then bounded LM/RM load transfer; no PPO or gait qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-040', '--on-active=25m',
