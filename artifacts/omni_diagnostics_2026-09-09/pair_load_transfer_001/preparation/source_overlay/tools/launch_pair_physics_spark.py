#!/usr/bin/env python3
"""Fresh full-C standing then bounded opposing-pair load-transfer diagnostic; no wave/PPO."""
import argparse
import fcntl
import hashlib
import json
...
from pair_screen_contract import PAIR_PROTOCOL
...
def command(source, output, name, phase):
    ...
    common = ["docker", "compose", "--env-file", "docker/.env.base", "-f", "docker/docker-compose.yaml",
        ...
        "/workspace/hexapod/tools/" + ("run_reference_physics.py" if phase == "standing" else "run_pair_physics.py"), "--mode", phase,
    ...
    if phase == "pair":
        common += ["--admission", "/outputs/standing/admission.json"]
...
def main():
    ...
    try:
        ...
        if (pair_identity.get('standing_identity') != identity or pair_identity.get('pair_protocol') != PAIR_PROTOCOL
            ...
            raise RuntimeError('Pair diagnostic does not meet its distinct source-bound proposed criteria')
        pair_solver = json.loads((args.output / 'pair/solver_comparison.json').read_text())
        ...
        check_source(args.source)
        report.update(status='completed', pair_state_sha256=digest(args.output / 'pair/state.json'),
            proposed_pair_diagnostic_criteria_met=True, physics_qualification=False,
