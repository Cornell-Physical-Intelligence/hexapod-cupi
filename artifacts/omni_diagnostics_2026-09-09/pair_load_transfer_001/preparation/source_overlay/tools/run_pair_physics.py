#!/usr/bin/env python3
"""Bounded full-C middle-pair load transfer; no wave/PPO or body-pose prescription."""
...
from pair_screen_contract import PAIR_PROTOCOL,preflight
...
def main():
    ...
    state={'status':'initializing','mode':'pair','identity':identity,'runtime_binding':RUNTIME,
        'started_unix':time.time(),'stage2_complete':False,'terrain_qualified':False,
    ...
    try:
        ...
        generator=PairLoadTransfer(layout['joint_names_runtime']);reset=generator.reset(snapshot)
        ...
        if failure is None and len(pair_rows)==1100:
            result=score_transfer(data,references,substeps=pair_recorder.data(),dt=env.step_dt)
