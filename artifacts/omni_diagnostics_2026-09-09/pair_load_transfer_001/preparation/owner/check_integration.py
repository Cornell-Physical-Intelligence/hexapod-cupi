...
HERE=Path(__file__).resolve().parent;source=HERE/'source_pair_001';sys.path.insert(0,str(source/'tools'))
import launch_pair_physics_spark as host
from pair_screen_contract import preflight,PAIR_PROTOCOL
...
args.mode='pair';args.num_envs=1;args.steps=1300;args.admission=HERE.parent/'reference_physics_results_009/run/standing/admission.json'
try:preflight(args,source)
except ValueError:old_rejected=True
else:raise AssertionError('Actual009 admission cannot authorize a new pair source')
with tempfile.TemporaryDirectory() as td:
 ...
 assert pair_identity['pair_protocol']==PAIR_PROTOCOL
...
result={'runtime':runtime,'source_manifest_sha256':host.digest(source/'campaign_source_hashes.json'),'source_files':len(json.loads((source/'campaign_source_hashes.json').read_text())),
 'actual009_old_admission_rejected':old_rejected,'failed_quiet_synthetic_receipt_rejected':failed_quiet_rejected,'pair_protocol':PAIR_PROTOCOL,
 'synthetic_receipts_used_for_CPU_logic_only':True,'physics_admitted':False,'all_original009_runtime_files_byte_identical':True,'no_origin_reset_adapter_in_source':not (source/'tools/matched_origin.py').exists()}
