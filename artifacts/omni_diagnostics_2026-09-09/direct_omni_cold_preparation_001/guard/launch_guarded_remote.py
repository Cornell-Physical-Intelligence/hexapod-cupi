"""External pause053 guard: two-phase direct315 cold measurement only."""
...
PAUSE=BASE/'forecast_pause_053'
UNIT='hexapod-direct-omni-cold-001-20260910.service'
HOST=BASE/'direct_omni_cold_host_001/launch_cold_spark.py'
SOURCE_SHA256='4671eab012aaf79ce49769cfdc1667142237a4b4a45966fd0399a9caa900c51b'
SUPERVISOR_SHA256='04942c62de1f557d8f117f648f990b6dac69443d61c546529d05a4e2a7f0e01e'
CONTRACT_SHA256='eb87f1dd456771950f7c1bf50ed0a51c1fc6ccb4631217e29d99fa8a84717095'
CHECKPOINT_SHA256='1971b782327408f8446b3271cc665dad8c9c515523cf8c87303141dc487d06e8'
HOST_SHA256='fe55d1a554de2a9ec574ffc8b7a1c34ea677148c7f63d88ae7c63d693ba71eec'
HOST_FREEZE_SHA256='efbeac2a0bfb7ebf93efb9ec4dce33363607092b30a7e81ac6351950e1fa6083'
PREVIOUS_UNIT='hexapod-learning-ppo-train-001-20260910.service'
PREVIOUS_INVOCATION='804a727558374268b185325305148b1a'
PRIOR_PINS={'reference_learning_ppo_001/learning_campaign.json': 'f2275f1615eeced38abfa6b67a54efee9d4e97e6798046272f364a7f5658a2ca', 'reference_learning_ppo_001/jobs/train_10.json': '37b7ae066bd546965dfdc5f29e792c7425a40d3611769d71291a6183d9faf4ac', 'forecast_pause_052/restored.json': '669f1159a1d271bef666a5100fe778dd9aaa4af368a22bb1e9233a75ab0be10a'}
...
def require_final_bindings():
    ...
    if set(PRIOR_PINS)!={'reference_learning_ppo_001/learning_campaign.json','reference_learning_ppo_001/jobs/train_10.json','forecast_pause_052/restored.json'}:raise RuntimeError('Exact failed training/restoration receipts required')
...
def verify_previous_owner():
    ...
    job=json.loads((BASE/'reference_learning_ppo_001/jobs/train_10.json').read_text())
    if job.get('status')!='failed' or job.get('cleanup_checked') is not True or not job.get('container_id') or not job.get('container_name'):raise RuntimeError('Prior failed training lacks exact cleanup proof')
...
def validate_cold_inputs(cold_host):
    if sha(SUPERVISOR_SOURCE/'campaign_source_hashes.json')!=SUPERVISOR_SHA256:raise RuntimeError('Changed supervisor source')
    if sha(SOURCE/'campaign_source_hashes.json')!=SOURCE_SHA256:raise RuntimeError('Changed cold source')
    if CHECKPOINT.is_symlink() or sha(CHECKPOINT)!=CHECKPOINT_SHA256:raise RuntimeError('Wrong original checkpoint')
    if sha(HOST)!=HOST_SHA256:raise RuntimeError('Changed cold host')
    verify_frozen(HOST.parent,HOST_FREEZE_SHA256);verify_frozen(CONTRACT,CONTRACT_SHA256)
    args=SimpleNamespace(source=SOURCE,checkpoint=CHECKPOINT,contract=CONTRACT,supervisor_source=SUPERVISOR_SOURCE,output=OUTPUT,isaaclab=Path('/home/orionh/IsaacLab'),host_freeze_sha256=HOST_FREEZE_SHA256)
    identity=cold_host.verify_inputs(args)
    if identity.get('source_manifest_sha256')!=SOURCE_SHA256 or identity.get('checkpoint_sha256')!=CHECKPOINT_SHA256:raise RuntimeError('Host admitted different cold inputs')
    if identity.get('training_allowed') is not False:raise RuntimeError('Cold measurement unexpectedly permits learning')
    return identity
...
def main():
    ...
    spec=importlib.util.spec_from_file_location('cold_host',HOST)
    cold_host=importlib.util.module_from_spec(spec);spec.loader.exec_module(cold_host)
    validate_cold_inputs(cold_host)
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh direct315 cold standing32x1000 then48x12s diagnostic only; no PPO updates or automatic continuation',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-053', '--on-active=35m',
    ...
    try:
        ...
        raise
    try:
        cmd=['systemd-run','--user','--unit='+UNIT,'--property=RuntimeMaxSec=1800',
