...
HERE=Path(__file__).resolve().parent;SOURCE=HERE/'source_rr_preload_002';sys.path.insert(0,str(SOURCE/'tools'))
...
identity,geometry=standing(a,SOURCE);a.mode='directional';a.num_envs=1;a.steps=2400;a.case='left_strafe';a.admission=HERE.parent/'reference_directional_results_002/run/standing/admission.json'
try:preflight(a,SOURCE)
except ValueError:old_rejected=True
else:raise AssertionError('Actual directional002 admission accepted for new source')
...
r={'runtime':runtime,'source_manifest_sha256':host.digest(SOURCE/'campaign_source_hashes.json'),'source_files':len(json.loads((SOURCE/'campaign_source_hashes.json').read_text())),'actual_directional002_old_admission_rejected':old_rejected,'failed_quiet_synthetic_receipt_rejected':quiet_rejected,'all_named_cases_preflight':cases,'synthetic_receipts_are_CPU_only':True,'physics_admitted':False}
p=HERE/'INTEGRATION_PREFLIGHT_FINAL.json'
