"""Dispatch only this fresh standing and one bounded RR preload strafe case under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_rr_preload_source_002'
OUTPUT = BASE / 'reference_rr_preload_002'
PAUSE = BASE / 'forecast_pause_048'
UNIT = 'hexapod-rr-preload-002-20260910.service'
...
def main():
    ...
    if hashlib.sha256((SOURCE/'tools/launch_directional_physics_spark.py').read_bytes()).hexdigest() != 'e4b16d9e05c08ee44e346baa27b69b426a5a5a1751a60132a1fe0993650194fb':
        raise RuntimeError('Wrong reviewed RR preload host')
    sys.path.insert(0, str(SOURCE / 'tools'))
    from launch_directional_physics_spark import check_source
    from launch_length_study_spark import preflight
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != 'bfb0bbde27fdfae1f2ddc7797ff12adf184c7cd68a8e637f5fce87f7e0e3cfa3':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-diagonal-pairs-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_047/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh32 standing then one cold RR preload strafe diagnostic; no PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-048', '--on-active=30m',
