...
SOURCE = BASE / 'reference_physics_source_003'
OUTPUT = BASE / 'reference_physics_003'
PAUSE = BASE / 'forecast_pause_029'
UNIT = 'hexapod-reference-physics-003-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '7c75f0372abcea9eace3a280a2c204164179b8c433fc9d6280f60dd189737e24':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-002-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_028/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-029', '--on-active=25m',
