"""Verify or reconstruct exact reference003 from immutable reference002 + delta.
...
TARGET='7c75f0372abcea9eace3a280a2c204164179b8c433fc9d6280f60dd189737e24'
...
def main():
    ...
    if sha(parent/'campaign_source_hashes.json')!=PARENT:raise ValueError('Wrong immutable source002')
    ...
    if set(overlay)!={'tools/wave_reference.py'}:
        raise ValueError('Only the reviewed landing delta is permitted')
    ...
    if virtual!=final:raise ValueError('Merged source does not equal frozen source003')
