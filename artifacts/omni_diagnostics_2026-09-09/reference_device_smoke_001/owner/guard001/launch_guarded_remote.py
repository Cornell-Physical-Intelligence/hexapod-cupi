"""Dispatch only this fresh bounded device bridge under the user's existing pause authorization."""
...
OUTPUT = BASE / 'reference_device_smoke_001'
PAUSE = BASE / 'forecast_pause_039'
UNIT = 'hexapod-reference-device-smoke-001-20260910.service'
HOST = BASE / 'reference_device_smoke_launch_002/launch_device_smoke_spark.py'
RUN = BASE / 'reference_physics_009'
ADAPTER = BASE / 'reference_device_smoke_adapter_001'
...
def main():
    ...
    if hashlib.sha256(HOST.read_bytes()).hexdigest() != 'b2cb3ea5c3869b81ff7becdbf56a73c3eb3a3e4112c26435dadea804164bafab':
        raise RuntimeError('Wrong reviewed recording host')
    host_manifest = HOST.parent / 'FREEZE_SHA256.json'
    if hashlib.sha256(host_manifest.read_bytes()).hexdigest() != 'adbb60b3d77b8d6b268dcf3060232fcf3c493361259a0997582e694d62a35658':
        raise RuntimeError('Wrong reviewed device host bundle')
    ...
    if host_actual != host_files or any(p.is_symlink() for p in HOST.parent.rglob('*')):
        raise RuntimeError('Device host or executable helper changed')
    sys.path.insert(0, str(HOST.parent))
    import importlib.util
    from types import SimpleNamespace
    spec = importlib.util.spec_from_file_location('recording_host', HOST)
    recording_host = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(recording_host)
    recording_host.validate_inputs(SimpleNamespace(source=SOURCE, run=RUN, adapter=ADAPTER, observation=OBSERVATION))
    previous = call(['systemctl','--user','show','hexapod-origin-rate-002-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_038/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Bounded one/32-replica device bridge holds; no actor, walking or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-039', '--on-active=25m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            ...
            '--source', str(SOURCE), '--run', str(RUN), '--adapter', str(ADAPTER), '--observation', str(OBSERVATION), '--output', str(OUTPUT)]
