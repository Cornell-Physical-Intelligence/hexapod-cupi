"""Dispatch only this fresh standing and five matched-origin cases under the user's existing pause authorization."""
...
SOURCE = BASE / 'reference_origin_source_001'
OUTPUT = BASE / 'origin_rate_001'
PAUSE = BASE / 'forecast_pause_037'
UNIT = 'hexapod-origin-rate-001-20260910.service'
...
def main():
    ...
    from launch_origin_physics_spark import check_source
    from launch_length_study_spark import preflight
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '47d190425712e7aec39c959fed3fa76aaaef9af6962860593cc0b558cd0df63f':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-progress-recording-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_036/restored.json').is_file():
    ...
    try:
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh standing and five matched-origin rate-fidelity cases; no wave, PPO or Stage2 qualification',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-037', '--on-active=75m',
        ...
        for fd in reversed(locks):
            os.close(fd)
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=3900',
            ...
            '/usr/bin/python3', str(SOURCE / 'tools/launch_origin_physics_spark.py'),
