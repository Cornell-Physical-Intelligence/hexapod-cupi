#!/usr/bin/env python3
"""Fresh standing then five cold matched-origin diagnostics; no wave/PPO or adoption."""
import argparse
import fcntl
import hashlib
import json
...
from origin_contract import PROTOCOL, CASES, load_initial
...
def command(source, output, name, phase):
    if phase != "standing" and phase not in CASES:
        raise ValueError("Only fresh standing and five named origin diagnostics exist")
    common = ["docker", "compose", "--env-file", "docker/.env.base", "-f", "docker/docker-compose.yaml",
        ...
        "/workspace/hexapod/tools/" + ("run_reference_physics.py" if phase == "standing" else "run_origin_physics.py"), "--mode", "standing" if phase == "standing" else "origin",
...
def main():
    ...
    report = dict(status="preparing", runtime_binding=runtime, started_unix=time.time(),
        ...
        source_manifest_sha256=digest(args.source / "campaign_source_hashes.json"), automatic_continuation=False, solver_comparison=SOLVER_PROTOCOL, origin_protocol=PROTOCOL)
    ...
    try:
        ...
        for case in CASES:
            ...
            if (case_identity.get('standing_identity') != identity or case_identity.get('origin_protocol') != PROTOCOL
                    or case_identity.get('case') != case or case_identity.get('translation_xy_m') != list(CASES[case])
                    or case_identity.get('standing_admission_sha256') != digest(args.output / 'standing/admission.json')
                    or result.get('acquisition_complete') is not True or result.get('control_steps') != 1000
                    or result.get('unchanged_physical_gate', {}).get('passed') is not True
                    or result.get('initial_readback', {}).get('passed') is not True or result.get('ground_readback', {}).get('passed') is not True):
                raise RuntimeError('Origin acquisition/readback differs from its exact fresh standing contract')
            ...
            if actual_solver.get('protocol') != SOLVER_PROTOCOL or actual_solver.get('actual_scene_attributes') != comparison.get('actual_scene_attributes'):
                raise RuntimeError('Origin solver readback differs from its fresh standing admission')
            if tree_hashes(package) != before:
                raise RuntimeError('Origin diagnostic changed its read-only admitted asset')
            case_states[case] = result
            report.update(status='running_origins', completed_cases=list(case_states))
            save(args.output / 'campaign.json', report)
        from origin_campaign_review import compare_cases
        paired = compare_cases(args.output, case_states)
        save(args.output / 'matched_origin_comparison.json', paired)
        check_source(args.source)
        report.update(status='completed', measurement_acquisition_completed=True,
                      all_cases_existing_bounds_met=all(r['all_existing_bounds_met'] for r in case_states.values()),
                      velocity_fidelity_qualified=False, case_state_sha256={case:digest(args.output / case / 'state.json') for case in CASES},
