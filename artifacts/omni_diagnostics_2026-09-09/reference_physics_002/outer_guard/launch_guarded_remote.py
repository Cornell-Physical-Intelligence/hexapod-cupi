...
SOURCE = BASE / 'reference_physics_source_002'
OUTPUT = BASE / 'reference_physics_002'
PAUSE = BASE / 'forecast_pause_028'
UNIT = 'hexapod-reference-physics-002-20260910.service'
...
def main():
    ...
    check_source(SOURCE)
    if hashlib.sha256((SOURCE/'campaign_source_hashes.json').read_bytes()).hexdigest() != '34390c6162f462baa4c531f1d5aff346ffbc98d4d0266a68ea797f5e5b992ee0':
        raise RuntimeError('Wrong reviewed reference source')
    previous = call(['systemctl','--user','show','hexapod-reference-physics-001-20260910.service','-p','ActiveState'])
    ...
    if not (BASE/'forecast_pause_027/restored.json').is_file():
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-028', '--on-active=25m',
