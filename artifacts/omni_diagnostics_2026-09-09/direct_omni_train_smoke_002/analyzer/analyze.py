...
NATIVE_FREEZE='20fbcd40a869b39d8e3143a6c9715ec62ada5cc6211eaf473f60f8b7167678cb'
...
def analyze(campaign,cold,output):
    ...
    inputs=Inputs();c=inputs.json(campaign/'campaign.json');old=inputs.json(cold/'campaign.json')
    require(c['identity']['schema']==SCHEMA,'Wrong native campaign schema')
    report={'schema':'direct315_matched_review_v1','native_contract_freeze_sha256':NATIVE_FREEZE,'campaign':c,'cold_campaign':old,
            'errors':[],'Stage2_complete':False,'automatic_continuation':False,'stop_baseline_available':False}
    require(c['identity']['source_manifest_sha256']==SOURCE and c['identity']['plan_sha256']==PLAN,'Unbound native source/plan')
    require(old['identity']['source_manifest_sha256']=='4671eab012aaf79ce49769cfdc1667142237a4b4a45966fd0399a9caa900c51b','Unbound cold004 source')
    require(old['status']=='completed' and old['terminal_inputs_unchanged'] is True,'Cold baseline integrity incomplete')
    expected_phases=['standing','train','final_constant','final_stop'] if c['allocation']=='smoke' else ['standing','initial_constant','initial_stop','train','final_constant','final_stop']
    require(c.get('planned_phases')==expected_phases,'Campaign phase allocation differs')
    report['phase_receipts']={}
    for phase in expected_phases:
        try:
            state=inputs.json(campaign/phase/'state.json');job=inputs.json(campaign/'jobs'/(phase+'.json'))
        ...
        except Exception as exc:report['errors'].append(phase+' identity/ownership receipt: '+repr(exc))
    def attempt(name,fn):
        try:report[name]=fn()
        except Exception as exc:report['errors'].append(name+': '+repr(exc))
    if (campaign/'train/training_receipt.json').is_file():attempt('training',lambda:training(inputs,campaign/'train',campaign/'logs/train.log'))
    else:report['errors'].append('No training receipt available; do not infer any completed updates')
    if (campaign/'train/failure.json').is_file():report['runtime_failure']=inputs.json(campaign/'train/failure.json')
    ...
    if report['errors']:
        ...
        report['next_decision']='Review every directional and quiet regression before separately dispatching the matched curriculum/CAPS pilots. Existing failures remain unqualified.'
    else:
        report['conclusion']='The bounded pilot completed. The tables show physical changes and regressions separately from optimizer loss.'
        report['next_decision']='Compare against the other matched branch and inspect all direction/quiet failures before any continuation. No loss-only or aggregate-only promotion.'
...
if __name__=='__main__':
    ...
    print(json.dumps({'conclusion':r['conclusion'],'updates':r.get('training',{}).get('updates_completed'),'errors':r['errors'],'report':str(a.output/'REPORT.md')},indent=2))
