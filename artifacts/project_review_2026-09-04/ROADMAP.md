...
**Historical execution state, 4 September: planning only.** That review paused new training and simulation runs. The user subsequently authorized execution and, on 9 September, requested Stage 2 completion as fast as possible with terrain/perception work in parallel. This supersedes the planning hold; it does not waive physical-model, actuator or qualification gates. See [canonical current status](../../STATUS.md) for verified jobs and outcomes, and [the prepared run sequence](../../docs/NEXT_RUNS.md) for experiment context.
...
### 9 September context for collaborators

The newer production work is the [physical four-bar candidate](../../docs/MKII_FOURBAR_TRAINING.md), with 31 bodies, 30 articulation coordinates and 18 motor actions. The serial-model audit and the statement that CAD was complete in this September 4 snapshot refer to their dated source state; they do not establish qualification of the physical mechanism or the newly selected C geometry.

The user separately selected the [C length study](../length_study_2026-09-09/README.md): 72.5 mm femur, 126 mm tibia, fixed coxa, and transferred current CAD mass/inertia on scaled mock geometry. Detailed C motor/linkage packaging remains unverified. Preserve the exact [forward Benchmark 1](../length_study_2026-09-09/benchmarks/benchmark_01_c_300/README.md) as immutable comparison evidence; a forward demo is not the omnidirectional or terrain milestone.

**Stage 2 is still open.** Smooth translation through all bearings, both yaw signs, combined translation/turning and paths, reversals/stops, and quiet zero-command standing must pass the tracking, contact, stability and motor-load gates together. Position and body heading remain independently plannable through a forward/left/yaw-rate interface. A finished PPO budget, an ideal-localization path video or a terrain-only fixture pass does not qualify full-robot motion.

Parallel preparation now has a [terrain/sensing plan](TERRAIN_AND_SENSING_PLAN_2026-09-09.md), [CPU terrain readiness](../terrain_readiness_2026-09-09/readiness.json), [mild-terrain curriculum](../terrain_readiness_2026-09-09/mild_curriculum_001/curriculum.json), [sampled serial-CAD mount study](../sensor_mount_study_2026-09-09/README.md) and [synthetic perception replay](../perception_readiness_2026-09-09/README.md). Their source hashes and limitations matter: they do not qualify the newer four-bar/C assembly, real sensing, ROS or a terrain student. The user has a Mid-360 and D455 and allows additional hardware evaluation; existing mechanical procurement/test decisions remain intact. Live runtime admission and training state belong in STATUS, not this dated plan.
...
| D07: no new runs for now; review and push the plan | Confirmed, latest instruction | No launch, queue, unattended restart or monitoring automation is created by this review. Revisit on the user's next execution request. |
...
| F01: imported USD fails all-link inertia round trip | Measured | Repair and revalidate before new training; prior scratch CAD runs cannot certify the repaired asset. |
