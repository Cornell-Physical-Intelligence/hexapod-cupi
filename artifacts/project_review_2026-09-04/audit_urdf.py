...
parser.add_argument('--output', type=Path, default=OUT / 'static_audit.json')
...
cfg={}
spec_path = ROOT / 'packages/hexapod_env/hexapod_env/assets/spec.py'
if spec_path.exists():
    # Main separates the CAD spec from the compatibility shim and mock config.
    # Read literals without importing the simulator or executing repository code.
    fields = {'COXA_LIMITS': 'coxa_limits', 'FEMUR_LIMITS': 'femur_limits',
              'TIBIA_LIMITS': 'tibia_limits', 'STANCE_ROOT_HEIGHT_M': 'nominal_height_m',
              'STANCE_RESET_ROOT_HEIGHT_M': 'reset_root_height_m',
              'STANCE_FEMUR_RAD': 'stance_femur_rad', 'STANCE_TIBIA_RAD': 'stance_tibia_rad'}
    for node in ast.parse(spec_path.read_text()).body:
        if (isinstance(node, ast.Assign) and any(isinstance(t, ast.Name) and t.id == 'MKII_V1_ASSET' for t in node.targets)
                and isinstance(node.value, ast.Call)):
            keywords = {item.arg: item.value for item in node.value.keywords}
            for key, field in fields.items():
                cfg[key] = ast.literal_eval(keywords[field])
else:
