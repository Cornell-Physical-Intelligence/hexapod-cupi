...
The completed `omni_flat_002` campaign ended at 22:55 UTC on 9 September, after 1,425 resumed PPO updates. Its final stage still passed 0/154 static command gates and 0/14 transition gates. The median scenario had 14.3% requested motor saturation; actual applied torque remained clipped at 1.6 N·m. A visually convincing closed-loop path can coexist with poor open-loop velocity tracking and excessive motor demand. The subsequent [controlled diagnostic campaign](../omni_diagnostics_2026-09-09/comparison_report.json) completed the unchanged-controller, slower-target-slew and solver-flag comparisons. Slower slew reduced median saturation and power but regressed some directions; the following [100-update quiet/smooth pilot](../omni_diagnostics_2026-09-09/repair_002/repair.json) was rejected. These are completed experiments, not queued work. Canonical [STATUS](../../STATUS.md) records current control probes and next actions. Terrain and perception development proceed in parallel; full terrain qualification remains a subsequent milestone.
...
Parallel work has delivered pre-reset PPO motor/velocity diagnostics, the original 30-fixture Isaac collision/ray/contact pass, a conservative full-C terrain adapter, and 2/4/6-camera screening against a hashed serial-CAD snapshot and sampled joint poses. The [terrain integration notes](../../isaaclab/hexapod_terrain/README.md), [mount study](../sensor_mount_study_2026-09-09/README.md), and [synthetic perception replay](../perception_readiness_2026-09-09/README.md) distinguish completed tests from remaining admission. These do not establish terrain walking, real sensor fusion or final C mechanical packaging. Mount results must be repeated for current physical four-bar motion and final detailed C CAD, then measured with real cameras. Use STATUS for live work rather than this dated preparation record.

![Prepared terrain collision fixtures](terrain_fixture_gallery.png)

The user made Stage 2 quality explicit: every direction, both turns, combined arcs and path transitions must be as smooth as the liked forward animation, with a quiet static stance after stopping. The completion contract is saved under `artifacts/omni_flat_2026-09-09/STAGE2_COMPLETION_CONTRACT.json`. Quiet-standing rewards and per-direction regression checks were implemented, and the bounded reward-repair pilot was subsequently rejected as recorded above. The separate quiet/stop and visual review modules still need their own Isaac smoke and evaluation. No Stage 2 completion is claimed.
