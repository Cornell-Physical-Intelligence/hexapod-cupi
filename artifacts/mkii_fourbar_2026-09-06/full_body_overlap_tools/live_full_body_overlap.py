...
from contact_audit import CAPACITY, LAYOUT, expected_bindings, read_pair_sample, grade
...
def compare_cases(filtered, negative):
    ...
    for result, case in ((filtered, "filtered"), (negative, "unfiltered_negative")):
        ...
        errors += [f"{case}: {error}" for error in grade(case, result.get("metrics", {}), result.get("physics_steps_requested"))]
    for key in ("source_identity", "fixture_source_sha256", "physics_steps_requested", "numerical_recipe",
                "solver_multiplier", "solver_iterations", "initial_reset_roots_m", "fixture_dependency_sha256", "native_pair_bindings", "native_rigid_path_readbacks"):
...
def main(argv=None):
    ...
    report = {"schema": "hexapod.live_full_body_collision_fixture.v1", "case": early.case,
        ...
        "fixture_dependency_sha256": digest(Path(__file__).with_name("contact_audit.py")),
        "scope": "Diagnostic full-body robot-pair contacts; unexcited negative bodies remain explicit limitations, never admission"}
    ...
    try:
        if early.compare:
            ...
            report["all_negative_source_channels_excited"] = cases[1].get("all_source_bodies_excited_in_negative") is True
            report["unexcited_negative_source_bodies"] = cases[1].get("unexcited_source_bodies")
        ...
        os.environ["HEXAPOD_MKII_ENVIRONMENT_LAYOUT"] = LAYOUT
        ...
        with launch_simulation(cfg, args):
            try:
                ...
                from hexapod_env.tasks.mkii_fourbar_v1.environment_layout import validate_layout_runtime, validate_layout_evidence
                ...
                validate_layout_runtime(env.runtime_manifest, LAYOUT)
                report["environment_layout"] = getattr(env, "layout_report", None)
                if not report["environment_layout"]:
                    raise ValueError("Candidate must expose actual pre-construction coincident layout verification")
                ...
                report["startup_ownership_evidence_scope"] = ("Production isolation was verified before the artifact-only mutual allowlink override; the negative case is deliberately not isolated"
                    if args.case == "unfiltered_negative" else "Production startup isolation remains unchanged")
                ...
                if not bool((origins == 0.).all()):
                    raise ValueError("Candidate did not initialize coincident origins before physics construction")
                env.reset(seed=0)
                validate_layout_evidence(env.layout_report, 2)
                ...
                if digest(ROOT / "configs/mkii_fourbar_v3_kinematics.json") != env.runtime_manifest["kinematics_sha256"]:
                    raise ValueError("Exact body mapping differs from the executed kinematics")
                bindings = expected_bindings(kinematics["body_paths"], list(env.scene.env_prim_paths))
                physics = PhysxManager.get_physics_sim_view()
                views, native_paths = [], {}
                for binding in bindings:
                    for path in [binding["source_body"], *binding["target_filters"]]:
                        if path not in native_paths:
                            rigid = physics.create_rigid_body_view(path)
                            resolved = list(rigid.prim_paths)
                            if resolved != [path] or not rigid.check():
                                raise ValueError("Native full-body path did not bind exactly one expected rigid body")
                            native_paths[path] = resolved
                    view = physics.create_rigid_contact_view(binding["source_body"],
                        filter_patterns=binding["target_filters"], max_contact_data_count=CAPACITY)
                    if view.sensor_count != 1 or view.filter_count != 31 or not view.check():
                        raise ValueError("Native full-body contact view did not resolve1source/31target filters")
                    views.append(view)
                report["native_pair_bindings"] = bindings
                report["native_rigid_path_readbacks"] = native_paths
                report["contact_filter_binding_scope"] = "Exact paths independently resolve through native rigid views; column order follows explicit filter list in documented contact API (no filter-path getter available)."
                report["native_pair_count_dtypes"] = [None]*62
                report["pair_count_storage_dtype"] = "int64"
                actions = torch.zeros(2, 18, device=env.device)
                pair_forces, pair_counts, ground_forces = [], [], []
                capacity_rows, start_rows = [], []
                columns = [f"view_{v}_target_{t}_force_{axis}" for v in range(62) for t in range(31) for axis in "xyz"]
                columns += [f"view_{v}_target_{t}_count" for v in range(62) for t in range(31)]
                ...
                for step in range(args.physics_steps):
                    ...
                    forces, counts, starts, capacities = [], [], [], []
                    for pair_index, view in enumerate(views):
                        force, count, start, full, dtypes = read_pair_sample(view, wp.to_torch, contract.PHYSICS_DT_S)
                        previous = report["native_pair_count_dtypes"][pair_index]
                        if previous is not None and previous != dtypes:
                            raise ValueError("Native contact index/count dtype changed")
                        report["native_pair_count_dtypes"][pair_index] = dtypes
                        forces.append(force); counts.append(count); starts.append(start); capacities.append(full)
                    force, count = torch.stack(forces), torch.stack(counts)
                    capacity_rows.append(capacities)
                    start_rows.append(torch.stack(starts).cpu().numpy().copy())
                ...
                report["metrics"] = {"samples": len(rows), "finite": True,
                    ...
                    "contact_buffer_capacity_reached": bool(np.asarray(capacity_rows).any()),
                ...
                for index, binding in enumerate(bindings):
                    count_seen = bool((counts[:,index] > 0).any())
                    force_seen = bool((np.linalg.norm(forces[:,index],axis=-1) > 1e-3).any())
                    coverage.append({"source_body": binding["source_body"], "source_name": binding["source_name"],
                        "source_environment": binding["source_environment"], "contact_count_observed": count_seen,
                        "force_above_1mN_observed": force_seen,
                        "joint_count_and_force_observed": bool(((counts[:,index] > 0) & (np.linalg.norm(forces[:,index],axis=-1) > 1e-3)).any()),
                        "excited_target_indices": np.flatnonzero((counts[:,index] > 0).any(0)).tolist()})
                report["metrics"]["source_body_excitation"] = coverage
                report["metrics"]["foreign_contact_observed_per_direction"] = [
                    bool(((counts[:,i*31:(i+1)*31] > 0) & (np.linalg.norm(forces[:,i*31:(i+1)*31],axis=-1) > 1e-3)).any()) for i in range(2)]
                report["unexcited_source_bodies"] = [row["source_body"] for row in coverage
                    if not row["joint_count_and_force_observed"]]
                report["all_source_bodies_excited_in_negative"] = args.case == "unfiltered_negative" and not report["unexcited_source_bodies"]
                report["negative_coverage_limitation"] = ("No negative-control excitation is expected in the filtered case" if args.case == "filtered" else
                    ("Complete62-source excitation observed" if report["all_source_bodies_excited_in_negative"] else "Unexcited bodies are explicitly listed; do not claim their channels were dynamically proven by this negative control"))
                report["fixture_complete"] = True
                report["errors"] += grade(args.case, report["metrics"], args.physics_steps)
                    ...
                    if rows:
                        ...
                        with trace_path.open("xb") as stream:
                            np.savez_compressed(stream, values=np.stack(rows), pair_contact_counts=np.stack(pair_counts),
                                pair_contact_starts=np.stack(start_rows), capacity_reached=np.asarray(capacity_rows),
