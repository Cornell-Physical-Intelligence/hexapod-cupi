...
from contact_audit import expected_bindings, grade, LAYOUT
FIXTURE_MOUNT = '/workspace/overlap_fixture'
FIXTURE_SCRIPT = 'live_full_body_overlap.py'
...
def require_report(path, *, case, steps, fixture_hash, contract, solver_multiplier, contract_source_dir):
    ...
    expected = {'schema': 'hexapod.live_full_body_collision_fixture.v1', 'case': case,
    ...
    bindings = result.get('native_pair_bindings')
    if not isinstance(bindings, list) or len(bindings) != 62:
        raise ValueError('Missing62 exact full-body view bindings')
    source = Path(contract_source_dir)
    layout_path = source / 'packages/hexapod_env/hexapod_env/tasks/mkii_fourbar_v1/environment_layout.py'
    spec = importlib.util.spec_from_file_location('frozen_full_overlap_layout', layout_path)
    layout = importlib.util.module_from_spec(spec); spec.loader.exec_module(layout)
    layout.validate_layout_runtime(result.get('runtime_manifest'), LAYOUT)
    kinematics = json.loads((source / 'configs/mkii_fourbar_v3_kinematics.json').read_text())
    expected_bindings_value = expected_bindings(kinematics['body_paths'], ['/World/envs/env_0', '/World/envs/env_1'])
    if bindings != expected_bindings_value:
        raise ValueError('Full-body native contact views do not cover every exact foreign body in both directions')
    paths = {row['source_body']: [row['source_body']] for row in bindings}
    if result.get('native_rigid_path_readbacks') != paths:
        raise ValueError('Exact native rigid-body readback map is incomplete')
    metrics = result.get('metrics', {})
    if not isinstance(metrics, dict) or grade(case, metrics, steps):
        raise ValueError('Incomplete full-body physical contact evidence')
    for binding, coverage in zip(bindings, metrics['source_body_excitation']):
        if any(coverage.get(key) != binding[key] for key in ('source_body','source_name','source_environment')):
            raise ValueError('Negative-excitation body identity changed')
    dtypes = result.get('native_pair_count_dtypes')
    if (not isinstance(dtypes,list) or len(dtypes) != 62
            or any(row != {'counts':'torch.uint32','start_indices':'torch.uint32'} for row in dtypes)
            or result.get('pair_count_storage_dtype') != 'int64'):
        raise ValueError('Native UInt32 count/index evidence is missing')
    if not isinstance(result.get('environment_layout'),dict) or not result['environment_layout']:
        raise ValueError('Missing actual candidate layout evidence')
    layout.validate_layout_evidence(result['environment_layout'], 2)
    if result.get('fixture_dependency_sha256') != hashlib.sha256(Path(__file__).with_name('contact_audit.py').read_bytes()).hexdigest():
        raise ValueError('Full-body helper source identity differs')
...
def run_case(host, args, source, fixture, output, contract, fixture_identity, shared_digest, case):
    ...
    report = {'schema': 'hexapod.collision_overlap_supervisor.v1', 'case': case,
              ...
              'coordination_protocol': 'canonical_share_status_v2',
    ...
    def unchanged():
        ...
        report['coordination_latest'] = host.read_coordination_control()
        host.require_coordination_none(report['coordination_latest'])
        ...
        try:
            ...
            if success:
                ...
                report['fixture_summary'] = require_report(output / 'report.json', case=case,
                    ...
                    contract=contract, solver_multiplier=args.solver_multiplier, contract_source_dir=source)
...
def main(argv=None):
    ...
    if not (source / 'isaaclab/deploy/run-mkii-fourbar').is_file() or not (fixture / FIXTURE_SCRIPT).is_file():
        parser.error('Frozen source supervisor and external live_full_body_overlap.py are required')
    ...
    try:
        ...
        if tree_identity(fixture) != fixture_identity:
            raise host.Blocked('External fixture changed during comparison')
