#!/usr/bin/env python3
"""One exact campaign → one guarded video; no GPU API and no recurring automation."""
...
def verify_campaign_process(identity, binding):
    ...
    argv = identity["argv"]
    if (len(argv)<2 or Path(argv[1]).resolve() != Path(binding["source"])/"isaaclab/deploy/run-mkii-fourbar-campaign"
...
def main(argv=None):
    ...
    with (folder/"follower.lock").open("a") as lock:
        ...
        state = common.read_json(path) if path.exists() else {"schema":"hexapod.campaign_capture_followup.v1",
        ...
        try:
            ...
            while state["state"] == "waiting":
                ...
                if campaign.get("state") == "complete":
                    ...
                    # Python exec may take a moment; record exact post-exec argv, never infer ownership.
                    owned = None
                    for _ in range(100):
