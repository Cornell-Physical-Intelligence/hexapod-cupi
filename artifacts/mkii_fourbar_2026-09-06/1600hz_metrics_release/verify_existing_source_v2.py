...
assert base.is_dir()
source=base/'source';assert source.is_dir()
with tarfile.open(archive) as tf:
 ...
 # Previous attempt extracted exact archive; verify in place without overwriting.
manifest=source/meta['manifest']
assert hashlib.sha256(manifest.read_bytes()).hexdigest()==meta['manifest_sha256']
rows=[line for line in manifest.read_text().splitlines() if line.strip() and not line.startswith('#')]
assert len(rows)==308
for row in rows:
