...
class CompleteMetricsTests(unittest.TestCase):
    ...
    def test_exact_baseline_patch_scope_and_unchanged_public_helpers_draining_grading(self):
        ...
        runtime_path = (ROOT/'isaaclab/validate_mkii_fourbar.py').resolve()
        candidate_path = Path(SETTINGS.get('candidate',HERE/'proposed_validate_mkii_fourbar.py.txt')).resolve()
        # Proposal review keeps the runtime at baseline. The permanent
        # regression explicitly supplies the installed file as its candidate.
        expected_runtime = after if candidate_path == runtime_path else before
        self.assertEqual(runtime_path.read_bytes(),expected_runtime)
        def excluded(text):
            tree=ast.parse(text)
