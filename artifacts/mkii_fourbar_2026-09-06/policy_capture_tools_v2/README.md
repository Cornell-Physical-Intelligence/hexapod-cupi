# External admitted-policy video capture v2
...
This version preserves the published `policy_capture_tools/` folder unchanged.
It requires the exact frozen training source's `runner_config_dict` helper before
constructing RSL-RL. That helper applies Isaac Lab's compatibility adapter to
remove obsolete MLP constructor fields while retaining the configured Gaussian
standard deviation and observation normalization. V2 does not independently
translate those fields or change the training configuration. Legacy source
without the corrected helper is rejected before Kit startup.
...
uv run python -m unittest discover \
  -s artifacts/mkii_fourbar_2026-09-06/policy_capture_tools_v2 -p 'test_*.py' -v
```

The 21 tests include real source/checkpoint byte corruption rejection, incomplete
...
Additional descriptor tests retain actor Gaussian `init_std=0.15`, actor
observation normalization and critic `distribution_cfg=None` through the shared
adapter boundary, and reject obsolete constructor keys or an uncorrected source.
