#!/usr/bin/env python3
"""Bounded 32-environment standing profiler; never starts PPO or grants admission.
...
def tools_identity(directory):
    return {name: hashlib.sha256((directory/name).read_bytes()).hexdigest()
            for name in ("run_profile.py", "profile_validation.py")}
...
def main():
    ...
    source, output = (p.resolve() for p in (args.source_dir, args.output))
    tool_dir = Path(__file__).resolve().parent
    tool_hashes = tools_identity(tool_dir)
    if output.exists():
    ...
    name = "hexapod-profile-" + owner[:12]
    opts = SimpleNamespace(mode="validate", steps=100, solver_multiplier=args.solver_multiplier,
    ...
    argv[entrypoint:entrypoint] = ["-v", f"{tool_dir}:/workspace/profile_tools:ro"]
    python_index = argv.index("/workspace/isaaclab/_isaac_sim/python.sh")
    validator_args = argv[python_index+2:]
    validator_args[validator_args.index("--report")+1] = "/workspace/validation_artifacts/validation_report.json"
    argv[python_index:] = [argv[python_index], "/workspace/profile_tools/profile_validation.py",
        "--source-dir", "/workspace/hexapod", "--source-sha256", args.source_sha256,
        "--output-dir", "/workspace/validation_artifacts", *validator_args]
    if not args.execute:
        print(json.dumps({"execution": "not_started", "argv": argv,
            "source_identity": contract, "profiling_tools_sha256": tool_hashes, "timeout_seconds": 900}, indent=2))
        return 0
    descriptors = []
    container = None
    report = {"execution": "preflight", "owner": owner, "name": name,
        "source_identity": contract, "profiling_tools_sha256": tool_hashes,
        "supervisor_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        "purpose": "100-control standing profile, not physics admission or PPO",
    ...
    try:
        ...
        host.require_unchanged_source(source, contract, "before diagnostic profile")
        if tools_identity(tool_dir) != tool_hashes:
            raise RuntimeError("Profiling tools changed before launch")
        ...
        host.atomic_json(output / "supervisor.json", report)
        print("FOURBAR_PROFILE_STARTED " + str(output), flush=True)
        while True:
            ...
            if not current["running"]:
                ...
                if (result.get("pass") is not True or result.get("coverage_pass") is not True
                        or result.get("simulation_training_admission") is not False
                        or result.get("source_identity") != contract
                        or result.get("wrapper_sha256") != tool_hashes["profile_validation.py"]):
                    raise RuntimeError("Diagnostic profile or coverage did not pass")
                if tools_identity(tool_dir) != tool_hashes:
                    raise RuntimeError("Profiling tools changed during execution")
                host.require_unchanged_source(source, contract, "after diagnostic profile")
                report["execution"] = "finished"
                break
            if time.monotonic() >= deadline:
                raise RuntimeError("Diagnostic profile exceeded 900 seconds")
            host.require_coordination_none(host.read_coordination_control())
