...
def publish_before_shutdown(validator, expected_path, publish):
    """Observe the original final report write without changing its bytes/order."""
    original = validator.write_json
    ...
    def write(path, value, *args, **kwargs):
        ...
        if Path(path).resolve() == expected_path:
            publish(value)
        return result
    validator.write_json = write
    return lambda: setattr(validator, 'write_json', original)
...
def main(argv=None):
    ...
    report = {'schema': 'hexapod.physical_substep_profile.v2', 'pass': False,
    ...
    persisted = False
    def publish(final_validation=None, error=None):
        # Kit may terminate the process in launch_simulation's app.close().
        # All durable evidence must therefore be written from the validator's
        # final write callback, while still inside the live app context.
        nonlocal persisted
        if persisted:
            return
        ...
        try:
            ...
            if final_validation is None:
                final_validation = json.loads((out/'validation_report.json').read_text())
            report['validation_report_sha256'] = digest(out/'validation_report.json')
            report['physical_report_pass'] = final_validation.get('pass') is True
            report['physics_substeps'] = final_validation.get('physics_substeps')
            ...
            report['coverage_pass'] = (len(recorder.control_rows) == STEPS
                ...
                and final_validation.get('physics_substeps') == expected
                and final_validation.get('steps_completed') == STEPS)
            report['pass'] = (error is None and report['physical_report_pass']
                and report['coverage_pass'] and report.get('cuda_activity_observed') is True)
        except BaseException as failure:
            report['pass'] = False
            report['export_error'] = type(failure).__name__ + ': ' + str(failure)
            report.setdefault('control_timings', recorder.control_rows)
        report['publication_boundary'] = 'validator final report written; before environment/app shutdown'
        write_json(out/'profile.json', report)
        persisted = True
        print('FOURBAR_PROFILE_PERSISTED ' + json.dumps({'pass': report['pass'],
            'profile': str(out/'profile.json')}), flush=True)

    restore = instrument(validator, recorder)
    restore_write = publish_before_shutdown(validator, out/'validation_report.json', publish)
    exit_code = 1
    try:
        exit_code = validator.main(validator_args)
        # Fallback for a kitless or future launcher that returns normally.
        if not persisted:
            publish(error=RuntimeError(f'Validator returned {exit_code} without its final report callback'))
    ...
    return 0 if report['pass'] and exit_code == 0 else 1
