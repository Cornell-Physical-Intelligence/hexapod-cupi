...
class FollowTests(unittest.TestCase):
    ...
    def wrapper_fixture(self):
        source=self.root.parent/'source'
        wrapper=self.root.parent/'reviewed_wrapper.py'
        wrapper.write_text('# reviewed, separately hashed campaign host wrapper\n')
        binding={'campaign_pid':42,'campaign_start_ticks':555,'source':str(source),
                 'campaign_file':str(self.root/'campaign.json'),'source_commit':'c'*40,
                 'campaign_wrapper':str(wrapper),'campaign_wrapper_sha256':self.common.digest(wrapper)}
        identity={'pid':42,'start_ticks':555,'argv':['python3',str(wrapper),
            '--source-dir',str(source),'--source-commit','c'*40,'--output-root',str(self.root.parent)]}
        return wrapper,binding,identity

    def test_reviewed_wrapper_requires_exact_bound_bytes_and_same_source_arguments(self):
        wrapper,binding,identity=self.wrapper_fixture()
        follower.verify_campaign_process(identity,binding)
        for change in ({'campaign_wrapper_sha256':'f'*64}, {'campaign_wrapper_sha256':None},
                       {'campaign_wrapper_sha256':'invalid'}):
            with self.subTest(change=change),self.assertRaises(ValueError):
                follower.verify_campaign_process(identity,{**binding,**change})
        unbound={k:v for k,v in binding.items() if not k.startswith('campaign_wrapper')}
        with self.assertRaises(ValueError):follower.verify_campaign_process(identity,unbound)
        argv=list(identity['argv']);argv[argv.index('--source-commit')+1]='d'*40
        with self.assertRaises(ValueError):follower.verify_campaign_process({**identity,'argv':argv},binding)
        argv=list(identity['argv']);argv[argv.index('--output-root')+1]='/foreign'
        with self.assertRaises(ValueError):follower.verify_campaign_process({**identity,'argv':argv},binding)

    def test_changed_wrapper_rejected_even_with_complete_campaign_and_no_live_pid(self):
        wrapper,binding,identity=self.wrapper_fixture()
        binding['source_sha256']=self.contract['sha256']
        campaign={**self.campaign,'schema':'hexapod.fourbar_campaign.v1','campaign':str(self.root),
                  'source':binding['source'],'pid':42,'source_commit':'c'*40}
        follower.verify_campaign_header(campaign,binding)
        wrapper.write_text('# changed after follower starts\n')
        with self.assertRaisesRegex(ValueError,'wrapper bytes'):
            follower.verify_campaign_header(campaign,binding)
        with self.assertRaisesRegex(ValueError,'wrapper bytes'):
            follower.verify_campaign_process(identity,binding)

    def test_wrapper_identity_checked_again_before_video_is_accepted(self):
        folder,output,state,common,verified,supervisor,result,calls=self.capture_fixture()
        wrapper,binding,identity=self.wrapper_fixture()
        state['binding'].update({k:v for k,v in binding.items() if k.startswith('campaign_wrapper')})
        follower.verify_completed_capture(folder,state,common)
        wrapper.write_text('# changed during capture\n')
        with self.assertRaisesRegex(ValueError,'wrapper bytes'):
            follower.verify_completed_capture(folder,state,common)

    def test_wrapper_cli_requires_both_path_and_hash(self):
        required=['--campaign',str(self.root/'campaign.json'),'--source-dir',str(self.root/'source'),
            '--capture-tools',str(self.root/'tools'),'--state-dir',str(self.root/'state'),
            '--campaign-pid','42','--campaign-start-ticks','555','--source-sha256','a'*64,
            '--source-commit','c'*40]
        for partial in (['--campaign-wrapper',str(self.root/'wrapper.py')],
                        ['--campaign-wrapper-sha256','b'*64]):
            with self.subTest(partial=partial),patch('sys.stderr'),self.assertRaises(SystemExit) as error:
                follower.main(required+partial)
            self.assertEqual(error.exception.code,2)

    def test_capture_shared_lock_prefix_is_nonblocking_no_fork_and_job_only(self):
        command=['/usr/bin/python3','/capture/capture_policy.py','--source-dir','/source']
        self.assertEqual(follower.capture_launch_command(command,{}),command)
        prefixed=follower.capture_launch_command(command,{'capture_shared_lock':'/opt/wx/gpu.lock'})
        self.assertEqual(prefixed,['/usr/bin/flock','--nonblock','--no-fork','/opt/wx/gpu.lock',*command])
        self.assertEqual(command,['/usr/bin/python3','/capture/capture_policy.py','--source-dir','/source'])
        with self.assertRaisesRegex(ValueError,'Unreviewed'):
            follower.capture_launch_command(command,{'capture_shared_lock':'/foreign'})
        state={'state':'waiting'};records=[]
        follower.reserve_launch(state,lambda:records.append(copy.deepcopy(state)),prefixed)
        self.assertEqual(records[0]['capture_argv'],prefixed)
        with self.assertRaisesRegex(ValueError,'duplicate'):
            follower.reserve_launch(state,lambda:None,prefixed)
