...
## Execution record, updated 9 September

The `omni_flat_002` campaign completed 1,425 resumed PPO updates; its final
evaluation passed 0/154 static and 0/14 transition gates. Stage 2 remains open.
Use [STATUS](../../STATUS.md) for the current control probes and next actions,
and the [controlled diagnostic report](../omni_diagnostics_2026-09-09/comparison_report.json)
for the completed slew/solver comparison. The later 100-update quiet/smooth
pilot was rejected; it is not a queued continuation.

Historical launch context: frozen `omni_source_v3` resumed the verified model425
checkpoint with gentle-through-tight arcs and the expanded evaluator. Its exact
standing validation passed with 32 environments, 19 bodies, 18 joints, six feet,
no falls/truncations/non-foot contacts or settled saturation, and maximum settled
requested torque 0.53668 N·m. This admitted that experiment, not its walking policy.

The [corrected five-path recording](omni_preview_001/rollout.mp4) completed and
was visually inspected. It uses actual PPO in the control loop. The future navigation
...
passed at that point (288Isaac +27robot/study). The ground-annotation renderer
still awaited actual Isaac recording and inspection at that stage; the corrected
recording described below subsequently completed.
...
The subsequent 700-update continuation completed as part of the total reported
above. Update counts alone cannot qualify it.
...
an independent render in `omni_preview_001`, which subsequently completed and was
visually inspected. That recording used the final checkpoint; training retained
source_v3. Preserve both recordings as dated evidence, and consult STATUS for
current work rather than treating this completed render as queued.
