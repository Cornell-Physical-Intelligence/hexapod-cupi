...
class IntegrityTests(unittest.TestCase):
    def test_frozen_helper_and_source_guard_are_reused(self):
        ...
        with self.assertRaisesRegex(ValueError,'frozen 1600'):e.verify_inputs(wrong,None,None,None,None)
...
class SupervisorTests(unittest.TestCase):
    def run_host(self,primary_pass=True,yield_requested=False):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with ExitStack() as stack:
                for context in (patch.object(host,'verify_inputs',return_value=verified),patch.object(host,'load_module',return_value=guard),
                    ...
                    patch('subprocess.run',return_value=SimpleNamespace(returncode=0)),patch('sys.stdout',new_callable=io.StringIO)):
