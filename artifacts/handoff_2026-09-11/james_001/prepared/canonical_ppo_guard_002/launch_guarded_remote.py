...
PRIOR_BASE=BASE
SOURCE=BASE/'ppo_source_003'
STANDING_SOURCE=BASE/'standing_source_005'
...
PAUSE=BASE/'forecast_pause_017'
UNIT='hexapod-canonical-ppo-smoke-001-20260910.service'
HOST=BASE/'ppo_host_004/launch_ppo_spark.py'
SOURCE_SHA256='970cd1766366c9bdba3239205695e998da80819d2283ce0e261a3b265ca48183'
SUPERVISOR_SHA256='04942c62de1f557d8f117f648f990b6dac69443d61c546529d05a4e2a7f0e01e'
HOST_SHA256='254384b956b602f81a494b57d361cd4858a9382b78ddbcecd488ba1c7d462890'
HOST_FREEZE_SHA256='efd7169161fa7586fd76741f129a66f66119253e510214080e6d4f33628e2130'
PREVIOUS_UNIT='hexapod-canonical-native-standing32-005-20260910.service'
PREVIOUS_INVOCATION='c5397bd5a8dd4640899ad3dec2c30f47'
PREVIOUS_ROOT=BASE/'native_standing32_005'
PREVIOUS_PAUSE=BASE/'forecast_pause_018'
...
STANDING_SOURCE_SHA256='c87f2d340c935cd947c7aed3f3dcd7c9a9965b6e19d6726546abb560b27ab131'
...
MASK_POLICY_SHA256='dcb623201a9ba63d50865ce1bcd527eb7844c0b65b629683582c747516e7d607'
...
def require_final_bindings():
    for name,bound in {'source':SOURCE_SHA256,'standing source':STANDING_SOURCE_SHA256,'supervisor':SUPERVISOR_SHA256,'host':HOST_SHA256,'host freeze':HOST_FREEZE_SHA256,'previous audit':PREVIOUS_AUDIT_SHA256,'standing1 state':STANDING_ONE_STATE_SHA256,'standing1 inventory':STANDING_ONE_INVENTORY_SHA256,'standing32 state':STANDING32_STATE_SHA256,'standing32 inventory':STANDING32_INVENTORY_SHA256,'enabled bindings':BINDINGS_SHA256,'mask/reservation policy':MASK_POLICY_SHA256,**PRIOR_PINS,**RESERVATION_PINS}.items():
...
def verify_previous_owner():
    ...
    if pause.get('unit')!=PREVIOUS_UNIT or pause.get('output')!=str(PREVIOUS_ROOT)or set(restored.get('timers',[]))!=timers or restored.get('restored_unix')!=PREVIOUS_RESTORED_UNIX:raise RuntimeError('Previous exact pause018 restoration missing')
...
def verify_reservation():
    ...
    if policy.get('schema')!='canonical_exclusive_mask_reservation_v2'or policy.get('release_only_on_user_instruction')is not True:raise RuntimeError('Wrong persistent mask policy')
    for name in policy['blocked_entry_directories']:
        p=Path(name)
        if p.is_symlink()or not p.is_dir():raise RuntimeError('Unconditional entry blocker directory missing or replaced: '+name)
        if str(p/'__main__.py')not in policy['files']:raise RuntimeError('Entry blocker __main__ is not bound')
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-canonical-forecast-restore-017', '--on-active=25m',
