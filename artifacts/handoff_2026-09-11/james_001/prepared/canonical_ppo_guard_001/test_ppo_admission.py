...
class PPOAdmissionTests(unittest.TestCase):
    def test_pending_actual32_prevents_main_before_any_external_call(self):
        with patch.object(g.subprocess,'run')as run,patch.object(g.subprocess,'check_output')as call,patch.object(g.os,'open')as opened,patch.object(g.importlib.util,'spec_from_file_location')as imported:
    ...
    def test_actual_frozen_source_host_standing_inventories(self):
        for root,bound in [(REPO/'tmp/canonical_ppo_host_003',g.HOST_FREEZE_SHA256),(REPO/'tmp/canonical_ppo_integration_002',g.SOURCE_SHA256),(REPO/'tmp/updated_native_standing_003',g.STANDING_SOURCE_SHA256)]:
    ...
    def test_reservation_and_embedded_restorer_byte_identical_to_adopted_parent(self):
        ...
        self.assertEqual(old['verify_reservation'],new['verify_reservation']);self.assertEqual(oldrest,newrest)
    ...
    def test_previous32_validation_preserves_parent_checks_with_declared_row_count(self):
        ...
        new=new.replace('standing32','standing1').replace('STANDING32','STANDING_ONE').replace('!=STANDING_SOURCE_SHA256','!=SOURCE_SHA256').replace("identity.get('num_envs')!=32","identity.get('num_envs')!=1").replace("receipt.get('num_envs')!=32","receipt.get('num_envs')!=1").replace('pause010','pause011')
