...
class ActualMainOrderTests(unittest.TestCase):
 def execute(self,fail_at=None):
  ...
  with contextlib.ExitStack() as stack:
   ...
   stack.enter_context(patch.object(g,'validate_ppo_inputs',side_effect=validate))
 ...
 def test_actual_order_fallback_before_timer_stop_two_checks_then_dispatch(self):
  ...
  self.assertIn('--property=RuntimeMaxSec=1320',dispatch);self.assertIn('--property=TimeoutStopSec=180',dispatch)
  self.assertEqual(dispatch[-8:],['--standing-source',str(g.STANDING_SOURCE),'--standing-one',str(g.STANDING_ONE),'--standing32',str(g.STANDING32),'--bindings',str(g.BINDINGS)])
  self.assertEqual(dispatch[dispatch.index('--supervisor-source')+1],str(pause.parent/'source'))
  self.assertNotIn('--num-envs',dispatch)
