...
from test_solver_fixture import write_native_records
...
class PrefixChecks(unittest.TestCase):
 def fixture(self,root):
  ...
  names=write_native_records(root,32,final=False)
  s=SimpleNamespace(n=32,count=8000,captured_count=8000,control=1000,reset_count=1,failure=None,output=root,
    controls=[{'control_index':np.array(i)}for i in range(1000)],files=['substeps_000.npz'],body_names=['body'],names=names,roots=['robot']*32)
  s.flush=lambda:None
  return s
