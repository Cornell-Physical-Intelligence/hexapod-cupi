...
def snapshot(session,destination,score,save):
    ...
    for name in ['initial_reset.json','native_readback.json','contacts.jsonl']:
    ...
    save(dest/'session.json',{'steps':session.count,'controls':session.control,'reset_count':session.reset_count,
         ...
         'all_rows_recorded':session.captured_count==session.count,'scope':'Snapshot before actor construction; original session remains live.'})
