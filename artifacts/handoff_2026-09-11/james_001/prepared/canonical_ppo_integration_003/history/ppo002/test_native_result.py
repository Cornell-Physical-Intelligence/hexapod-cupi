...
class ResultChecks(unittest.TestCase):
 def fixture(self,d):
  identity={'schema':SCHEMA,'CPU_PROTOCOL_FIXTURE_ONLY':True,'policy_lineage':{'CPU_ONLY':True}}
  checks=['sdk_source_bound','usd_identity','native_scene','native_materials_offsets','all_native_inertias_limits_no_drives','all_native_sdf_paths','all_reset_coordinate_frames','fresh_neutral_prefix_admitted','learner_2_updates_strict_reload']
  ...
  write(d/'session.json',{'steps':8384,'captured_steps':8384,'controls':1048,'reset_count':1,'failure':None,'all_rows_recorded':True,'substep_files':['substeps_000.npz']})
  write(d/'neutral_prefix/standing_report.json',{'all_pass':True,'num_envs':32,'replicas':[{'pass':True}]*32})
  write(d/'neutral_prefix/SHA256.json',{'standing_report.json':sha(d/'neutral_prefix/standing_report.json')})
  updates=[]
  for i in [1,2]:
