"""Exact source009 run_owned adaptation for canonical PPO smoke only.
...
SCHEMA = 'canonical_ppo_supervisor_deadline_v1'
SUPERVISOR_SHA256 = '9ebabaf254cc77fabad7d4759cc80fc26d7efd3912ea84bd55304594c88ed561'
PHASES = ('canonical_ppo_smoke',)
HELPER_NAME = '_canonical_ppo_phase_deadline_seconds'
SEAMS = (
    ('deadline_seconds=600', 'deadline_seconds=_canonical_ppo_phase_deadline_seconds(args, phase)'),
    ('deadline = time.monotonic() + 600', 'deadline = time.monotonic() + _canonical_ppo_phase_deadline_seconds(args, phase)'),
    ('raise TimeoutError("Standing phase exceeded ten-minute bound")',
     'raise TimeoutError(f"{phase} phase exceeded {_canonical_ppo_phase_deadline_seconds(args, phase)}-second bound")'),
)


def phase_deadline_seconds(args, phase):
    if phase not in PHASES:
        raise ValueError('Deadline adapter is restricted to the fixed canonical_ppo_smoke phase')
    return 1200
