...
SOURCE=Path(os.environ.get('HEXAPOD_STANDING_SOURCE',str(ROOT/'tmp/updated_native_standing_005'))).resolve()
...
def load():
    assert sha(SOURCE/'FREEZE_SHA256.json')=='c87f2d340c935cd947c7aed3f3dcd7c9a9965b6e19d6726546abb560b27ab131'
    expected=json.loads((ROOT/'tmp/canonical_standing_contact_cost_001/INPUT_BINDINGS.json').read_text())
    for name,want in expected['measured_inputs'].items():assert sha(ACTUAL/name)==want,name
