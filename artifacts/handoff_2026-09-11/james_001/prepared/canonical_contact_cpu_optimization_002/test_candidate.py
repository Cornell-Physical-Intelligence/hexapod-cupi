...
class CandidateTests(unittest.TestCase):
 ...
 def long_fixture(self,n=1,dtype=np.float32):
  original,mapping,poses=self.fixture(n,dtype)
  values=[np.zeros_like(x)for x in original[:4]]
  counts=np.full((19*n,1),40,np.uint32);starts=np.zeros_like(counts)
  for sensor in range(19*n):
   old=int(original[5][sensor,0]);start=sensor*40;starts[sensor]=start
   for field in range(4):values[field][start:start+40]=np.tile(original[field][old:old+4],(10,1))
  return values+[counts,starts],mapping,poses
 def test_repeated_patches_keep_ordered_float32_products_and_float64_sums(self):
  for dtype in [np.float32,np.float64]:
   d,m,p=self.long_fixture(2,dtype)
   for sensor in range(38):
    k=int(d[5][sensor,0]);d[1][k:k+4]=d[1][k];d[2][k:k+4]=[0.,0.,1.]
    d[0][k:k+4,0]=[1e8,.6,-1e8,.6]
   equal(old.classify_contacts(d,m,p,self.geometry,2),new.classify_contacts(d,m,p,self.geometry,2))
 def test_identical_inactive_points_and_signed_zero_are_not_coalesced_in_output(self):
  d,m,p=self.long_fixture(1)
  for sensor in range(19):
   k=int(d[5][sensor,0]);d[0][k:k+40]=0.;d[1][k:k+40]=d[1][k];d[2][k:k+40]=0.
   d[2][k+1,1]=-0.;d[3][k+2,0]=-0.;d[1][k+3,2]=-0.
  a=old.classify_contacts(d,m,p,self.geometry,1);b=new.classify_contacts(d,m,p,self.geometry,1);equal(a,b)
  self.assertEqual(len(b['patches']),760)
  self.assertTrue(all(x['inactive_zero_normal']for x in b['patches']))
 def test_invalid_duplicate_after_valid_prefix_and_unused_nonfinite_slots(self):
  for field,index,value in [(0,(3,0),np.nan),(1,(3,1),np.inf),(2,(3,0),np.nan),(3,(3,0),1e-9)]:
   d,m,p=self.long_fixture(1);d[0][:40]=0.;d[1][:40]=d[1][0];d[2][:40]=0.;d[3][:40]=0.;d[field][index]=value
   errors=[]
   for fn in [old.classify_contacts,new.classify_contacts]:
    with self.assertRaises(ValueError)as error:fn(d,m,p,self.geometry,1)
    errors.append(str(error.exception))
   self.assertEqual(errors[0],errors[1])
  d,m,p=self.fixture(1)
  for field in range(4):d[field][-1]=np.nan
  equal(old.classify_contacts(d,m,p,self.geometry,1),new.classify_contacts(d,m,p,self.geometry,1))
