...
def classify_contacts(data,sensor_map,poses,geometry,num_envs):
 ...
 for i,(e,body)in enumerate(sensor_map):
  ...
  # Small ranges use the exact scalar path; hashing costs more without repetition.
  patch_cache={}if count>=32 else None
  for k in range(start,start+count):
   ...
   # Cache only interpretation of identical raw bytes within this sensor range.
   # Aggregation remains in original patch order with original float32 products.
   cached=None
   if patch_cache is not None:
    patch_key=(force[k,0].tobytes(),p.tobytes(),n.tobytes(),sep[k,0].tobytes())
   ...
   if cached is None:
    ...
    if patch_cache is not None:patch_cache[patch_key]=(inactive_zero_normal,local,category)
   else:inactive_zero_normal,local,category=cached
