...
SOURCE=REPO/'tmp/canonical_ppo_integration_003'
class HostTests(unittest.TestCase):
    ...
    def identity(self,args):
        ...
        return {'schema':h.SCHEMA,'phase':h.PHASE,'runtime_binding':{'runtime_tree_sha256':h.SOURCE_FREEZE,'scope':'fresh_canonical_405_408_two_update_smoke'},
            ...
            'training_integration_only':True,'quality_admitted':False,'Stage2_complete':False,'bindings_sha256':'a'*64,'protocol':protocol,'solver_recipe':{'position_iterations':32,'velocity_iterations':0},
            'native_identity':{'solver_recipe':{'position_iterations':32,'velocity_iterations':0},'runtime_binding':{'runtime_tree_sha256':h.STANDING_FREEZE,'scope':'canonical_provisional_native_standing_only'},
            ...
            'policy_lineage':{'consumer_source_freeze_sha256':h.SOURCE_FREEZE,'standing_source_freeze_sha256':h.STANDING_FREEZE,'fresh_checkpoint_only':True,
                **{k:'a'*64 for k in ['standing1_state_sha256','standing32_state_sha256','servo_sha256','geometry_sha256','adapter_sha256','rsl_source_map_sha256']},'standing1_state_sha256':h.STANDING_ONE_STATE}}
    ...
    def test_exact_owner_and_source_inventories(self):
        ...
        h.verify_tree(REPO/'tmp/updated_native_standing_005','FREEZE_SHA256.json',h.STANDING_FREEZE)
    ...
    def test_fixed_identity_and_missing_admissions_reject(self):
        ...
        with patch.object(h,'sha',return_value='a'*64):
            ...
            for target,key,value in [('protocol','updates',50),('protocol','replicas',1024),('protocol','checkpoint_input','old.pt'),('protocol','auto_reset',True),('protocol','actor_width',315),('native_identity','num_envs',1),('native_identity','ground',False),('policy_lineage','standing32_state_sha256',None),('policy_lineage','standing1_state_sha256','a'*64),('native_identity','solver_recipe',{'position_iterations':32,'velocity_iterations':4}),('policy_lineage','fresh_checkpoint_only',False)]:
            ...
            for key,value in [('total_substeps',8000),('quality_admitted',True),('Stage2_complete',True),('schema','legacy'),('solver_recipe',{'position_iterations':32,'velocity_iterations':1})]:
    ...
    def test_real_contract_template_remains_disabled_without_process(self):
        ...
        a=self.args(Path('/unused'));a.standing_source=REPO/'tmp/updated_native_standing_005';a.bindings=SOURCE/'BINDINGS.json'
        with patch.object(subprocess,'Popen',side_effect=AssertionError('No subprocess')),patch.object(subprocess,'run',side_effect=AssertionError('No subprocess')):
            with self.assertRaisesRegex(ValueError,'standing32_state_sha256'):contract.verify_inputs(h.native_args(a))
    ...
    def test_original_supervisor_codetypes_and_truthful_metadata(self):
        ...
        with patch.object(sys,'path',list(sys.path)):
            ...
            self.assertNotEqual(old.run_owned.__code__,new.run_owned.__code__)
            for name in ('owned_container',):
    ...
    def test_exact_deadlines_and_appready_remain(self):
        ...
        self.assertEqual(h.load_deadline_adapter().phase_deadline_seconds(NS(),h.PHASE),1200)
