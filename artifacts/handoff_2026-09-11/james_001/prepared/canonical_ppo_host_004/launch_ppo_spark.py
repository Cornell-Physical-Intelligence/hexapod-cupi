...
SOURCE_FREEZE='970cd1766366c9bdba3239205695e998da80819d2283ce0e261a3b265ca48183'
STANDING_FREEZE='c87f2d340c935cd947c7aed3f3dcd7c9a9965b6e19d6726546abb560b27ab131'
STANDING_ONE_STATE='33920a4af296ed124643fe61a16f4840c19949b5f6e434327d7cd7ea9953cd3f'
...
EXPECTED_COORDINATION='649ccda1bdef20bc967ae78f68a992cc8001f4ed7009f78cab5133cc67b29d8f'
...
def validate_identity(identity,args):
    ...
    if identity.get('solver_recipe')!={'position_iterations':32,'velocity_iterations':0}or native.get('solver_recipe')!={'position_iterations':32,'velocity_iterations':0}:raise ValueError('Wrong source00532/0 solver declaration')
    if lineage.get('standing1_state_sha256')!=STANDING_ONE_STATE:raise ValueError('Wrong exact accepted native6 standing1')
...
def load_deadline_adapter():
    return load_module('_canonical_ppo_deadline_adapter',Path(__file__).resolve().parent/'deadline_adapter.py')
...
def load_supervisor(args,identity):
    ...
    deadline_proof=load_deadline_adapter().install(PARENT,args.supervisor_source/'tools/launch_reference_physics_spark.py')
    ...
    def metadata_save(path,data):
        if Path(path).parent.name=='jobs'and Path(path).name==PHASE+'.json':
            data={**data,'supervisor_runtime_adapter':deadline_proof,'no_policy_loaded':False,'no_checkpoint_loaded':True,'checkpoint_input':None,
