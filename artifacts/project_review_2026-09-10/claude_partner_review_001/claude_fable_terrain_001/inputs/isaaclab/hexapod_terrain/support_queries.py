...
class TerrainSupportQueries:
    ...
    def query_local(self, xy, fixture_indices, *, ray_origin_z_m=2., boundary_margin_m=0.):
        ...
        shape = xy.shape[:-1]
        fixture_input = torch.as_tensor(fixture_indices, device=self.device)
        if fixture_input.dtype not in (torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8):
            raise ValueError("Fixture indices must be integers, never rounded from floating point")
        fixture = _expand_prefix(fixture_input, shape, device=self.device, dtype=torch.long).reshape(-1)
