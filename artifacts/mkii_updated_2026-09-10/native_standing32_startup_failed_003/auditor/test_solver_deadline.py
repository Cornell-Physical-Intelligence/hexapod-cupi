...
class NewChecks(unittest.TestCase):
 ...
 def test_complete_solver_evidence_strict_all_phases_roots_and_flags(self):
  identity={'num_envs':32,'solver_recipe':{'position_iterations':32,'velocity_iterations':0}};state={'checks':{'solver_recipe_readback':True}}
  r={p:{root:{'position_iterations':32,'velocity_iterations':1 if p=='before' else 0,'authored':[p!='before']*2}for root in ({'/Robot'}|{f'/Robot_{i:03d}'for i in range(1,32)})}for p in ('before','after_authoring','after_reset','after_controlled_steps')}
  ...
  for phase in r:
   ...
   with self.assertRaises(ValueError):a.solver_check(bad,identity,state)
   bad=copy.deepcopy(r);bad[phase]['/Robot_999']=bad[phase]['/Robot']
   with self.assertRaises(ValueError):a.solver_check(bad,identity,state)
 ...
 def test_actual_frozen_deadline_metadata_and_wrong_budget_reject(self):
  path=HERE.parent/'canonical_native_standing_host_007/deadline_adapter.py';spec=importlib.util.spec_from_file_location('deadline',path);d=importlib.util.module_from_spec(spec);spec.loader.exec_module(d)
