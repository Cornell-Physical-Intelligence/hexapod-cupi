...
class GuardTests(unittest.TestCase):
 ...
 def test_unique_namespace_and_fixed_bounds(self):
  self.assertEqual(g.OUTPUT.name,'native_standing32_003');self.assertEqual(g.PAUSE.name,'forecast_pause_015');self.assertEqual(g.ASSET.name,'asset_001')
  t=(HERE/'launch_guarded_remote.py').read_text()
  for x in ('hexapod-canonical-forecast-restore-015','RuntimeMaxSec=1320','TimeoutStopSec=180','--on-active=25m','/opt/wx/gpu.lock','/tmp/hexapod-isaac-gpu.lock'):self.assertIn(x,t)
