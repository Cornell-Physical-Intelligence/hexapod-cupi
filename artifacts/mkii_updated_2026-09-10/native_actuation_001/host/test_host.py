...
s=importlib.util.spec_from_file_location('canonical_host',HERE/'launch_actuation_spark.py');h=importlib.util.module_from_spec(s);s.loader.exec_module(h)
class HostTests(unittest.TestCase):
 def args(self,r):return NS(source=r/'source',asset=r/'asset',admission=r/'admission',supervisor_source=r/'supervisor',output=r/'output',host_freeze_sha256='a'*64,isaaclab=r/'isaaclab')
 def identity(self):return {'schema':'canonical_native_actuation_v1','inspector_freeze_sha256':'b'*64,'runtime_binding':{'runtime_tree_sha256':'b'*64,'scope':'canonical_native_coordinate_effort_only'},'training_allowed':False,'physical_admission':False,'steps':2064,'import_prefix_steps':8,'dt':.0025,'gravity':[0.,0.,0.],'phase_a_state_sha256':h.ADMISSION_STATE,'diagnostic':{'baseline_steps':40,'pulse_steps':8,'coast_steps':8,'pulse_nm':.005,'cases':36,'experiment_steps':2056,'coordinate_rad':.01,'bound_nm':.1,'bound_joint_excursion_rad':.05,'bound_joint_rate_rad_s':2.,'gravity':[0.,0.,0.],'ground':False,'position_drives':False,'external_forces_every_iteration':True}}
 ...
 def test_stdlib_import(self):
  subprocess.run([sys.executable,'-S','-B','-c',"import launch_actuation_spark,sys;assert not any(k in sys.modules for k in ('torch','numpy','isaaclab'))"],cwd=HERE,check=True,capture_output=True)
 def test_only_actuation_no_checkpoint_asset_and_source_readonly(self):
  args=self.args(Path('/a'));c=h.command(args,'exact-owned','actuation');mounts=[c[i+1] for i,v in enumerate(c) if v=='-v']
  self.assertEqual(mounts,['/a/output:/output:rw','/a/source:/actuation:ro','/a/asset:/asset:ro','/a/admission:/admission:ro'])
  self.assertIn('/actuation/run_actuation.py',c);self.assertNotIn('--checkpoint',c);self.assertNotIn('--num-envs',c)
  for phase in ('standing','train','recording','wave'):
 ...
 def test_real_contract_identity_shape_and_phase_only(self):
  a=self.args(Path('/a'));i=self.identity();m=NS(__file__=str(a.source/'actuation_contract.py'),verify_inputs=lambda args:i)
  ...
  with patch.object(h,'SOURCE_FREEZE','b'*64),patch.object(h,'verify_own_bundle'),patch.object(h,'verify_tree'),patch.object(h,'sha',side_effect=digest),patch.object(h,'CONTRACT',m):
   ...
   for key,value in [('training_allowed',True),('physical_admission',True),('steps',1000),('schema','old_C_study'),('phase_a_state_sha256','f'*64)]:
    before=i[key];i[key]=value
    with self.assertRaises(ValueError):h.verify_inputs(a)
    i[key]=before
 def test_runtime_old_C_identity_rejects(self):
  self.assertNotIn('abe4e354',(HERE/'launch_actuation_spark.py').read_text())
  self.assertIn("identity.get('runtime_binding')!={'runtime_tree_sha256':SOURCE_FREEZE,'scope':'canonical_native_coordinate_effort_only'}",(HERE/'launch_actuation_spark.py').read_text())
 def test_parent_functions_untouched_and_canonical_verifier_installed(self):
  ...
  with patch.object(h,'load_module',return_value=parent),patch.object(sys,'path',list(sys.path)),patch.object(h,'verify_tree') as verify:
 ...
 def test_failed_actuation_preserved_and_terminal_checked(self):
  with tempfile.TemporaryDirectory() as t:
 ...
 def test_success_requires_native_contract_and_post_exit_input_recheck(self):
  with tempfile.TemporaryDirectory() as t:
   a=self.args(Path(t));a.output.mkdir();i=self.identity();result={'phase':'actuation','status':'completed','physical_admission':False}
   ...
   with patch.object(h,'PARENT',NS(run_owned=Mock())),patch.object(h,'CONTRACT',c),patch.object(h,'verify_inputs',return_value=i) as verify:
    d=h.run_campaign(a,i);self.assertEqual(verify.call_count,2);c.validate_result.assert_called_once_with(a.output/'actuation',i)
   self.assertEqual(d['status'],'completed');self.assertTrue(d['post_exit_original_inputs_reverified']);self.assertFalse(d['training_allowed']);self.assertTrue(d['post_exit_all_actuation_payloads_inventoried']);self.assertEqual(h.read(a.output/'actuation_immutable.sha256.json'),{})
