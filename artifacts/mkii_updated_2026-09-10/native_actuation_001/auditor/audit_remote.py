...
RUN=BASE/'native_actuation_001'; PAUSE=BASE/'forecast_pause_004'
UNIT='hexapod-canonical-native-actuation-001-20260910.service'
INV='c6d287881452428d90e8e432ae739c86'
SOURCE=BASE/'actuation_source_002'; HOST=BASE/'actuation_host_001'; GUARD=BASE/'actuation_guard_001'; ASSET=BASE/'asset_001'
...
PINS=[('source',SOURCE,'FREEZE_SHA256.json','1f02009cb22c1efbe4a0d0c7f7988d9d63b658d78e509ea444b7afafee6178c9',38),
 ('host',HOST,'FREEZE_SHA256.json','0f9a54963ec3220d0625722e588bb17c3efd534b0c5ac04c1f482db69511163e',19),
 ('guard',GUARD,'FREEZE_SHA256.json','9aef608cf860f32101a4aaf494d8d211adc47e81123418b423fc082a28c3b9e4',40),
...
def owned_absence(job):
 require(job.get('phase')=='actuation' and job.get('cleanup_checked') is True,'Owned cleanup not verified')
...
def expected_launch_command():
 return ['systemd-run','--user','--unit='+UNIT,'--property=RuntimeMaxSec=720',
  ...
  '/usr/bin/python3',str(HOST/'launch_actuation_spark.py'),'--source',str(SOURCE),'--asset',str(ASSET),'--admission',str(ADMISSION),
...
def classify(campaign,job,state,unit,native_validation):
 ...
 require(campaign.get('planned_phases')==['actuation'],'Unexpected allocated phase')
 ...
 if campaign['status']=='completed':
  ...
  require(campaign.get('actuation')==native_validation.get('receipt'),'Native completed receipt mismatch')
  require(campaign.get('terminal_inputs_unchanged') is True and campaign.get('post_exit_original_inputs_reverified') is True and campaign.get('post_exit_all_actuation_payloads_inventoried') is True,'Missing completed post-exit integrity')
  return 'authentic_completed_actuation'
...
def audit():
 result={'schema':'canonical_native_actuation_terminal_audit_v1','expected_invocation':INV,'unit_name':UNIT,'checked_unix':time.time(),'read_only':True,'audit_verified':False,'actuation_completed':False,'physical_admission':False,'training_allowed':False,'errors':[]}
 ...
 if all(checks.values()):
  ...
  def admissioncheck():
   actual=inventory(ADMISSION);require(len(actual)==13 and {k:v['sha256'] for k,v in actual.items()}==read(SOURCE/'PHASE_A_SHA256.json'),'Original Phase A admission changed');require(sha(SOURCE/'PHASE_A_SHA256.json')=='3905a714dce4055d5f85d2952584b2d42a236f65d144cd51ef9d32fda41fafe5','Wrong Phase A map');return actual
  attempt('phase_a_admission_inventory',admissioncheck)
 campaign=attempt('campaign',lambda:read(RUN/'campaign.json'))
 job=attempt('job',lambda:read(RUN/'jobs/actuation.json'))
 state=attempt('native_state',lambda:read(RUN/'actuation/state.json')) if (RUN/'actuation/state.json').is_file() else None
 result['native_state_available']=state is not None
 if job is not None:attempt('owned_absence',lambda:owned_absence(job))
 def phase_inventory():
  actual={p.name for p in (RUN/'jobs').glob('*.json') if not p.name.endswith('_contact_data_audit.json')};require(actual=={'actuation.json'},'Unexpected or missing allocated jobs');return sorted(actual)
 ...
 if all(checks.values()) and result.get('input_asset'):
  def hostidentity():
   s=importlib.util.spec_from_file_location('_audit_canonical_host',HOST/'launch_actuation_spark.py');h=importlib.util.module_from_spec(s);s.loader.exec_module(h)
   args=SimpleNamespace(source=SOURCE,asset=ASSET,admission=ADMISSION,supervisor_source=SUPERVISOR,output=RUN,host_freeze_sha256=PINS[1][3])
   ...
   if state is not None and state.get('status')=='completed':
    ...
    try:native_validation.update(passed=True,receipt=h.CONTRACT.validate_result(RUN/'actuation',identity))
 ...
 if campaign is not None and job is not None:
  ...
  result['actuation_completed']=outcome=='authentic_completed_actuation'
  if result['actuation_completed']:
   def seal():
    p=RUN/'actuation_immutable.sha256.json';require(sha(p)==campaign.get('actuation_payload_manifest_sha256'),'Wrong post-exit manifest');require(read(p)=={k:v['sha256'] for k,v in inventory(RUN/'actuation').items()},'Post-exit actuation payload changed');return {'passed':True,'manifest_sha256':sha(p)}
 ...
 if all(checks.values()):
  ...
  final_admission=attempt('final_phase_a_admission_inventory',admissioncheck)
  if final_admission is not None and final_admission!=result.get('phase_a_admission_inventory'):result['errors'].append({'check':'phase_a_stability','error':'Original Phase A admission changed between audit passes'})
  result.pop('final_phase_a_admission_inventory',None)

 result['audit_verified']=not result['errors']
 if not result['audit_verified']:result['actuation_completed']=False
 return result
