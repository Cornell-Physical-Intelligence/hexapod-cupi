...
class AuditTests(unittest.TestCase):
 ...
 def test_failed_outcome_is_authentic_not_completed(self):
  c={'status':'failed','planned_phases':['actuation'],'training_allowed':False,'physical_admission':False,'error':'Missing installed API source'}
 ...
 def test_completed_needs_native_postexit_receipt_and_zero_exit(self):
  receipt={'scope':'passive_import_cooking_actuation_only'}
  c={'status':'completed','planned_phases':['actuation'],'training_allowed':False,'physical_admission':False,'actuation':receipt,'terminal_inputs_unchanged':True,'post_exit_original_inputs_reverified':True,'post_exit_all_actuation_payloads_inventoried':True}
  job={'status':'completed','exit_code':0,'container_id':'a'*64};unit={'ExecMainStatus':'0','Result':'success'};state={'status':'completed'}
  self.assertEqual(a.classify(c,job,state,unit,{'passed':True,'receipt':receipt}),'authentic_completed_actuation')
 ...
 def test_exact_two_absences_unknown_docker_rejects_no_fabricated_id(self):
  job={'phase':'actuation','cleanup_checked':True,'container_name':'hexapod-reference-physics-'+'a'*32,'container_id':'b'*64}
