#!/usr/bin/env python3
"""Canonical bounded coordinates/external-effort diagnostic. No servo, standing or learning."""
...
from actuation_contract import DT, STEPS, SCHEMA, read, sha, verify_inputs
...
def main(argv=None):
    ...
    output=args.output; app=None; rows=[]; experiment=None
    ...
    try:
        ...
        cfg=SimulationCfg(dt=DT,gravity=(0.,0.,0.),device=args.device,render_interval=8,
                          physics=PhysxCfg(enable_external_forces_every_iteration=True),visualizer_cfgs=[],log_dir=str(output/'isaac_logs'))
        ...
        state['checks']['external_force_setting']=scene['attributes'].get('physxScene:enableExternalForcesEveryIteration') is True
        if not state['checks']['external_force_setting']:raise ValueError('Explicit Phase B external-force setting not retained')
        ...
        from effort_experiment import Experiment
        experiment=Experiment(view,sim,model,native,output,snapshot,save,to_list,errors,args.device)
        result=experiment.run()
        state['explicit_steps_completed']=STEPS+result['step_count']
        state['checks']['coordinate_effort_cases']=result['status']=='completed'
        ...
        print('CANONICAL_NATIVE_COORDINATE_EFFORT_COMPLETED physical_admission=false',flush=True)
    except BaseException as e:
        state['status']='failed';state['errors'].append(repr(e))
        if experiment is not None:
            state['explicit_steps_completed']=STEPS+experiment.sim.get_physics_step_count()-experiment.initial_counter
