...
with patch.object(subprocess,'run',side_effect=forbidden),patch.object(subprocess,'Popen',side_effect=forbidden),patch.object(subprocess,'check_output',side_effect=forbidden):
 ...
 hostpath=root/'tmp/canonical_native_actuation_host_001/launch_actuation_spark.py'
 host=load('_bound_canonical_host',hostpath)
 with patch.multiple(guard,HOST=hostpath,SOURCE=root/'tmp/updated_native_actuation_002',ADMISSION=root/'tmp/canonical_native_inspection_terminal_003/run/inspection',ASSET=root/'artifacts/mkii_updated_2026-09-10/usd_002/rs05_mass_corrected',SUPERVISOR_SOURCE=root/'tmp/reference_physics_adapter_009/source_009',OUTPUT=root/'tmp/canonical_actuation_guard_uncreated_preflight_001'):
  identity=guard.validate_actuation_inputs(host)
