...
class Evidence(unittest.TestCase):
 ...
 def test_complete_flag_cannot_hide_numeric_quiet_failure_or_late_native_error(self):
  with tempfile.TemporaryDirectory()as x:
   ...
   after={'/Robot':{'position_iterations':32,'velocity_iterations':4,'authored':[True,True]}}
   ...
   identity={'num_envs':1};state={'schema':SCHEMA,'identity':identity,'status':'completed','inputs_unchanged':True,'errors':[],'native_error_events':[],'physical_admission':False,'physics_admitted':False,'training_allowed':False,'explicit_steps_completed':8000,'checks':{'test':True,'solver_recipe_readback':True}}
   def seal():
   ...
   solver['after_reset']['/Robot']['velocity_iterations']=4;save(d/'solver_readback.json',solver);seal()
