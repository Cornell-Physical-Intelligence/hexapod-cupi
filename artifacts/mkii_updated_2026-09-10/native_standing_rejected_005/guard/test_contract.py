...
class GuardTests(unittest.TestCase):
 ...
 def actual_prior(self,unit=None,docker=None):
  ...
  with patch.object(Path,'read_text',read),patch.object(Path,'glob',glob),patch.object(g,'sha',side_effect=localsha),patch.object(g,'call',return_value=unit or 'ActiveState=failed\nMainPID=0\nExecMainStatus=1\nInvocationID='+g.PREVIOUS_INVOCATION),patch.object(g.subprocess,'run',return_value=docker or subprocess.CompletedProcess([],1,'','No such object')) as run:
   yield run
 def test_exact_thirty_six_previous_payload_pins_and_audit(self):
  d=json.loads((HERE/'inputs/previous_terminal_audit.json').read_text());self.assertTrue(d['audit_verified']);self.assertFalse(d['standing_completed']);self.assertEqual(len(g.PRIOR_PINS),36)
 ...
 def test_unique_namespace_and_fixed_bounds(self):
  self.assertEqual(g.OUTPUT.name,'native_standing_005');self.assertEqual(g.PAUSE.name,'forecast_pause_013');self.assertEqual(g.ASSET.name,'asset_001')
  t=(HERE/'launch_guarded_remote.py').read_text()
  for x in ('hexapod-canonical-forecast-restore-013','RuntimeMaxSec=1320','TimeoutStopSec=180','--on-active=25m','/opt/wx/gpu.lock','/tmp/hexapod-isaac-gpu.lock'):self.assertIn(x,t)
