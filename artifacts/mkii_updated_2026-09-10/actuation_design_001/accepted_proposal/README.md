...
```sh
PYTHONDONTWRITEBYTECODE=1 .venv/bin/python3 -m unittest discover -s tmp/updated_native_phase_b_design_002 -p 'test_*.py'
python3 -S tmp/updated_native_phase_b_design_002/verify_bundle.py
```

Eleven tests pass. They independently check mass/energy/gravity derivatives, static world force/moment equilibrium, supported joint torque by virtual work, numerical free-response decay, and separate demand/ceiling/applied/power behavior. They do not run Isaac. `tests_corrected.log` is the final test receipt; earlier `tests.log` is retained. `servo_candidate.py`, `stance_probe.py` and `inertia_probe.py` refuse to overwrite their reports.
...
Draft001 is retained at its original freeze d51ccac48427286bb3b74e47a6f6b92daba4c3fec0a2881afa3960b3456c8b0a. Root caught an ideal pulse/coast excursion conflict before adoption. This successor proves that old setting exceeds.05rad and that .005Nm20ms+20ms coast predicts.008968rad maximum with fullfloatingcoupling, keepingboundsunchanged.
