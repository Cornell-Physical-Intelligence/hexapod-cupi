...
class NewChecks(unittest.TestCase):
 ...
 def test_complete_solver_evidence_strict_all_phases_roots_and_flags(self):
  identity={'num_envs':1,'solver_recipe':{'position_iterations':32,'velocity_iterations':0}};state={'checks':{'solver_recipe_readback':True}}
  r={p:{'/Robot':{'position_iterations':32,'velocity_iterations':1 if p=='before' else 0,'authored':[p!='before']*2}}for p in ('before','after_authoring','after_reset','after_controlled_steps')}
