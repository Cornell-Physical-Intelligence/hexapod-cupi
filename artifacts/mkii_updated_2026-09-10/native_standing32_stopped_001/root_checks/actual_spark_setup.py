...
with patch.object(subprocess, 'run', side_effect=forbid), patch.object(subprocess, 'Popen', side_effect=forbid), patch.object(subprocess, 'check_output', side_effect=forbid):
    host = load('_root_actual_host', base/'standing_host_002/launch_standing_spark.py')
    args = SimpleNamespace(source=base/'standing_source_002', asset=base/'asset_001', admission=base/'native_actuation_001/actuation', num_envs=32, standing_one=base/'native_standing_002/standing',
        supervisor_source=prior/'reference_physics_source_009', output=base/'native_standing32_001',
    ...
    assert str(args.standing_one)+':/standing_one:ro' in command
