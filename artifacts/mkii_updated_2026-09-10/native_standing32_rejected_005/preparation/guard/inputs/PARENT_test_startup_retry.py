...
s=importlib.util.spec_from_file_location('startup_guard',HERE/'launch_guarded_remote.py');g=importlib.util.module_from_spec(s);s.loader.exec_module(g)
class StartupRetryTests(unittest.TestCase):
 @contextlib.contextmanager
 def actual(self,unit=None,docker=None,state_patch=None):
  base=HERE/'inputs/startup_failure';orig_read=Path.read_text;orig_glob=Path.glob;sha=g.sha
  def local(p):
  ...
  with patch.object(Path,'read_text',rd),patch.object(Path,'glob',glob),patch.object(g,'sha',side_effect=lambda p:sha(local(p))),patch.object(g,'call',return_value=unit or 'ActiveState=failed\nMainPID=0\nExecMainStatus=1\nInvocationID='+g.RETRY_INVOCATION),patch.object(g.subprocess,'run',return_value=docker or subprocess.CompletedProcess([],1,'','No such object'))as run:yield run
 def test_exact_actual_startup_failure_and_two_absences(self):
  ...
  self.assertEqual(len(g.RETRY_RAW_PINS),9);self.assertFalse(any('traceback' in k for k in g.RETRY_RAW_PINS));self.assertFalse(any('substeps' in k for k in g.RETRY_RAW_PINS))
