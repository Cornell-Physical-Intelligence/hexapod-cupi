...
class Reservation(unittest.TestCase):
 def fixture(self):
  ...
  worker=base/'worker.py';worker.write_text('raise SystemExit("deferred")\n')
  ...
  policy={'schema':'canonical_exclusive_mask_reservation_v1','release_only_on_user_instruction':True,'files':{str(p):digest(p)for p in (worker,helper,unit,before)},'mask_paths':masks,'system_masked_units':system,'original_units_before':str(before),'original_units_backup':str(backup),'queue_unit':'queue.service','queue_unit_path':str(unit),'queue_helper':str(helper),'queue_lock_path':str(lock),'queue_lock_record':str(record),'marker_sha256':digest(marker)}
  file=base/'inputs/reservation_policy.json';file.write_text(json.dumps(policy))
  return dict(base=base,reservation=reservation,original=original,worker=worker,backup=backup,proc=proc,states=states,policy=policy,file=file,record=record,masks=masks)
 ...
 def test_exact_policy_matches_root_receipts_and_original_reservation_files(self):
  ...
  for p,h in worker['files'].items():self.assertEqual(policy['files'][p],h)
