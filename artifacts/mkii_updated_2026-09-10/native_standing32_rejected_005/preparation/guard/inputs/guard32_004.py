...
OUTPUT=BASE/'native_standing32_004'
PAUSE=BASE/'forecast_pause_016'
UNIT='hexapod-canonical-native-standing32-004-20260910.service'
...
RETRY_UNIT='hexapod-canonical-native-standing32-003-20260910.service'
RETRY_INVOCATION='6eac4972e2e64f9991955a1b7bb73afc'
RETRY_AUDIT_SHA256='ba384bbc04558744254d5180a32c723a0debcf96817265ad84b648bd509ba6c8'
RETRY_NAME='hexapod-reference-physics-5e6f8b80923e42d1bc7f318d0c2bc294'
RETRY_ID='3f8fc2709874679d70bac5db2e73477ee25b6d81ea896ecd1c1e76addb00af89'
RETRY_ERROR="TimeoutError('No AppReady by90s; preserve startup log and45s traceback')"
RETRY_RESTORED_UNIX=1789085110.9715965
RETRY_RAW_PINS={'native_standing32_003/campaign.json': '54c06742dd7ffd51f68adfd94a43f3fb6fa94cc3c216f1dbc0ead42f60960917', 'native_standing32_003/jobs/standing.json': 'bdb6b8717424c01242881da179ccaca502800c7c63fb0351e928fbca0d290c0f', 'native_standing32_003/logs/standing.log': 'd9f40ba31aa3169c0e51a55b21bee9cb761d72072eb7155dbe02e5f7a096f8b8', 'native_standing32_003/standing/state.json': '9e34cb925a208d67f1cc3c23f61b46a1c988bb788c6169f382ba3dad85789c0c', 'forecast_pause_015/launch.json': 'cddef4999acd6ff188305994ea2d5b1177ce336fd663393aa217911e5adc1ce4', 'forecast_pause_015/pause.json': '4edc0cd9be109b99ee466258758c44ee9e84567292a05f4e08f4969d05c1dd37', 'forecast_pause_015/restore.lock': 'e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855', 'forecast_pause_015/restored.json': 'fa8afefc4f8dd432a4e7bfac63baccd4bb419734ef9923bdbe161c784bbe2016', 'forecast_pause_015/resume_forecasting.py': 'daffe2b2397c31b37145b193c29c947c4162123799365323f27acda0218a5d90'}
RETRY_ROOT=BASE/'native_standing32_003'
RETRY_PAUSE=BASE/'forecast_pause_015'
...
def verify_retry_cleanup():
    audit_path=Path(__file__).resolve().parent/'inputs/startup_failure_audit.json'
    if sha(audit_path)!=RETRY_AUDIT_SHA256:raise RuntimeError('Startup failure audit changed')
    audit=json.loads(audit_path.read_text())
    if audit.get('audit_verified')is not True or audit.get('errors')!=[]or audit.get('terminal_outcome')!='authentic_terminal_failure'or audit.get('standing_completed')is not False or audit.get('expected_invocation')!=RETRY_INVOCATION:raise RuntimeError('Startup failure provenance changed')
    ...
    if fields.get('ActiveState')!='failed'or fields.get('MainPID')!='0'or fields.get('ExecMainStatus')!='1'or fields.get('InvocationID')not in ('',RETRY_INVOCATION):raise RuntimeError('Previous startup owner is not the exact terminal failure')
    for rel,bound in RETRY_RAW_PINS.items():
        if sha(BASE/rel)!=bound:raise RuntimeError('Startup failure raw changed: '+rel)
    ...
    if campaign.get('status')!='failed'or campaign.get('planned_phases')!=['standing']or campaign.get('error')!=RETRY_ERROR or campaign.get('terminal_inputs_unchanged')is not True or campaign.get('host_freeze_sha256')!=HOST_FREEZE_SHA256 or campaign.get('source_freeze_sha256')!=SOURCE_SHA256:raise RuntimeError('Startup failed campaign differs')
    if identity.get('num_envs')!=32 or identity.get('inspector_freeze_sha256')!=SOURCE_SHA256 or identity.get('standing_one_state_sha256')!=STANDING_ONE_STATE_SHA256 or identity.get('standing_one_inventory_sha256')!=STANDING_ONE_INVENTORY_SHA256:raise RuntimeError('Startup failed admission identity differs')
    # Preserve the native initial state. No AppReady or completed physics was observed.
    ...
    if job.get('phase')!='standing'or job.get('status')!='failed'or job.get('startup_failure_kind')!='no_reference_AppReady_by90s'or job.get('error')!=RETRY_ERROR or job.get('exit_code')is not None or job.get('cleanup_checked')is not True or job.get('container_name')!=RETRY_NAME or job.get('container_id')!=RETRY_ID or job.get('deadline_seconds')!=1200 or job.get('app_ready_deadline_seconds')!=90:raise RuntimeError('Exact startup failure or cleanup differs')
    ...
    if pause.get('unit')!=RETRY_UNIT or pause.get('output')!=str(RETRY_ROOT)or set(restored.get('timers',[]))!=timers or restored.get('restored_unix')!=RETRY_RESTORED_UNIX or restored.get('scope')!='per_job_snapshot_only'or restored.get('persistent_reservation_release_attempted')is not False or restored.get('reservation_path')!=str(RESERVATION):raise RuntimeError('Startup failure pause015 restoration differs')
    names={f.stem for f in (RETRY_ROOT/'jobs').glob('*.json')if not f.name.endswith('_contact_data_audit.json')}
    if names!={'standing'}:raise RuntimeError('Startup job inventory changed')
    for identifier in (RETRY_NAME,RETRY_ID):
        q=subprocess.run(['docker','inspect','--format','{{.Id}} {{.Name}} {{.State.Running}}',identifier],capture_output=True,text=True,timeout=20)
        if q.returncode==0 or not any(v in q.stderr.lower()for v in ('no such object','no such container')):raise RuntimeError('Startup owned container absence unknown: '+identifier)
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-canonical-forecast-restore-016', '--on-active=25m',
