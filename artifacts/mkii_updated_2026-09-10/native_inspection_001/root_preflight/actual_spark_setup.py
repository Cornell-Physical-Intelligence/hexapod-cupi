...
with patch.object(subprocess, 'run', side_effect=forbid), patch.object(subprocess, 'Popen', side_effect=forbid), patch.object(subprocess, 'check_output', side_effect=forbid):
    host = load('_root_actual_host', base/'inspection_host_001/launch_inspection_spark.py')
    args = SimpleNamespace(source=base/'inspection_source_001', asset=base/'asset_001',
        supervisor_source=prior/'reference_physics_source_009', output=base/'native_inspection_001',
