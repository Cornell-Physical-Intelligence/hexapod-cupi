...
with patch.object(subprocess,'run',side_effect=forbidden),patch.object(subprocess,'Popen',side_effect=forbidden),patch.object(subprocess,'check_output',side_effect=forbidden):
 ...
 hostpath=root/'tmp/canonical_native_inspection_host_002/launch_inspection_spark.py'
 host=load('_bound_canonical_host',hostpath)
 with patch.multiple(guard,HOST=hostpath,SOURCE=root/'tmp/updated_native_inspection_002',ASSET=root/'artifacts/mkii_updated_2026-09-10/usd_002/rs05_mass_corrected',SUPERVISOR_SOURCE=root/'tmp/reference_physics_adapter_009/source_009',OUTPUT=root/'tmp/canonical_guard_uncreated_preflight_002'):
