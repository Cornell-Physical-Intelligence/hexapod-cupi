...
class Checks(unittest.TestCase):
 ...
 def test_source_rotation_projection_against_actual_single_patch(self):
  ...
  for p in row['patches']:
   ...
   po=pose[names.index(p['body'])];T=g.shapes[p['body']]['shape_to_link'];actual=a.shape_point(p['point_world_m'],po.tolist(),T);expected=g.cap(p['body'],np.asarray(p['point_world_m'],dtype=np.float32),po)[1]
