...
with patch.object(subprocess,'run',side_effect=forbidden),patch.object(subprocess,'Popen',side_effect=forbidden),patch.object(subprocess,'check_output',side_effect=forbidden):
 ...
 hostpath=root/'tmp/canonical_native_standing_host_004/launch_standing_spark.py'
 host=load('_bound_canonical_host',hostpath)
 with patch.multiple(guard,HOST=hostpath,SOURCE=root/'tmp/updated_native_standing_003',ADMISSION=root/'tmp/canonical_native_actuation_terminal_001/run/actuation',ASSET=root/'artifacts/mkii_updated_2026-09-10/usd_002/rs05_mass_corrected',SUPERVISOR_SOURCE=root/'tmp/reference_physics_adapter_009/source_009',OUTPUT=root/'tmp/canonical_standing_guard_uncreated_preflight_004'):
  ...
  with patch.object(guard,'RESERVATION_PINS',localpins),patch.object(guard,'verify_reservation',return_value={'reservation_files_independently_hashed_locally':True}):
