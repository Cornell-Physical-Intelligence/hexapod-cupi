...
def principal(tensor):
    ...
    q = gf_quat(C)
    rotation = Gf.Rotation(Gf.Quatd(q).GetNormalized())
...
def audit(bundle):
    ...
    for name, l in links.items():
        ...
        # quatf roundoff must be normalized after widening to double. Near identity,
        # Gf's angle extraction otherwise magnifies tiny length error.
        rot = Gf.Rotation(q.GetNormalized())
    ...
    for p in usd_joints:
        ...
        for bi in [0, 1]:
            ...
            rot = Gf.Rotation(Gf.Quatd(q).GetNormalized())
            R = np.array([rot.TransformDir(Gf.Vec3d(*v)) for v in np.eye(3)]).T
