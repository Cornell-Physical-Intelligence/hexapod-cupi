"""Bounded native canonical SDF query acquisition; no controller or physical admission."""
from pathlib import Path
from types import SimpleNamespace
import argparse,hashlib,importlib.util,json,signal,sys,time
sys.dont_write_bytecode=True
SOURCE_FREEZE='174a0c23eb351816abc9fea72acacaf8f40173baa40f73bbe69f70bc71e785f2'
...
def verify_own_bundle(expected=None):
 ...
 if expected is not None and expected!=bound:raise ValueError('Host changed during query')
 verify_tree(p,'FREEZE_SHA256.json',bound);return bound

def require_fresh_output(args):
 if args.output.exists() or args.output.is_symlink():raise ValueError('Fresh query output required')
...
def verify_inputs(args):
 ...
 if CONTRACT is None:CONTRACT=load_module('_canonical_native_query_contract',args.source/'query_contract.py')
 if Path(CONTRACT.__file__).resolve()!=args.source/'query_contract.py':raise ValueError('Wrong native contract import origin')
 identity=CONTRACT.verify_inputs(SimpleNamespace(source=args.source,asset=args.asset,admission=args.admission,output=args.output/'query'))
 if identity.get('training_allowed') is not False or identity.get('physical_admission') is not False:raise ValueError('Native query cannot admit training or physics')
 if identity.get('schema')!='canonical_native_sdf_query_v1' or identity.get('inspector_freeze_sha256')!=SOURCE_FREEZE or identity.get('runtime_binding')!={'runtime_tree_sha256':SOURCE_FREEZE,'scope':'canonical_native_sdf_query_only'}:raise ValueError('Native query runtime identity missing or incompatible')
 if identity.get('steps')!=8 or identity.get('dt')!=.0025 or identity.get('gravity')!=[0.,0.,0.]:raise ValueError('Unexpected query allocation')
 if identity.get('phase_a_state_sha256')!=ADMISSION_STATE:raise ValueError('Wrong completed native Phase A admission')
 if identity.get('ground') is not False or identity.get('query_calls')!=12 or identity.get('points_per_call')!=140 or identity.get('probe_freeze_sha256')!='e1109fc90dc3953bcb064fd0ab80158503ff0d48a3457453d504c5aa0e23e367' or identity.get('query_schema')!='canonical_sdf_distance_diagnostic_v1':raise ValueError('Query allocation or frozen probe differs')
 return identity

validate_inputs=verify_inputs

def command(args,name,phase):
 if phase!='query':raise ValueError('Only native query is allocated')
 return ['docker','compose','--env-file','docker/.env.base','-f','docker/docker-compose.yaml','--profile','base',
  ...
  '-e','PYTHONPATH=/query:/workspace/isaaclab/source/isaaclab',
  '-v',str(args.output)+':/output:rw','-v',str(args.source)+':/query:ro','-v',str(args.asset)+':/asset:ro','-v',str(args.admission)+':/admission:ro',
  '--entrypoint','/workspace/isaaclab/_isaac_sim/python.sh','isaac-lab-base','/query/run_query.py',
  '--asset','/asset','--admission','/admission','--output','/output/query','--device','cuda:0','--headless']
...
def load_supervisor(args,identity):
 ...
 def metadata_save(path,data):
  if Path(path).parent.name=='jobs' and Path(path).name=='query.json':
   data={**data,'no_policy_loaded':True,'training_allowed':False,'physical_admission':False,
    'query_scope':'canonical_native_sdf_query_v1','legacy_supervisor_reused_for_ownership_only':True}
...
def run_campaign(args,identity):
 report={'status':'preparing','started_unix':time.time(),'identity':identity,'host_freeze_sha256':args.host_freeze_sha256,
  'planned_phases':['query'],'training_allowed':False,'physical_admission':False,'stage2_complete':False,
  'automatic_continuation':False,'source_freeze_sha256':SOURCE_FREEZE,'scope':'Native canonical SDF query acquisition only; diagnostic booleans are not physical admission'}
 save(args.output/'campaign.json',report);phase_payloads=None
 try:
  PARENT.run_owned(args,'query')
  ...
  report['query']=CONTRACT.validate_result(args.output/'query',identity)
  phase_payloads=tree(args.output/'query')
  save(args.output/'query_immutable.sha256.json',phase_payloads)
  report['query_payload_manifest_sha256']=sha(args.output/'query_immutable.sha256.json')
  report['post_exit_all_query_payloads_inventoried']=True
  ...
  try:
   ...
   if phase_payloads is not None and tree(args.output/'query')!=phase_payloads:raise ValueError('Query payload changed after post-exit seal')
 ...
 if report['status']!='completed':raise RuntimeError('Native query terminal integrity failed')
 return report
