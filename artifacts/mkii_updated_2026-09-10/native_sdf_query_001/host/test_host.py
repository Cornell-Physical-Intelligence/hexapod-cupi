...
s=importlib.util.spec_from_file_location('canonical_host',HERE/'launch_query_spark.py');h=importlib.util.module_from_spec(s);s.loader.exec_module(h)
class HostTests(unittest.TestCase):
 ...
 def identity(self):return {'schema':'canonical_native_sdf_query_v1','inspector_freeze_sha256':'b'*64,'runtime_binding':{'runtime_tree_sha256':'b'*64,'scope':'canonical_native_sdf_query_only'},'training_allowed':False,'physical_admission':False,'steps':8,'dt':.0025,'gravity':[0.,0.,0.],'phase_a_state_sha256':h.ADMISSION_STATE,'ground':False,'query_calls':12,'points_per_call':140,'probe_freeze_sha256':'e1109fc90dc3953bcb064fd0ab80158503ff0d48a3457453d504c5aa0e23e367','query_schema':'canonical_sdf_distance_diagnostic_v1'}
 ...
 def test_stdlib_import(self):
  subprocess.run([sys.executable,'-S','-B','-c',"import launch_query_spark,sys;assert not any(k in sys.modules for k in ('torch','numpy','isaaclab'))"],cwd=HERE,check=True,capture_output=True)
 def test_only_query_no_checkpoint_asset_and_source_readonly(self):
  args=self.args(Path('/a'));c=h.command(args,'exact-owned','query');mounts=[c[i+1] for i,v in enumerate(c) if v=='-v']
  self.assertEqual(mounts,['/a/output:/output:rw','/a/source:/query:ro','/a/asset:/asset:ro','/a/admission:/admission:ro'])
  self.assertIn('/query/run_query.py',c);self.assertNotIn('--checkpoint',c);self.assertNotIn('--num-envs',c)
 ...
 def test_real_contract_identity_shape_and_phase_only(self):
  a=self.args(Path('/a'));i=self.identity();m=NS(__file__=str(a.source/'query_contract.py'),verify_inputs=lambda args:i)
 ...
 def test_runtime_old_C_identity_rejects(self):
  self.assertNotIn('abe4e354',(HERE/'launch_query_spark.py').read_text())
  self.assertIn("identity.get('runtime_binding')!={'runtime_tree_sha256':SOURCE_FREEZE,'scope':'canonical_native_sdf_query_only'}",(HERE/'launch_query_spark.py').read_text())
 ...
 def test_failed_query_preserved_and_terminal_checked(self):
  with tempfile.TemporaryDirectory() as t:
 ...
 def test_success_requires_native_contract_and_post_exit_input_recheck(self):
  with tempfile.TemporaryDirectory() as t:
   a=self.args(Path(t));a.output.mkdir();i=self.identity();result={'phase':'query','status':'completed','physical_admission':False,'declared_semantics_supported':False,'geometry_accuracy_within_proposed_bounds':False}
   ...
   with patch.object(h,'PARENT',NS(run_owned=Mock())),patch.object(h,'CONTRACT',c),patch.object(h,'verify_inputs',return_value=i) as verify:
    d=h.run_campaign(a,i);self.assertEqual(verify.call_count,2);c.validate_result.assert_called_once_with(a.output/'query',i)
   self.assertEqual(d['status'],'completed');self.assertTrue(d['post_exit_original_inputs_reverified']);self.assertFalse(d['training_allowed']);self.assertTrue(d['post_exit_all_query_payloads_inventoried']);self.assertEqual(h.read(a.output/'query_immutable.sha256.json'),{})
