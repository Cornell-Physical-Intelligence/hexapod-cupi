...
with patch.object(subprocess,'run',side_effect=forbid),patch.object(subprocess,'Popen',side_effect=forbid),patch.object(subprocess,'check_output',side_effect=forbid):
 h=load('_actual_canonical_query_host',here/'launch_query_spark.py')
 ...
 identity={'runtime_binding':{'runtime_tree_sha256':'a'*64,'scope':'canonical_native_sdf_query_only'}}
 ...
 cmd=parent.command(args.source,args.output,'never-created','query')
 assert '/query/run_query.py' in cmd and '/uncreated/canonical/admission:/admission:ro' in cmd
 c=load('_actual_native_query_contract',native/'query_contract.py')
 assert (c.SCHEMA,c.STEPS,c.PHASE,c.DT)==('canonical_native_sdf_query_v1',8,'query',.0025)
 assert Path(c.parent.__file__).resolve()==native/'inspection_contract.py'
 compile((native/'run_query.py').read_text(),str(native/'run_query.py'),'exec')
