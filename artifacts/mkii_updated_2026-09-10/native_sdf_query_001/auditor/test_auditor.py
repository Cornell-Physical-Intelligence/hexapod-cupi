...
class AuditTests(unittest.TestCase):
 ...
 def test_failed_outcome_is_authentic_not_completed(self):
  c={'status':'failed','planned_phases':['query'],'training_allowed':False,'physical_admission':False,'error':'Missing installed API source'}
 ...
 def test_completed_needs_native_postexit_receipt_and_zero_exit(self):
  receipt={'scope':'canonical_native_sdf_query_only','declared_semantics_supported':False,'geometry_accuracy_within_proposed_bounds':False}
  c={'status':'completed','planned_phases':['query'],'training_allowed':False,'physical_admission':False,'query':receipt,'terminal_inputs_unchanged':True,'post_exit_original_inputs_reverified':True,'post_exit_all_query_payloads_inventoried':True}
  job={'status':'completed','exit_code':0,'container_id':'a'*64};unit={'ExecMainStatus':'0','Result':'success'};state={'status':'completed'}
  self.assertEqual(a.classify(c,job,state,unit,{'passed':True,'receipt':receipt}),'authentic_completed_query')
 ...
 def test_exact_two_absences_unknown_docker_rejects_no_fabricated_id(self):
  job={'phase':'query','cleanup_checked':True,'container_name':'hexapod-reference-physics-'+'a'*32,'container_id':'b'*64}
 ...
 def test_restore_exact_timer_and_launch_bindings(self):
  pause={'unit':a.UNIT,'output':str(a.RUN),'source':str(a.SOURCE),'admission':str(a.ADMISSION),'coordination_sha256':'35af5100e39980f55a3f5e19654e4550b3c3fb6da114b64fbe0d143817242cf3','units':{'a.timer':'ActiveState=active','b.timer':'ActiveState=inactive'},'created_unix':10}
