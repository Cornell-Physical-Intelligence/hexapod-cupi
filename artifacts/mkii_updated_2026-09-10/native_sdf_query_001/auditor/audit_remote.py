...
RUN=BASE/'native_query_001'; PAUSE=BASE/'forecast_pause_005'
UNIT='hexapod-canonical-native-query-001-20260910.service'
INV='8304b4418c734871a21cad8f2fe4184e'
SOURCE=BASE/'query_source_002'; HOST=BASE/'query_host_001'; GUARD=BASE/'query_guard_001'; ASSET=BASE/'asset_001'
...
PINS=[('source',SOURCE,'FREEZE_SHA256.json','174a0c23eb351816abc9fea72acacaf8f40173baa40f73bbe69f70bc71e785f2',50),
 ('host',HOST,'FREEZE_SHA256.json','b919fc90924da8765fb32510730392e42b94b7b5fa41d832272a6912f60509ae',16),
 ('guard',GUARD,'FREEZE_SHA256.json','36d56d88953a54bde60191fea7cfb74c49da8f95e4808b3e4f08f0d36f3b459f',46),
...
def owned_absence(job):
 require(job.get('phase')=='query' and job.get('cleanup_checked') is True,'Owned cleanup not verified')
...
def expected_launch_command():
 return ['systemd-run','--user','--unit='+UNIT,'--property=RuntimeMaxSec=720',
  ...
  '/usr/bin/python3',str(HOST/'launch_query_spark.py'),'--source',str(SOURCE),'--asset',str(ASSET),'--admission',str(ADMISSION),
...
def classify(campaign,job,state,unit,native_validation):
 ...
 require(campaign.get('planned_phases')==['query'],'Unexpected allocated phase')
 ...
 if campaign['status']=='completed':
  ...
  require(campaign.get('query')==native_validation.get('receipt'),'Native completed receipt mismatch')
  require(campaign.get('terminal_inputs_unchanged') is True and campaign.get('post_exit_original_inputs_reverified') is True and campaign.get('post_exit_all_query_payloads_inventoried') is True,'Missing completed post-exit integrity')
  return 'authentic_completed_query'
...
def audit():
 result={'schema':'canonical_native_query_terminal_audit_v1','expected_invocation':INV,'unit_name':UNIT,'checked_unix':time.time(),'read_only':True,'audit_verified':False,'query_completed':False,'physical_admission':False,'training_allowed':False,'errors':[]}
 ...
 job=attempt('job',lambda:read(RUN/'jobs/query.json'))
 state=attempt('native_state',lambda:read(RUN/'query/state.json')) if (RUN/'query/state.json').is_file() else None
 result['native_state_available']=state is not None
 if job is not None:attempt('owned_absence',lambda:owned_absence(job))
 def phase_inventory():
  actual={p.name for p in (RUN/'jobs').glob('*.json') if not p.name.endswith('_contact_data_audit.json')};require(actual=={'query.json'},'Unexpected or missing allocated jobs');return sorted(actual)
 ...
 if all(checks.values()) and result.get('input_asset'):
  def hostidentity():
   s=importlib.util.spec_from_file_location('_audit_canonical_host',HOST/'launch_query_spark.py');h=importlib.util.module_from_spec(s);s.loader.exec_module(h)
   ...
   if state is not None and state.get('status')=='completed':
    ...
    try:native_validation.update(passed=True,receipt=h.CONTRACT.validate_result(RUN/'query',identity))
 ...
 if campaign is not None and job is not None:
  ...
  result['query_completed']=outcome=='authentic_completed_query'
  if result['query_completed']:
   def seal():
    p=RUN/'query_immutable.sha256.json';require(sha(p)==campaign.get('query_payload_manifest_sha256'),'Wrong post-exit manifest');require(read(p)=={k:v['sha256'] for k,v in inventory(RUN/'query').items()},'Post-exit query payload changed');return {'passed':True,'manifest_sha256':sha(p)}
 ...
 if not result['audit_verified']:result['query_completed']=False
 return result
