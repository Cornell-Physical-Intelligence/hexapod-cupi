...
with patch.object(subprocess, 'run', side_effect=forbid), patch.object(subprocess, 'Popen', side_effect=forbid), patch.object(subprocess, 'check_output', side_effect=forbid):
    host = load('_root_actual_host', base/'query_host_001/launch_query_spark.py')
    args = SimpleNamespace(source=base/'query_source_002', asset=base/'asset_001', admission=base/'native_inspection_003/inspection',
        supervisor_source=prior/'reference_physics_source_009', output=base/'native_query_001',
    ...
    command = parent.command(args.source, args.output, 'cpu-setup-no-container', 'query')
    assert '/query/run_query.py' in command
    assert str(args.admission)+':/admission:ro' in command
    for name in ('run_query.py', 'query_contract.py', 'inspection_contract.py'):
