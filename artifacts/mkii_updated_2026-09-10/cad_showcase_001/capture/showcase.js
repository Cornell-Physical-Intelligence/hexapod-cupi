...
async function loadGeometry(files){
  ...
  const queue=[...files];
  async function worker(){while(queue.length){const name=queue.shift(),g=await loader.loadAsync('/robot/hexapod_mkii_updated_v1/meshes/'+encodeURIComponent(name));g.computeVertexNormals();geometries.set(name,g);loaded++;$('summary').textContent='Loading original meshes · '+loaded+' / '+files.length;}}
  await Promise.all(Array.from({length:6},worker));return geometries;
}
async function initialize(){
  const response=await fetch('/robot/hexapod_mkii_updated_v1/model_rs05_mass_corrected.json');if(!response.ok)throw new Error('Model data HTTP '+response.status);const payload=await response.text();model=JSON.parse(payload);modelDigest=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',new TextEncoder().encode(payload)))).map(v=>v.toString(16).padStart(2,'0')).join('');
  ...
  const review=await (await fetch('/robot/hexapod_mkii_updated_v1/overlap_review_parts.json')).json();
...
resize();initialize().catch(fail);

// Versioned capture-only camera/display adapter; model coordinates and geometry are unchanged.
const smooth=x=>{x=THREE.MathUtils.clamp(x,0,1);return x*x*x*(10+x*(-15+6*x));};
let showcaseBase=null;
window.showcaseFrame=function(time){
 if(document.body.dataset.modelReady!=='true')throw Error('Model not ready');
 if(!showcaseBase){
  stop();setPose('stand');$('axes').checked=false;jointAxes.forEach(a=>a.visible=false);$('tint').checked=false;updateAppearance();grid.visible=false;
  controls.enableDamping=false;controls.enabled=false;
  const box=new THREE.Box3().setFromObject(robot);showcaseBase={center:box.getCenter(new THREE.Vector3()),size:box.getSize(new THREE.Vector3())};
  renderer.setPixelRatio(1);renderer.setClearColor(0xeef1f2);resize();
 }
 const base=showcaseBase;for(const[n,j]of joints)angles.set(n,j.default_value||0);
 // A modest isolated LF pitch demonstration inside the approved visual bounds.
 const demo=(time>=12&&time<=21)?Math.sin(Math.PI*(time-12)/9)**2:0;
 if(joints.has('lf_femur_pitch'))angles.set('lf_femur_pitch',.22*demo);
 if(joints.has('lf_tibia_pitch'))angles.set('lf_tibia_pitch',.28*demo);
 applyAngles();
 const leg=links.get('lf_tibia');const detail=leg?new THREE.Box3().setFromObject(leg).getCenter(new THREE.Vector3()):base.center.clone();
 const close=smooth((time-10)/3)*(1-smooth((time-20)/4));
 const target=base.center.clone();target.z-=.012;
 const azimuth=-1.12+time*.105;const elevation=.53+.13*Math.sin(time*Math.PI/24);
 const extent=Math.max(base.size.x,base.size.y);const distance=extent*(1.90-.10*close);
 camera.position.set(target.x+distance*Math.cos(azimuth)*Math.cos(elevation),target.y+distance*Math.sin(azimuth)*Math.cos(elevation),target.z+distance*Math.sin(elevation));
 controls.target.copy(target);camera.lookAt(target);camera.updateMatrixWorld(true);
 // Keep the complete articulated assembly inside the central caption-free viewport.
 const frameBox=new THREE.Box3().setFromObject(robot),corners=[];
 for(const x of[frameBox.min.x,frameBox.max.x])for(const y of[frameBox.min.y,frameBox.max.y])for(const z of[frameBox.min.z,frameBox.max.z])corners.push(new THREE.Vector3(x,y,z));
 let projectedBounds;
 for(let iteration=0;iteration<6;iteration++){
  const ndc=corners.map(p=>p.clone().project(camera));projectedBounds={minX:Math.min(...ndc.map(p=>p.x)),maxX:Math.max(...ndc.map(p=>p.x)),minY:Math.min(...ndc.map(p=>p.y)),maxY:Math.max(...ndc.map(p=>p.y))};
  const width=projectedBounds.maxX-projectedBounds.minX,height=projectedBounds.maxY-projectedBounds.minY;
  const dist=camera.position.distanceTo(target),scale=Math.max(width/1.7,height/1.28,1);
  if(scale>1.0001){const offset=camera.position.clone().sub(target).multiplyScalar(scale*1.015);camera.position.copy(target).add(offset);}
  const tangent=Math.tan(THREE.MathUtils.degToRad(camera.fov/2));
  const shift=new THREE.Vector3(1,0,0).applyQuaternion(camera.quaternion).multiplyScalar((projectedBounds.minX+projectedBounds.maxX)/2*dist*tangent*camera.aspect);
  shift.add(new THREE.Vector3(0,1,0).applyQuaternion(camera.quaternion).multiplyScalar(((projectedBounds.minY+projectedBounds.maxY)/2+.04)*dist*tangent));
  camera.position.add(shift);target.add(shift);controls.target.copy(target);camera.lookAt(target);camera.updateMatrixWorld(true);
 }
 const ndcFinal=corners.map(p=>p.clone().project(camera));projectedBounds={minX:Math.min(...ndcFinal.map(p=>p.x)),maxX:Math.max(...ndcFinal.map(p=>p.x)),minY:Math.min(...ndcFinal.map(p=>p.y)),maxY:Math.max(...ndcFinal.map(p=>p.y))};
 $('shot').textContent=time<11?'Six legs · eighteen direct-drive joints':time<21?'Front-left leg · smooth kinematic articulation':'Original detailed geometry · canonical training asset';
 renderer.render(scene,camera);
 return {projectedBounds,time,model_sha256:modelDigest,parts:parts.size,bodies:links.size,joints:joints.size,angles:Object.fromEntries(angles),camera:camera.position.toArray(),target:target.toArray(),size:base.size.toArray()};
};
