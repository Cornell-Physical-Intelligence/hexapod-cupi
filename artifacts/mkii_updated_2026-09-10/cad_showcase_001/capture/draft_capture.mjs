...
try{
 ...
 if(process.argv.includes('--preview-only'))process.exitCode=0;
  ...
  const encoder=spawn('/opt/homebrew/bin/ffmpeg',['-hide_banner','-loglevel','warning','-y','-f','image2pipe','-vcodec','mjpeg','-framerate',String(fps),'-i','pipe:0','-an','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart',path.join(out,'hexapod_detailed_cad_showcase.mp4')],{stdio:['pipe','ignore','pipe']});
  ...
  fs.writeFileSync(path.join(out,'capture_trace.json'),JSON.stringify({fps,frames,duration_s:frames/fps,errors,trace},null,2)+'\n');
  fs.writeFileSync(path.join(out,'ffmpeg.log'),stderr);if(code!==0)throw Error('ffmpeg exit '+code);
  console.log(JSON.stringify({video:path.join(out,'hexapod_detailed_cad_showcase.mp4'),bytes:fs.statSync(path.join(out,'hexapod_detailed_cad_showcase.mp4')).size}));
 }
