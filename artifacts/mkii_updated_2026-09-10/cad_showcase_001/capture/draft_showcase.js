...
window.showcaseFrame=function(time){
 ...
 const target=base.center.clone().lerp(detail,.55*close);target.z+=.025;
 const azimuth=-1.12+time*.105;const elevation=.53+.13*Math.sin(time*Math.PI/24);
 const extent=Math.max(base.size.x,base.size.y);const distance=extent*(1.78-.40*close);
 ...
 return {time,model_sha256:modelDigest,parts:parts.size,bodies:links.size,joints:joints.size,angles:Object.fromEntries(angles),camera:camera.position.toArray(),target:target.toArray(),size:base.size.toArray()};
};
