...
class Standing32Admission(unittest.TestCase):
 ...
 def test_full_prior_map_binds_every_external_large_payload(self):
  ...
  expected={k.replace('run/','native_standing_004/',1) if k.startswith('run/') else k.replace('forecast_pause/','forecast_pause_011/',1):v['sha256'] for k,v in inventory.items()}
