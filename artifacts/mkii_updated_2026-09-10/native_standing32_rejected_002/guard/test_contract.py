...
class GuardTests(unittest.TestCase):
 ...
 def actual_prior(self,unit=None,docker=None):
  base=HERE/'inputs/previous_owner';orig_read=Path.read_text;orig_glob=Path.glob;inventory=json.loads((HERE/'inputs/PRIOR_RAW_INVENTORY.json').read_text())
  def local(p):
  ...
  def localsha(p):
   target=local(p)
   if target.is_file():return hashlib.sha256(target.read_bytes()).hexdigest()
   key=target.relative_to(base).as_posix();return inventory[key]['sha256']
  ...
  with patch.object(Path,'read_text',read),patch.object(Path,'glob',glob),patch.object(g,'sha',side_effect=localsha),patch.object(g,'call',return_value=unit or 'ActiveState=inactive\nMainPID=0\nExecMainStatus=0\nInvocationID='+g.PREVIOUS_INVOCATION),patch.object(g.subprocess,'run',return_value=docker or subprocess.CompletedProcess([],1,'','No such object')) as run:
   yield run
 def test_exact_thirty_eight_previous_payload_pins_and_compact_audit(self):
  d=json.loads((HERE/'inputs/previous_terminal_audit.json').read_text());self.assertTrue(d['audit_verified']);self.assertTrue(d['standing_completed']);self.assertEqual(len(g.PRIOR_PINS),38);self.assertFalse((HERE/'inputs/previous_owner/run/standing/contacts.jsonl').exists())
  with self.actual_prior():
   for name,value in g.PRIOR_PINS.items():self.assertEqual(g.sha(g.PRIOR_BASE/name),value)
 def test_completed_prior_has_exact_two_absences(self):
 ...
 def test_restorer_gpu_and_main_unchanged_except_unique_names(self):
  ...
  old_code=ast.parse(embed(old));new_code=ast.parse(embed(new))
  for n in ast.walk(new_code):
   if isinstance(n,ast.Dict):
    keep=[i for i,k in enumerate(n.keys) if not(isinstance(k,ast.Constant) and k.value in {'scope','persistent_reservation_release_attempted','reservation_path'})]
    n.keys=[n.keys[i] for i in keep];n.values=[n.values[i] for i in keep]
  self.assertEqual(ast.dump(old_code),ast.dump(new_code))
  def gpu(tree):return next(n for n in ast.walk(tree) if isinstance(n,ast.For) and isinstance(n.target,ast.Name) and n.target.id=='line' and any(isinstance(x,ast.Constant) and isinstance(x.value,str) and x.value.startswith('Unrelated CUDA') for x in ast.walk(n)))
  self.assertEqual(ast.dump(gpu(old)),ast.dump(gpu(new)))
 def test_unique_namespace_and_fixed_bounds(self):
  self.assertEqual(g.OUTPUT.name,'native_standing32_002');self.assertEqual(g.PAUSE.name,'forecast_pause_010');self.assertEqual(g.ASSET.name,'asset_001')
  t=(HERE/'launch_guarded_remote.py').read_text()
  for x in ('hexapod-canonical-forecast-restore-010','RuntimeMaxSec=720','TimeoutStopSec=180','--on-active=15m','/opt/wx/gpu.lock','/tmp/hexapod-isaac-gpu.lock'):self.assertIn(x,t)
