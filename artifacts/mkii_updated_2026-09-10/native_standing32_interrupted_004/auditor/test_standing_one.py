...
class StandingOneTests(unittest.TestCase):
 def test_exact_actual_guard_map_and_malformed_or_missing_admission(self):
  guard=HERE.parent/'canonical_native_standing32_guard_004';raw=json.loads((guard/'inputs/PRIOR_RAW_INVENTORY.json').read_text());mapping={k.removeprefix('run/standing/'):v for k,v in raw.items()if k.startswith('run/standing/')}
