"""Audit the first optimized forward replay without changing its verdict."""
...
base = A/'replay_001/standing/evaluation'
report, capture, c, n, integrity, binding = load_recording(base)
remote = read(A/'native_pack_002/REMOTE_INVENTORY.json')
local = {str(p.relative_to(A/'replay_001')):sha(p) for p in sorted((A/'replay_001').rglob('*')) if p.is_file()}
...
state = read(A/'replay_001/standing/state.json')
freeze = read(A/'native_pack_002/source/FREEZE_SHA256.json')
assert state['identity']['source_files'] == freeze
assert all(sha(A/'native_pack_002/source'/name) == digest for name,digest in freeze.items())
assert state['identity']['learned_policy'] is False
assert not state['eligible_for_motion_prior'] and not state['stage2_complete']
...
native=read(A/'replay_001/standing/native/native_readback.json')
limits=torch.tensor(native['limits'],dtype=torch.float32)
solved=arrays(A/'solve_002/trajectory.npz')
...
transitions=arrays(A/'replay_001/standing/recorded_transitions.npz')
...
assert scored['failed_bounds']==report['results'][0]['failed_bounds']==['planar_error_mps']
physical={
...
cleanup=read(A/'replay_001/cleanup.json')
...
assert not remote['containers'] and not remote['gpu'] and 'MainPID=0' in remote['service']
config=Config(**read(A/'solve_002/INPUT.json')['config'])
...
result={'schema':'optimized_trajectory_replay_audit_v1','utc':datetime.now(timezone.utc).isoformat(),
    'source_base':'252db65b0e078e71c381f0d9e924460c56cdead3','trajectory_sha256':sha(A/'solve_002/trajectory.npz'),
    ...
    'video_sha256':sha(base/'rollout.mp4'),'cleanup_verified':True,'learned_policy':False,'eligible_for_motion_prior':False,'stage2_complete':False}
