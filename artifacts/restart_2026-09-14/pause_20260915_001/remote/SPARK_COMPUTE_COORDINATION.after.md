# Current dispatcher paused for handoff — 15 September 2026 UTC

James instructed: “take a pause for now and let someone else continue work, push all non commited changes”. The current dispatcher has stopped research and relinquishes execution after publishing the preserved work. No refit or native allocation is launched by this pause. Another designated HEXAPOD lead may take over the existing user-authorized goal after inspecting the latest repository state and fresh host controls. Do not automatically resume this dispatcher or the historical heartbeat.

The corrected-model Stage 2 goal remains unmet. The latest PPO checkpoint320 fails all three native probes; both cold and settled BC startup trials fail forward tracking. The two-arm BC refit exists only as unexecuted preparation. Full numerical and human visual gates remain unchanged.

This handoff does not release the exclusive HEXAPOD reservation, scheduler masks, reconstruction queue lock or preserved recovery state. Only one successor may own dispatch. Read the repository STATUS.md, docs/TRAINING.md and docs/OPERATIONS.md, verify actual processes and both GPU locks, and bind this new coordination-file hash into a fresh successor guard identity before any allocation. Never overwrite the old admitted source or launch bindings. The previous coordination bytes are preserved at /home/orionh/HEXAPOD_runs/restart_20260914/pause_20260915_001/SPARK_COMPUTE_COORDINATION.before.md.

---

# Exclusive HEXAPOD Spark ownership renewed — 14 September 2026 UTC
...
Current reservation: /home/orionh/HEXAPOD_runs/restart_20260914/spark_ownership_001. DeepSeek service is stopped and persistently masked; reconstruction entry and queue are blocked; existing forecast/Ollama masks remain. Only a later explicit user release permits restoring competing compute. Hold both GPU locks for each allocation, verify live resources and controls before and during launch, and clean only exact owned containers. Authorization does not admit physics or complete a stage. Historical release/pause receipts below remain unchanged.

---

# Exclusive Spark allocation for HEXAPOD — 10 September 2026 UTC

The user's latest explicit instruction is: “make sure our job takes full control of the spark at all times”. HEXAPOD now has exclusive priority for user compute on this Spark, including the intervals between its validation, training, evaluation and recording jobs. This supersedes earlier weather-only priority, sharing-on-request and no-exclusive-reservation statements below. Defer forecasting, reconstruction and other user compute until the user releases or changes this reservation. Preserve their outputs and record exact pause/resume ownership; do not discard another job's work.

Do not automatically restore competing schedulers merely because one HEXAPOD allocation ends. Keep scheduler deferral in force across runs and ordinary session disconnects. SSH, operating-system services and host health remain available. Root remains the sole HEXAPOD GPU dispatcher, with exact job/container ownership, source admission, truthful resource checks, immutable evidence and bounded native phases. This policy is not a claim of a hardware GPU partition; actual reservation enforcement is recorded under `/home/orionh/HEXAPOD_runs/canonical_direct_20260910/exclusive_reservation_001` and in project STATUS.md.

New competing work must honor this reservation. A request by another workload does not override this user instruction; an explicit later user instruction can release or change it. Restore only exact recorded scheduler/process state when that happens. Earlier notes below are preserved historical context.
...
## DeepSeek V4.1 full-memory inference test - 2026-09-14T02:28:54.231100+00:00

The user explicitly authorized stopping buzz-vllm-gemma and requested a full DeepSeek test with memory allocated to DeepSeek. The dsv41-inference.service user service is serving the exact V4.1 Flash GGUF using a 96 GiB expert cache, around 105 GiB total GPU allocation, with an exclusive lock on /opt/wx/gpu.lock. Keep competing GPU model servers, including Gemma, stopped while this service is running. The service is left running for the user after testing; coordinate before restarting another large model. Status and test reports: /home/orionh/projects/dsv41-spark/status.json and /home/orionh/projects/dsv41-spark/full-test-96g/. No other workloads were terminated.
