...
def classify_contacts(data,sensor_map,poses,geometry,num_envs):
 ...
 feet=np.zeros((num_envs,6,3));other=np.zeros((num_envs,4,3));patches=[];used=set()
 for i,(e,body)in enumerate(sensor_map):
  ...
  for k in range(start,start+count):
   ...
   if abs(np.linalg.norm(n)-1)>1e-3:raise ValueError('Invalid patch normal')
   local=None
   if body.endswith('_tibia'):
    cap,local=geometry.cap(body,p,poses[e,geometry.names.index(body)]);category='toe'if cap else'shaft'
   ...
   patches.append({'buffer_index':k,'env':e,'body':body,'category':category,'normal_force_n':f,'point_world_m':p.tolist(),'normal_world':n.tolist(),'separation_m':d,'shape_point_m':None if local is None else local.tolist()})
 ...
 nonfoot=np.zeros(num_envs,dtype=bool)
 for p in patches:
  if p['category']!='toe' and abs(p['normal_force_n'])>1.:nonfoot[p['env']]=True
 for i,(e,body)in enumerate(sensor_map):
  body_force=sum((p['normal_force_n']*np.asarray(p['normal_world'])for p in patches if p['env']==e and p['body']==body and p['category']!='toe'),np.zeros(3))
  if np.linalg.norm(body_force)>1.:nonfoot[e]=True
