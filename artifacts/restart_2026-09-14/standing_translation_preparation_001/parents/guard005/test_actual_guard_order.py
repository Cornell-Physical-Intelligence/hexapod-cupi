...
class ActualMainOrderTests(unittest.TestCase):
 def execute(self,fail_at=None):
  ...
  with contextlib.ExitStack() as stack:
   ...
   stack.enter_context(patch.object(g,'verify_manifest'))
   stack.enter_context(patch.object(g,'require_final_bindings',side_effect=lambda:events.append(('bindings',None))))
   stack.enter_context(patch.object(g,'validate_standing_inputs',side_effect=validate))
   stack.enter_context(patch.object(g,'verify_previous_owner',side_effect=lambda:events.append(('prior_owner',None))))
   stack.enter_context(patch.object(g,'verify_retry_cleanup',side_effect=lambda:events.append(('retry_cleanup',None))))
   ...
   stack.enter_context(patch.object(g.hashlib,'sha256',return_value=SimpleNamespace(hexdigest=lambda:'649ccda1bdef20bc967ae78f68a992cc8001f4ed7009f78cab5133cc67b29d8f')))
 ...
 def test_actual_order_fallback_before_timer_stop_two_checks_then_dispatch(self):
  ...
  self.assertEqual(sum(k=='retry_cleanup'for k,_ in events),2)
  first_check=next(i for i,(k,_)in enumerate(events)if k=='retry_cleanup');nvidia=next(i for i,(k,v)in enumerate(events)if k=='call'and v[0]=='nvidia-smi');self.assertLess(first_check,nvidia)
  runs=[cmd for kind,cmd in events if kind=='run']
  fallback=next(i for i,c in enumerate(runs) if '--on-active=25m' in c)
  ...
  self.assertEqual(dispatch[-8:],['--supervisor-source',str(pause.parent/'source'),'--output',str(pause.parent/'out'),'--num-envs','32','--standing-one',str(g.STANDING_ONE)])
  self.assertEqual(dispatch[dispatch.index('--asset')+1],str(g.ASSET))
  self.assertNotIn('--checkpoint',dispatch)
  self.assertEqual(dispatch[dispatch.index('--admission')+1],str(g.ADMISSION))
