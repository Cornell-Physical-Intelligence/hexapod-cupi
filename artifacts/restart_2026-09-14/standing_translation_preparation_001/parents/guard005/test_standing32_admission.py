...
class Standing32Admission(unittest.TestCase):
 def host(self):
  identity=json.loads((HERE/'CPU_ADMISSION_IDENTITY.json').read_text())['identity']
 ...
 def test_explicit_32_and_actual_standing_one_host_arguments(self):
  h=self.host()
  with patch.object(g,'verify_manifest'),patch.object(g,'verify_reservation'),patch.object(g,'sha',return_value=g.HOST_SHA256):r=g.validate_standing_inputs(h)
 ...
 def test_wrong_1_source_scope_or_admission_hash_rejected(self):
  for key,value in [('num_envs',1),('standing_one_state_sha256','f'*64),('standing_one_inventory_sha256','f'*64),('training_allowed',True)]:
   ...
   with patch.object(g,'verify_manifest'),patch.object(g,'verify_reservation'),patch.object(g,'sha',return_value=g.HOST_SHA256),self.assertRaises(RuntimeError):g.validate_standing_inputs(h)
 def test_full_prior_map_binds_every_external_large_payload(self):
  ...
  self.assertEqual(inventory,audit['raw_inventory']);self.assertEqual(len(inventory),40)
  expected={k.replace('run/','native_standing_006/',1) if k.startswith('run/') else k.replace('forecast_pause/','forecast_pause_014/',1):v['sha256'] for k,v in inventory.items()}
  self.assertEqual(g.PRIOR_PINS,expected)
  self.assertEqual(inventory['run/standing/contacts.jsonl']['size_bytes'],210173083)
 ...
 def test_host_actual_readonly_standing_one_mount_seam_is_frozen(self):
  mapping=json.loads((HERE/'inputs/HOST007_FREEZE_SHA256.json').read_text());self.assertEqual(mapping['launch_standing_spark.py'],g.HOST_SHA256)
