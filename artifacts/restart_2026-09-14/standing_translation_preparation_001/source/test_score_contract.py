...
class Evidence(unittest.TestCase):
 def fixture(self,d):
  ...
  r[LINK_FIELD]=np.zeros((t,n,19,6),np.float32);r[FLOOR_FIELD]=np.zeros((t,19*n,1,3),np.float32)
  ...
  save(d/'session.json',{'steps':8000,'captured_steps':8000,'all_rows_recorded':True,'controls':1000,'reset_count':1,'failure':None,'substep_files':['substeps_000.npz'],'joint_names':NAMES,'root_paths':['/Robot'],'solver_diagnostics':declaration(n,t)})
 ...
 def test_complete_flag_cannot_hide_numeric_quiet_failure_or_late_native_error(self):
  with tempfile.TemporaryDirectory()as x:
   ...
   before={'/Robot':{'position_iterations':32,'velocity_iterations':1,'authored':[False,False]}}
