#!/usr/bin/env python3
"""Fresh full-C standing then bounded zero-residual wave; no PPO or automatic continuation."""
import argparse
import fcntl
import hashlib
import json
...
def command(source, output, name, phase):
    if phase not in ("standing", "wave"):
        raise ValueError("Only fresh standing and bounded wave phases exist")
    common = ["docker", "compose", "--env-file", "docker/.env.base", "-f", "docker/docker-compose.yaml",
        ...
        "/workspace/hexapod/tools/run_reference_physics.py", "--mode", phase,
...
def main():
    ...
    report = dict(status="preparing", runtime_binding=runtime, started_unix=time.time(),
        ...
        source_manifest_sha256=digest(args.source / "campaign_source_hashes.json"), automatic_continuation=False, solver_comparison=PROTOCOL)
    ...
    try:
        ...
        if (identity.get("solver_comparison") != PROTOCOL or comparison.get("protocol") != PROTOCOL
        ...
        wave = run_owned(args, "wave")
        if wave.get("identity") != identity or not wave.get("gate", {}).get("passed"):
            raise RuntimeError("Wave evidence does not pass the exact admitted source contract")
        wave_solver = json.loads((args.output / "wave/solver_comparison.json").read_text())
        if wave_solver.get("protocol") != PROTOCOL or wave_solver.get("actual_scene_attributes") != comparison.get("actual_scene_attributes"):
            raise RuntimeError("Wave solver identity/readback differs from its standing admission")
        if tree_hashes(package) != before:
            raise RuntimeError("Wave changed its read-only admitted asset")
        check_source(args.source)
        report.update(status="completed", wave_state_sha256=digest(args.output / "wave/state.json"),
                      bounded_wave_physics_passed=True, source_unchanged=True, admitted_asset_unchanged=True)
    except Exception as exc:
        report.update(status="stopped" if isinstance(exc, InterruptedError) else "failed", error=repr(exc))
        raise
    finally:
