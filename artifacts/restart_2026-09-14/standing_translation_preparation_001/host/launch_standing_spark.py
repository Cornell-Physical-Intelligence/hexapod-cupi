...
SOURCE_FREEZE='6e11c382b2c942aad3050e9827624e6e9bb529aef8f5608f07054656b70d7163'
...
BINDING_SCHEMA='canonical_placement_host_binding_v1'
...
def verify_binding(args):
 ...
 expected={'schema':BINDING_SCHEMA,'source_freeze_sha256':SOURCE_FREEZE,'host_freeze_sha256':getattr(args,'host_freeze_sha256',None),'placement':args.placement,'output':str(args.output),'native_dispatch_authorized':True}
 if any(binding.get(k)!=v for k,v in expected.items()) or not valid_hash(binding.get('coordination_sha256')):raise ValueError('Fresh placement/source/host/output authorization binding required')
 return binding

def verify_inputs(args):
 global CONTRACT
 if type(args.num_envs) is not int or args.num_envs!=1 or args.standing_one is not None:raise ValueError('Single placement diagnosis only; no standing admission input')
 if args.placement not in ('origin','xy14_4'):raise ValueError('Explicit placement required')
 verify_binding(args)
 ...
 identity=CONTRACT.verify_inputs(SimpleNamespace(source=args.source,asset=args.asset,admission=args.admission,output=args.output/'standing',num_envs=args.num_envs,standing_one=args.standing_one,placement=args.placement))
 ...
 if identity.get('schema')!='canonical_single_placement_diagnostic_v1' or identity.get('inspector_freeze_sha256')!=SOURCE_FREEZE or identity.get('runtime_binding')!={'runtime_tree_sha256':SOURCE_FREEZE,'scope':'canonical_single_placement_diagnostic_only'}:raise ValueError('Native standing runtime identity missing or incompatible')
 if any(identity.get(k)is not False for k in ('standing_admission','batch_admission','stage2_complete')) or identity.get('placement',{}).get('placement')!=args.placement:raise ValueError('Diagnostic identity admits broader work or changes placement')
...
def command(args,name,phase):
 ...
 return ['docker','compose','--env-file','docker/.env.base','-f','docker/docker-compose.yaml','--profile','base',
  ...
  '--asset','/asset','--admission','/admission','--output','/output/standing','--num-envs',str(args.num_envs),'--placement',args.placement,*(['--standing-one','/standing_one'] if args.standing_one else []),'--device','cuda:0','--headless']

def load_deadline_adapter():
 return load_module('_canonical_standing_deadline_adapter',Path(__file__).resolve().parent/'deadline_adapter.py')
...
def load_supervisor(args,identity):
 ...
 load_deadline_adapter().install(PARENT,args.supervisor_source/'tools/launch_reference_physics_spark.py')
 ...
 def metadata_save(path,data):
  if Path(path).parent.name=='jobs' and Path(path).name=='standing.json':
   data={**data,'no_policy_loaded':True,'training_allowed':False,'physical_admission':False,
    'standing_scope':'canonical_single_placement_diagnostic_v1','standing_admission':False,'batch_admission':False,'legacy_supervisor_reused_for_ownership_only':True,
    'supervisor_runtime_adapter':PARENT.CANONICAL_DEADLINE_ADAPTER}
...
def run_campaign(args,identity):
 report={'status':'preparing','started_unix':time.time(),'identity':identity,'host_freeze_sha256':args.host_freeze_sha256,
  ...
  'automatic_continuation':False,'source_freeze_sha256':SOURCE_FREEZE,'standing_admission':False,'batch_admission':False,'scope':'Single placement diagnostic only; no standing, hardware, walking or training admission'}
...
def main():
 ...
 p.add_argument('--num-envs',type=int,choices=[1],required=True);p.add_argument('--standing-one',type=Path);p.add_argument('--placement',choices=['origin','xy14_4'],required=True);p.add_argument('--bindings',type=Path,required=True)
 ...
 if args.coordination_sha256!=verify_binding(args)['coordination_sha256']:raise ValueError('Coordination changed; no allocation')
