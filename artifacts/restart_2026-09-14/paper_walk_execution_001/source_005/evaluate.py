...
def run_batch(env, policy, cases, output, geometry_extrema, *, checkpoint_sha256, source_sha256,
    ...
    rows=[];failure=None;writer=None;frames=0;allocation_limit_reached=False
    ...
    try:
        ...
        for control in range(cases[0]["controls"]):
            ...
            if bool(result["terminated"].any() or result["truncated"].any()):
                raise RuntimeError("Native terminal/timeout: preserved failed prefix; no automatic reset")
    ...
    report={**declaration,"acquisition_complete":failure is None,"failure":failure,"controls":len(rows),
