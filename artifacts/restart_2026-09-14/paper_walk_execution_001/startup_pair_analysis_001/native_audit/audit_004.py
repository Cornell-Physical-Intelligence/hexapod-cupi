...
def inspect(arm,geometry):
    ...
    compatibility=[]
    assert set(differences)<={'vertical_velocity_abs_p95_mps','tilt_p95_degrees'},differences
    assert not(set(differences)&set(saved['checks'])), 'A checked metric differs'
    for metric,values in differences.items():
        if metric=='vertical_velocity_abs_p95_mps':
            samples=np.sort(abs(tail['velocity_world_mps'][100:,0,2]))
        else:
            x,y=tail['root_pose_xyzw'][100:,0,3:5].T
            samples=np.sort(np.arccos(np.clip(1-2*(x*x+y*y),-1.,1.)))
        rank=np.float32(len(samples)-1)*np.float32(.95)
        lo=int(rank);fraction=rank-np.float32(lo);delta=samples[lo+1]-samples[lo]
        value=samples[lo+1]-delta*(1-fraction) if fraction>=.5 else samples[lo]+delta*fraction
        if metric=='tilt_p95_degrees':value=np.degrees(value)
        archived=float(value)
        assert archived==saved['metrics'][metric],(metric,archived,saved['metrics'][metric])
        compatibility.append({'metric':metric,'values':values,
            'native_float32_rank_reconstruction':archived,'scope':'Descriptive percentile only in this profile; no gate or native value changed. All gate checks and verdicts reproduce exactly.'})
    ...
    return dict(arm=arm,controls=count,physics_steps=packets,patches_reclassified=patches,inactive_zero_tuples=inactive,
        ...
        original_metrics=saved['metrics'],local_metrics=score['metrics'],windows=windows,support=support,
...
if __name__=='__main__':
    ...
    dump(HERE/'RESULT_004.json',result)
