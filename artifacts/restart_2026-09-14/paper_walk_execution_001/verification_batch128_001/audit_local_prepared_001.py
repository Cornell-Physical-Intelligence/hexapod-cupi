...
def audit(result):
    ...
    for translated in read(preparation/'input_translation.json').values():
        require(sha(translated['local_path'])==translated['sha256'],'Prepared input changed')
    ...
    model=read(REPO/'artifacts/mkii_updated_2026-09-10/usd_002/rs05_mass_corrected/source/model.json')
...
if __name__=='__main__':
    ...
    inputs.update({row['local_path']:sha(row['local_path']) for row in read(preparation/'input_translation.json').values()})
    try:report=audit(args.result)
