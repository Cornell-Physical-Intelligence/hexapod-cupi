...
class NativePolicyCamera:
    ...
    def _initialize(self,width,height):
        ...
        kits=[viz for viz in self.env.sim.visualizers
            if getattr(viz.cfg,"visualizer_type",None)=="kit" and viz.pumps_app_update()]
        if not kits or any(not getattr(viz,"_is_initialized",False) or not viz.is_running() for viz in kits):
            raise RuntimeError("Native capture requires an initialized running Kit visualizer")
    ...
    def _capture(self,index,width,height):
        ...
        try:
            ...
            for attempt in range(self.max_capture_attempts):
                ...
                if float(self.timeline.get_current_time())!=timeline_before:
                    raise RuntimeError("Kit RGB capture changed timeline time")
