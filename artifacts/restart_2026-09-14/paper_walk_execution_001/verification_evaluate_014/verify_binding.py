"""Read-only binding/lifecycle validation for actual evaluation014."""
...
def check():
    base=A/'results_evaluate_014'
    state=read(base/'standing/state.json');identity=read(base/'standing/identity.json')
    old=read(A/'results_evaluate_013/standing/identity.json')
    launch=read(base/'launch_binding.json');dispatch=read(A/'DISPATCH_evaluate_014.json')
    freeze=read(A/'source_018/FREEZE_SHA256.json')
    native=read(base/'standing/native/native_readback.json')
    previous_native=read(A/'results_evaluate_013/standing/native/native_readback.json')
    ...
    summary=read(base/'standing/evaluation/summary.json');transfer=read(A/'VERIFIED_TRANSFER_evaluate_014.json')
    ...
    checkpoint=A/'results_train_007/standing/learner/checkpoint_000220.pt'
    checkpoint_hash=hashlib.sha256(checkpoint.read_bytes()).hexdigest();inputs[str(checkpoint)]=checkpoint_hash
    assert checkpoint_hash==state['input_checkpoint_sha256']=='e608b454abbed24827810dbb91acf6c17fca677447e49e68cd6fc7e981c2de2a'
    ...
    assert transfer['all_file_bytes_verified'] and transfer['verified_files']==71
    passes=[]
    for i in range(3):
    ...
    return {'schema':'evaluate014_binding_review_v1','checks_passed':True,'source_freeze_sha256':digest,
        'checkpoint_sha256':checkpoint_hash,'same_physics_identity_as013':True,
