...
def score_recording(data: Mapping, metadata: Mapping):
    ...
    if profile not in {"omni_static", "quiet_stand", "stop_to_stand", "stage2c_formal"}:
    ...
    n = len(root)
    expected = {"omni_static": 1000, "quiet_stand": 1000, "stop_to_stand": 1050,
                "stage2c_formal": 500}[profile]
    start = {"omni_static": 100, "quiet_stand": 200, "stop_to_stand": 500,
    ...
    elif profile == "quiet_stand" and command is not None:
    ...
    if profile in {"quiet_stand", "stop_to_stand"}:
        _check(r, "quiet_window_controls", max(n - start, 0), 800 if profile == "quiet_stand" else 500, minimum=True)
