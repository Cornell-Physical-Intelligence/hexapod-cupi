...
class PaperWalkEnv:
    ...
    def get(self, getter):
        return self.wp.to_torch(getattr(self.view, getter)()).clone()
    ...
    def _observations(self, state):
        ...
        amp = torch.cat((state["q"], state["dq"], state["linear"], state["angular"],
                         state["root"][:, 2:3], state["toe_body"].flatten(1)), dim=-1)
        return {"obs": obs.clone(), "critic": critic.clone(), "amp": amp.clone()}
    ...
    def step(self, action):
        ...
        tracking = torch.exp(-((velocity_nav[:, :2] - self.commands[:, :2]).square().sum(-1)) / .01)
        yaw_tracking = torch.exp(-(angular_nav[:, 2] - self.commands[:, 2]).square() / .25)
        tilt_cost = state["gravity"][:, :2].square().sum(-1)
        torque_cost = torque_square.mean(-1) / 8
        slew_cost = ((target - old_target) / .04).square().mean(-1)
        reward = tracking + .3 * yaw_tracking - .5 * tilt_cost - .005 * torque_cost - .01 * slew_cost - .05 * velocity_nav[:, 2].square()
        ...
        self.telemetry = {"root_pose_xyzw": state["root"].clone(), "linear_velocity_body": state["linear"].clone(),
            ...
            "terminated": terminated.clone(), "truncated": truncated.clone(), "tracking_reward": tracking.clone()}
        result = self._observations(state)
        result.update(reward=reward, terminated=terminated, truncated=truncated)
        return result

    def render(self, index=0, width=1280, height=720):
        if not self.cfg.render:
            raise RuntimeError("RGB rendering was not enabled before app startup")
        import omni.replicator.core as rep
        if self._rgb is None:
            self._camera = rep.create.camera(position=(1., 1., .6), look_at=(0., 0., .1), focal_length=24.)
            self._render_product = rep.create.render_product(self._camera, (width, height))
            self._rgb = rep.AnnotatorRegistry.get_annotator("rgb")
            self._rgb.attach([self._render_product])
            warmup = 12
        else:
            warmup = 1
        position = self.current["root"][index, :3].detach().cpu().tolist()
        with self._camera:
            rep.modify.pose(position=(position[0] + .8, position[1] + 1., .65), look_at=(position[0], position[1] - .1, .1))
        before = self.sim.get_physics_step_count()
        for _ in range(warmup):
            self.sim.render()
        if self.sim.get_physics_step_count() != before:
            raise RuntimeError("Rendering unexpectedly advanced physics")
        pixels = np.asarray(self._rgb.get_data())
        if pixels.ndim != 3 or pixels.shape[-1] < 3 or float(np.std(pixels)) < 1.:
            raise RuntimeError("Missing or blank native Isaac RGB frame")
