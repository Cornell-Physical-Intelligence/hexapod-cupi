...
for run,payload in [('012',bc32),('013',trained)]:
    ...
    for index in range(3):
        ...
        feature_error=float(np.max(abs(expected_feature.numpy()-obs[:,213:])))
        assert feature_error<=1e-7
        ...
        r={'evaluation':run,'case':report['cases'][0]['case_id'],'controls':len(a),'joint_names':names,
           'command_in_policy_obs_exact':True,'previous_executed_target_feature_max_cpu_gpu_error':feature_error,
           'previous_executed_target_feature_tolerance':1e-7,
