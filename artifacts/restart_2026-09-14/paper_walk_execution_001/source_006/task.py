...
class TrainingTask:
    ...
    def step(self, action):
        ...
        self._metrics(command, output)
        result = self._next_command_observation(output)
        result["reward"] = reward
        return result

    def _metrics(self, command, output):
        ...
        values = {"signed_navigation_velocity_error_mps": error.sum(0),
            ...
            "zero_command_rows": (command == 0).all(-1).sum()}
    ...
    def status(self, *, reset_interval=False):
        def summary(values, rows):
            counts = {"moving_command_rows", "terminations", "truncations", "zero_command_rows"}
            return {"environment_controls": rows, **{key: (value if key in counts else value/max(1, rows)).cpu().tolist()
                for key, value in values.items()}}
