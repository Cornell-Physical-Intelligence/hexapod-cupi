"""CPU-only verification of all native013 samples and unchanged gate results."""
...
def inspect(index,geometry):
    base=A/f'results_evaluate_013/standing/evaluation/batch_{index:03d}'
    ...
    for key,value in recomputed.items():
        if key not in ('pass','failed_bounds'):assert value==original[key],key
    ...
    native=read(A/'results_evaluate_013/standing/native/native_readback.json')
    ...
    previous=read(A/f'results_evaluate_012/standing/evaluation/batch_{index:03d}/report.json')['results'][0]
    return {'case_id':case['case_id'],'controls':len(c['time_s']),'physics_steps':packets,
        'original_score_recomputed_exact_with_float32_percentile_compatibility':True,'percentile_compatibility':quantile_compatibility,'original_pass':original['pass'],
        ...
        'metrics012':previous['metrics'],'metrics013':original['metrics'],
        'failed_bounds012':previous['failed_bounds'],'input_integrity':integrity}


def main():
    inputs=[p for p in (A/'results_evaluate_013').rglob('*') if p.is_file()]
    source=A/'source_017';freeze=read(source/'FREEZE_SHA256.json')
    ...
    identity=read(A/'results_evaluate_013/standing/identity.json')
    assert sha(geometry_path)==identity['geometry_sha256']
    assert read(A/'VERIFIED_TRANSFER_evaluate_013.json')['all_file_bytes_verified']
    before={str(p):sha(p) for p in inputs}
    result={'schema':'independent_evaluate013_native_verification_v1','status':'running','stage2_complete':False}
    try:
    ...
    result['limitations']=['Scoring reuses exact original maintained scorer; independent code separately reconstructs every saved contact point and force.',
        ...
        'Verification passing means the original failed numerical verdicts are reproducible, not policy qualification.']
