...
one initial reset and uninterrupted physics. The additional startup diagnostic
explicitly labels its scripted neutral prefix before policy control. Failed prefixes,
...
def run_batch(env, policy, cases, output, geometry_extrema, *, checkpoint_sha256, source_sha256,
    ...
    startup = _startup_prefix_controls is not None
    if startup:
        prefix = _startup_prefix_controls
        if (type(prefix) is not int or prefix not in (0, 200) or env.num_envs != 1
                or len(cases) != 1 or cases[0] != startup_case('cold' if prefix == 0 else 'neutral4')):
            raise ValueError("Startup prefix belongs only to the exact additional diagnostic case")
    ...
    if startup:
        declaration.update(startup_prefix_controls=prefix, policy_control_slice=[prefix,prefix+1000],
            action_source_labels=['scripted_neutral','bc'],
            policy_action_semantics='Actual issued action; scripted prefix is not a BC action. actor_mean_action records the unused actor proposal separately.',
            scripted_neutral_is_policy_quiet=False)
    ...
    try:
        ...
        for control in range(cases[0]["controls"]):
            ...
            scripted=startup and control < prefix
            requested=[[0.,0.,0.]] if scripted else [command_at(c,control) for c in assignments]
            observation=_set_command(env,requested)
            if startup and control==prefix:
                handoff_counter=env.sim.get_physics_step_count()
                handoff={'global_control_index':control,'physics_counter':handoff_counter,
                    'policy_command':_cpu(env.commands).tolist(),
                    'actual_held_target_rad':_cpu(env.held).tolist(),
                    'native_state_before_policy':{k:_cpu(v).tolist() for k,v in env.current.items()},
                    'observations_before_policy':{k:_cpu(v).tolist() for k,v in observation.items()},
                    'extra_reset_or_physics_step':False}
                if env.sim.get_physics_step_count()!=handoff_counter:
                    raise RuntimeError('Startup handoff recording advanced physics')
                _save(out/'policy_handoff.json',handoff)
            ...
            if hasattr(policy, "model"):
                ...
                if startup and (not bool(torch.isfinite(mean).all()) or not bool(torch.isfinite(estimate).all())):
                    raise ValueError('Nonfinite startup actor output')
                if not scripted and not torch.equal(mean, action):
                ...
                if startup:
                    row['actor_mean_action']=_cpu(mean)
            if startup:
                row['issued_action']=_cpu(action)
                row['action_source']=np.array('scripted_neutral' if scripted else 'bc')
                row['global_control_index']=np.array(control)
                row['policy_control_index']=np.array(-1 if scripted else control-prefix)
                if scripted and (bool(action.any()) or not torch.equal(env.held, env.neutral.expand_as(env.held))):
                    raise RuntimeError('Scripted neutral did not retain the actual neutral held target')
    ...
    results=[]
    startup_windows=None
    if startup:
        try:
            startup_windows=_startup_physical_windows(out/'native400hz',receipt,env.native_readback,prefix)
        except Exception as error:
            startup_windows={'pass':False,'error':repr(error)}
    for index,case in enumerate(cases):
        ...
        try:
            scored={k:v[prefix:].copy() for k,v in data.items()} if startup else data
        ...
        if startup:
            native_ok = native_ok and startup_windows['pass']
            result.update(policy_control_slice=[prefix,len(rows)], policy_time_keeps_original_absolute_timestamps=True,
                          policy_window_requested_controls=1000, startup_physical_windows=startup_windows)
    ...
    if startup:
        report['startup_physical_windows']=startup_windows
...
def startup_case(arm):
    """Fixed extra cases, outside both the13 probes and96 required cases."""
    if arm not in ('cold','neutral4'):
        raise ValueError('Startup arm must be cold or neutral4')
    return {'case_id':'startup:'+arm+':bc_forward_0.05','profile':'omni_static',
            'command':[.05,0.,0.],'controls':1000 if arm=='cold' else 1200}


def _startup_physical_windows(directory, receipt, native, prefix):
    """Audit saved whole/prefix/policy samples; benign prefix cannot dilute tail."""
    directory=Path(directory)
    arrays=[]
    for name in receipt['substep_files']:
        path=directory/name
        if _sha(path)!=receipt['files'][name]:
            raise ValueError('Startup native chunk changed')
        with np.load(path,allow_pickle=False) as saved:
            arrays.append({key:saved[key].copy() for key in ('sequence','control_index','substep_index',
                'explicit_counter','joint_position_rad','joint_velocity_rad_s','applied_torque_nm',
                'computed_torque_nm','native_input_pre_nm','root_pose_xyzw','nonfoot_contact',
                'minimum_non_toe_floor_m','distal_contact')})
    if not arrays:
        raise ValueError('Missing startup native samples')
    raw={key:np.concatenate([a[key] for a in arrays]) for key in arrays[0]}
    n=len(raw['sequence']);seq=np.arange(n)
    if (n!=receipt['steps'] or not np.array_equal(raw['sequence'],seq)
            or not np.array_equal(raw['control_index'],seq//8)
            or not np.array_equal(raw['substep_index'],seq%8)
            or not np.array_equal(raw['explicit_counter'],receipt['initial_counter']+seq+1)):
        raise ValueError('Startup native sequence/counters differ')
    if not all(np.isfinite(x).all() for x in raw.values()):
        raise ValueError('Nonfinite startup native samples')
    if not np.array_equal(raw['native_input_pre_nm'],raw['applied_torque_nm']):
        raise ValueError('Startup native applied input differs')
    contact_path=directory/'contacts.jsonl'
    if _sha(contact_path)!=receipt['files']['contacts.jsonl']:
        raise ValueError('Startup contact packets changed')
    contact_count=0
    with contact_path.open() as stream:
        for i,line in enumerate(stream):
            row=json.loads(line)
            if row.get('sequence')!=i or row.get('explicit_counter')!=receipt['initial_counter']+i+1 or not isinstance(row.get('patches'),list):
                raise ValueError('Startup contact packet sequence differs')
            contact_count+=1
    if contact_count!=n:
        raise ValueError('Missing startup contact packets')
    limits=np.asarray(native['limits']);speed=np.asarray(native['native_max_velocity'])
    windows={}
    for name,start,end,bound in [('full',0,(prefix+1000)*8,.001),
                               ('scripted_prefix',0,prefix*8,0.),
                               ('policy',prefix*8,(prefix+1000)*8,.001)]:
        expected=end-start;selected=(seq>=start)&(seq<end);count=int(selected.sum())
        if expected==0:
            windows[name]={'applicable':False,'expected_substeps':0,'recorded_substeps':0,'pass':True}
            continue
        if not count:
            windows[name]={'applicable':True,'expected_substeps':expected,'recorded_substeps':0,'pass':False}
            continue
        take=lambda key:raw[key][selected]
        q=take('joint_position_rad');dq=take('joint_velocity_rad_s')
        values={'applicable':True,'expected_substeps':expected,'recorded_substeps':count,
            'global_substep_slice':[start,end],
            'joint_bound_violation_steps':int(((q<limits[:,:,0]-2e-6)|(q>limits[:,:,1]+2e-6)).any(-1).sum()),
            'speed_bound_violation_steps':int((abs(dq)>speed+2e-6).any(-1).sum()),
            ...
            'nonfoot_steps':int(take('nonfoot_contact').sum()),'maximum_nonfoot_fraction':bound,
        ...
        values['nonfoot_fraction']=values['nonfoot_steps']/count
        values['pass']=bool(count==expected and receipt['failure'] is None
            and not values['joint_bound_violation_steps'] and not values['speed_bound_violation_steps']
            and values['maximum_applied_nm']<=1.60001 and values['minimum_non_toe_floor_m']>=-.001
            and values['minimum_plate_height_m']>=.055 and values['nonfoot_fraction']<=bound)
        windows[name]=values
    return {'pass':all(w['pass'] for w in windows.values()),'windows':windows,
        'contacts_and_native_hashes_verified':True,
        'scope':'Original native motor/joint/clearance/contact checks over complete recorded windows. Scripted prefix is not BC quiet qualification; demand saturation and missing six-toe counts remain explicit diagnostics.'}


def _policy_state_sha(policy):
    digest=hashlib.sha256()
    for owner in ('model','amp'):
        for key,value in getattr(policy,owner).state_dict().items():
            value=value.detach().cpu().contiguous()
            digest.update((owner+':'+key+':'+str(value.dtype)+':'+str(tuple(value.shape))).encode())
            digest.update(value.numpy().tobytes())
    return digest.hexdigest()


def run_startup_diagnostic(env,policy,output,geometry_extrema,checkpoint,*,arm,
                           record_video=True,seed=27057,progress=None,max_wall_seconds=None):
    """One arm per fresh app/allocation; scripted prefix never admits policy quiet."""
    deadline=_deadline(max_wall_seconds)
    case=startup_case(arm);prefix=case['controls']-1000
    if env.num_envs!=1 or getattr(env,'total_controls',None)!=0 or getattr(env,'_startup_diagnostic_used',False):
        raise ValueError('Startup diagnostic requires a fresh one-robot control allocation')
    if not hasattr(policy,'model') or not hasattr(policy,'amp'):
        raise ValueError('Startup diagnostic requires the actual checkpoint actor and AMP')
    if record_video and not env.cfg.render:
        raise ValueError('Startup video requires native cameras before startup')
    checkpoint_sha=_sha(checkpoint);source_sha=_sha(__file__);scorer_sha=_sha(scoring.__file__)
    state_sha=_policy_state_sha(policy)
    env._startup_diagnostic_used=True
    report=run_batch(env,policy,[case],output,geometry_extrema,checkpoint_sha256=checkpoint_sha,
        source_sha256=source_sha,seed=seed,video_case_id=case['case_id'] if record_video else None,
        progress=progress,max_wall_seconds=max_wall_seconds,_startup_prefix_controls=prefix)
    integrity_failure=None
    try:
        if (_sha(checkpoint)!=checkpoint_sha or _sha(__file__)!=source_sha
                or _sha(scoring.__file__)!=scorer_sha or _policy_state_sha(policy)!=state_sha):
            integrity_failure='Startup checkpoint/source/actor/AMP state changed'
    except Exception as error:
        integrity_failure=repr(error)
    allocation_limit_reached=report['allocation_limit_reached'] or time.monotonic()>=deadline
    report.update(schema='canonical_bc_startup_diagnostic_v1',arm=arm,stage2_complete=False,
        diagnostic_policy_screen_passed=not integrity_failure and not allocation_limit_reached and all(r['pass'] for r in report['results']),
        allocation_limit_reached=allocation_limit_reached,
        policy_state_sha256_before=state_sha,policy_state_unchanged=integrity_failure is None,
        integrity_failure=integrity_failure,policy_control_slice=[prefix,prefix+1000],
        onset_control_slice=[prefix,prefix+100],scripted_prefix_is_bc_quiet=False,
        required_cases_not_evaluated=[c['case_id'] for c in case_manifest()],
        learning_probes_not_evaluated=[c['case_id'] for c in learning_probe_cases()],
        scope='One explicitly separate startup diagnostic arm; no cold qualification, quiet/stop result or required case is replaced.',
        app_scope='Run each arm in a separate fresh application allocation; no prior control steps accepted. Native SDK initialization is retained.')
    _save(Path(output)/'startup_result.json',report)
    return report
...
def run_learning_probe_suite(env,policy,output,geometry_extrema,checkpoint,*,
                             ...
                             progress=None,max_wall_seconds=None,selected_case_ids=None):
    """Run all 13 probes, or an ordered subset, without admitting Stage2.
    ...
    Unselected probes remain missing; only all 13 can pass the diagnostic suite.
    """
    ...
    if selected_case_ids is None:
        ...
        if not isinstance(selected_case_ids,(list,tuple)) or not selected_case_ids:
            raise ValueError("Selected probe cases must be a nonempty ordered list or tuple")
        selected_ids=list(selected_case_ids)
        if any(not isinstance(case_id,str) or case_id not in by_id for case_id in selected_ids):
            raise ValueError("Unknown selected learning-probe case")
    ...
    video_case_id=(video_case_id or selected_ids[0]) if record_video else None
    if video_case_id is not None and video_case_id not in selected_ids:
    ...
    declaration={"schema":"canonical_learning_probe_allocation_v1","probe_cases":cases,
        ...
        "unselected_probe_case_ids":[case_id for case_id in by_id if case_id not in selected_ids],
        ...
        "record_video":record_video,"video_case_id":video_case_id,"requested_controls":sum(c["controls"] for c in selected),
    ...
    def summary():
        ...
        value.update(schema="canonical_learning_probe_suite_v1",allocation=declaration,results=results,
            ...
            missing_selected_probe_cases=[case_id for case_id in selected_ids if case_id not in ids],
            ...
            diagnostic_screens_passed=not acquisition_failure and not allocation_limit_reached and
                len(results)==len(cases) and ids==set(by_id) and all(r["pass"] for r in results),
    ...
    for index,case in enumerate(selected):
        ...
        def trial_progress(update):
            if progress is not None:
                progress({"probe_index":index,"probe_count":len(selected),"declared_probe_count":len(cases),**update})
        try:
