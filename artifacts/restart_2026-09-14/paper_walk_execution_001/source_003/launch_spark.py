...
def verify(binding, own):
    ...
    guard.require(binding['mode'] in ('diagnostic','replay','train','video'),'Unsupported native mode')
...
def main():
    ...
    def save(path,data):
        if Path(path).parent.name=='jobs' and Path(path).name=='standing.json':
            data={**data,'no_policy_loaded':binding['mode'] in ('diagnostic','replay'),'native_mode':binding['mode'],
