"""CPU-only verification of all native015 samples and unchanged gate results."""
...
def inspect(index,geometry):
    base=A/f'results_evaluate_015/standing/evaluation/batch_{index:03d}'
    ...
    native=read(A/'results_evaluate_015/standing/native/native_readback.json')
    ...
    previous=read(A/f'results_evaluate_014/standing/evaluation/batch_{index:03d}/report.json')['results'][0]
    return {'case_id':case['case_id'],'controls':len(c['time_s']),'physics_steps':packets,
        ...
        'metrics014':previous['metrics'],'metrics015':original['metrics'],
        'failed_bounds014':previous['failed_bounds'],'input_integrity':integrity}


def main():
    inputs=[p for p in (A/'results_evaluate_015').rglob('*') if p.is_file()]
    source=A/'source_018';freeze=read(source/'FREEZE_SHA256.json')
    inputs.extend([source/'FREEZE_SHA256.json',A/'VERIFIED_TRANSFER_evaluate_015.json',Path(__file__)])
    inputs.extend(A/f'results_evaluate_014/standing/evaluation/batch_{i:03d}/report.json' for i in range(3))
    for name in ('env.py','env_config.py','evaluation.py','analyze.py'):
        path=REPO/'experiments/paper_walk'/name;assert sha(path)==freeze[name];inputs.append(path)
    inputs.append(source/'evaluate.py')
    assert sha(source/'evaluate.py')==freeze['evaluate.py']
    ...
    identity=read(A/'results_evaluate_015/standing/identity.json')
    assert sha(geometry_path)==identity['geometry_sha256']
    assert read(A/'VERIFIED_TRANSFER_evaluate_015.json')['all_files_verified']
    before={str(p):sha(p) for p in inputs}
    result={'schema':'independent_evaluate015_native_verification_v1','status':'running','stage2_complete':False}
    try:
    ...
    result['limitations']=['Frozen018 acquisition orchestrator is verified as data; evolving maintained evaluate.py is not imported by this audit.',
        'Scoring reuses exact original maintained scorer; independent code separately reconstructs every saved contact point and force.',
