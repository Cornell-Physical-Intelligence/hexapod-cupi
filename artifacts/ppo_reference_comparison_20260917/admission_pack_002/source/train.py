...
def main(argv=None):
    ...
    try:
        ...
        if env is not None:
            ...
            native = args.output/'native400hz'
            if (native/'session.json').is_file() and not (native/'force_metrics.json').exists():
                try:
                    loads_report(native, MKII_RS05_MODEL_SHA256, 1000, args.source_freeze_sha256)
                except (ValueError, KeyError, OSError) as error:
                    save(native/'force_metrics.json', {'status': 'unavailable', 'error': repr(error)})
            env.close()
        if app is not None:
            app.close()
