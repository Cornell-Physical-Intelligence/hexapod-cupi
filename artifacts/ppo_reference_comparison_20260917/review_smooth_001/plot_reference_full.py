...
for ax,(title,path,color) in zip(axes,trials):
    ...
    ax.set(title=title,ylabel="Forward speed (m/s)",xlim=(0,20),ylim=(-.05,.14))
    ax.grid(alpha=.2)
axes[0].legend(loc="upper right",ncol=3,fontsize=8)
axes[-1].set_xlabel("Simulation time (s)")
fig.suptitle("Measured forward speed in Isaac Sim",fontsize=15)
fig.savefig(Path(__file__).with_name("forward_speed_full.png"),dpi=180)
fig.savefig(Path(__file__).with_name("forward_speed_full.svg"))
