...
def command(source, output, name, phase):
    ...
    if phase == "flat":
        common += ["/workspace/hexapod/tools/train_length_study.py", "--mode", "validate"]
    else:
        common += ["/workspace/hexapod/tools/validate_terrain_robot.py", "--admission", "/outputs/flat/admission.json",
            "--fixture-admission", "/workspace/hexapod/" + FIXTURE_ADMISSION,
            "--catalog", "/workspace/hexapod/" + CATALOG, "--fixture-id", FIXTURE, "--steps", "1000", "--reset-diagnostic"]
...
def run_owned(args, phase):
    ...
    try:
        ...
        if process.returncode != 0 or state.get("status") != ("diagnostic_completed" if phase == "terrain" else "completed") or (args.output / phase / "failure.json").exists():
...
def main():
    ...
    try:
        ...
        check_source(args.source)
        report.update(status="diagnostic_completed", terrain_state_sha256=digest(args.output / "terrain/state.json"),
                      terrain_standing_passed=False, source_unchanged=True, admitted_asset_unchanged=True)
    except Exception as exc:
        report.update(status="stopped" if isinstance(exc, InterruptedError) else "failed", error=repr(exc))
        raise
    finally:
