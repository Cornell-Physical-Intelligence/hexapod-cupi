...
parser.add_argument("--reset-diagnostic", action="store_true")
    ...
    try:
        ...
        if args.reset_diagnostic:
            ...
            def snapshot():
                ...
                return dict(root_pos_w=values(data.root_pos_w), root_quat_xyzw=values(data.root_quat_w),
                    projected_gravity_b=values(gravity), default_root_pose=values(data.default_root_pose),
                    env_origins=values(env._terrain.env_origins),
                    computed_torque_nm=values(data.computed_torque),
                    episode_elapsed_s=values(env._episode_elapsed_s),
                    torque_excess_duration_s=values(env._torque_demand_excess_duration_s),
                    base_contact=values(forces.norm(dim=-1).max(dim=1)[0][:, 0] > 5.),
                    too_low=values(data.root_pos_w.torch[:, 2] < .055),
                    upside_down=values(gravity[:, 2] > -.45))
            trace = dict(scope="Five-control pre-reset diagnosis only; no standing admission",
                configured_rotation_xyzw=list(cfg.robot.init_state.rot),
                intended_body_yaw_rad=spec.start_body_yaw_rad,
                torque_demand_limit_nm=getattr(cfg, "terminate_on_computed_torque_demand_nm", None),
                torque_demand_grace_s=getattr(cfg, "torque_demand_termination_grace_s", 0.),
                torque_demand_duration_s=getattr(cfg, "terminate_on_computed_torque_demand_duration_s", 0.),
                after_explicit_reset=snapshot(), pre_reset_steps=[])
            original_dones = env._get_dones
            def traced_dones():
                terminated, truncated = original_dones()
                row = snapshot()
                row.update(terminated=values(terminated), truncated=values(truncated))
                trace["pre_reset_steps"].append(row)
                return terminated, truncated
            env._get_dones = traced_dones
            for _ in range(5):
                with torch.inference_mode():
                    env.step(zero)
            if len(trace["pre_reset_steps"]) != 5:
                raise RuntimeError("Expected exactly five pre-reset done evaluations")
            (args.output / "reset_diagnostic.json").write_text(json.dumps(trace, indent=2, allow_nan=False) + "\n")
            state.update(status="diagnostic_completed", gate={"passed": False}, control_steps=5,
                scope=trace["scope"], finished_unix=time.time())
            save(state)
            print("RESET_DIAGNOSTIC_COMPLETED_NO_ADMISSION", flush=True)
            return 0
        settle = args.steps // 5
