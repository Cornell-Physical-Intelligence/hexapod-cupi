...
SOURCE = BASE / 'terrain_robot_source_002'
OUTPUT = BASE / 'terrain_robot_smoke_002'
PAUSE = BASE / 'forecast_pause_014'
UNIT = 'hexapod-terrain-robot-smoke-002-20260909.service'
...
def main():
    ...
    from launch_terrain_robot_smoke_spark import check_source
    ...
    try:
        ...
        for line in gpu.splitlines():
            ...
            if pid != 0 and pid not in parents:
                raise RuntimeError('Unrelated CUDA process; no forecasting pause or terrain launch: ' + line)
        ...
        record = dict(user_authorized_pause=True, created_unix=time.time(), units=units, unit=UNIT,
            ...
            reason='Fresh exact-plan flat gate then original-fixture full-C standing; no PPO',
...
''')
        ...
        subprocess.run(['systemd-run', '--user', '--unit=hexapod-forecast-restore-014', '--on-active=30m',
    ...
    try:
        cmd = ['systemd-run', '--user', '--unit=' + UNIT, '--property=RuntimeMaxSec=1320',
            '--property=TimeoutStopSec=180', '--property=KillMode=process',
            '--property=ExecStopPost=/usr/bin/python3 ' + str(restorer),
            '/usr/bin/python3', str(SOURCE / 'tools/launch_terrain_robot_smoke_spark.py'),
