...
OUT=s.ROOT/"artifacts/length_study_2026-09-09/stance_audit"
...
for name in ("f050_t060","f050_t080"):
    ...
    for gait in ("tripod","ripple"):
        for lo,hi in ((.08,.10),(.10,.12),(.12,.14)):
            ...
            for pose in eligible[:2]:
                for stride in (.04,.06,.10):
                    t=s.trajectory(robot,pose,gait,stride,.02,64,speeds=(.10,.20))
                    t.pop("samples",None)
            ...
            print(name,gait,lo,hi,"checked",len(eligible[:2]),"of",len(eligible),"poses",flush=True)
