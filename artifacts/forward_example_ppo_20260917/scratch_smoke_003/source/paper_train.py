...
def main(argv=None):
    ...
    parser.add_argument("--eval-scope", choices=("diagnostic", "learning", "full", "startup"), default="diagnostic")
    parser.add_argument("--startup-arm", choices=("cold", "neutral4"),
        help="One separate startup diagnostic allocation: BC20s, or scripted neutral4s then BC20s")
    ...
    if args.command is None:
        args.command=[.05,0.,0.] if args.eval_scope=='startup' else [.04,0.,0.]
    if args.eval_scope == 'startup':
        if args.mode != 'evaluate' or args.startup_arm is None:
            raise ValueError('Startup scope requires --mode evaluate and --startup-arm')
        if list(args.command) != [.05,0.,0.]:
            raise ValueError('Startup diagnostic fixes command [.05,0,0]')
    elif args.startup_arm is not None:
        raise ValueError('--startup-arm requires --mode evaluate --eval-scope startup')
    ...
    if args.mode == "evaluate":
        ...
        elif args.eval_scope == 'startup':
            identity['startup_diagnostic']={'arm':args.startup_arm,
                'scripted_neutral_controls':0 if args.startup_arm=='cold' else 200,
                'policy_controls':1000,'policy_command':[.05,0.,0.],
                'separate_fresh_app_allocation':True,'stage2_complete':False}
    ...
    try:
        ...
        if args.mode == "diagnostic":
            ...
            if args.mode == "train":
                ...
                from .evaluate import evaluate_suite, run_diagnostic_trial, run_learning_probe_suite, run_startup_diagnostic
                ...
                if args.eval_scope == "full":
                    ...
                    state['evaluation']=run_startup_diagnostic(env,learner,args.output/'evaluation',
                        args.geometry_extrema,args.checkpoint,arm=args.startup_arm,
                        record_video=True,progress=evaluation_progress,max_wall_seconds=args.max_wall_seconds)
                else:
