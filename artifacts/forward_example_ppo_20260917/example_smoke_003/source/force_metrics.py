...
# Fresh replay packs contain the preserved paper_train entry point.
PAPER_PACKAGE = __package__ if Path(__file__).with_name('paper_train.py').is_file() else 'experiments.paper_walk'
_config = importlib.import_module(PAPER_PACKAGE+'.env_config')
JOINT_NAMES, LEGS, MASS_KG, MODEL_SHA256 = _config.JOINT_NAMES, _config.LEGS, _config.MASS_KG, _config.MODEL_SHA256
...
def run_batch(env, policy, cases, output, geometry_extrema, **kwargs):
    """Extend a fresh evaluation result without changing its saved verdict."""
    evaluate = importlib.import_module(PAPER_PACKAGE+'.evaluate')
    report = evaluate.run_batch(env, policy, cases, output, geometry_extrema, **kwargs)
    path = Path(output)/'force_metrics.json'
    summary = write_summary(output, path)
    return {**report, 'force_metrics': {'path': path.name, 'status': summary['status']},
            'files': {**report['files'], path.name: sha(path)}}
