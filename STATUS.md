...
Last reviewed: 2026-09-05 21:51 UTC. **No physical-model PPO has started.** The first full nominal motion validation passed, but the refined repeat and subsequent Kd 0.30 standing comparison failed torque convergence. Matched traces reproduce the noisy transient with one robot at (−2, 0) m; its physical trajectory matches the corresponding batch member exactly. Contact can leave inconsistent linkage velocities despite very small position errors.

The sixteen-final-velocity-iteration candidate `c804169` completed the matched eight-robot diagnostic. Against one final iteration, worst settled passive velocity residual decreased from 4.425369 to 0.097386 rad/s and pin relative speed from 0.362515 to 0.003487 m/s. This is substantial numerical improvement, but some individual positions regress and minimum support remains five feet. Do not claim convergence or training admission from those aggregate improvements. **891 CPU tests and GitHub CI passed.** A matched 32-environment × 600-control-step standing pair is now running on the same frozen source: 64/16 then 128/16 position/velocity iterations. Each phase is bounded to 1,200 seconds; neither can admit PPO. It preserves the 2.4–12 s settled window of the earlier failed standing comparison. Geometry, controller, timestep and physical acceptance limits are unchanged.

The separate GPU collision-isolation defect is fixed and the live root-body-pair controls succeeded: filtered overlapping robots report zero pair contacts/force while both contact the ground; the unfiltered control reaches 340 native contact points per directional query/sample. This measured pair response complements the authored whole-environment group audit; it is not full-body, workspace or hardware qualification. The old and fixed spaced-world traces are byte-identical, so this defect does not explain the standing transient. [Position replay](artifacts/mkii_fourbar_2026-09-05/translation_diagnostics/batch_followup_analysis/README.md) · [One/four/sixteen comparison](artifacts/mkii_fourbar_2026-09-05/translation_diagnostics/final_velocity16_comparison/README.md) · [Live overlap evidence](artifacts/mkii_fourbar_2026-09-05/dynamics_trace_analysis/live_controls_attempt_002/README.md) · [First full nominal pass](artifacts/mkii_fourbar_2026-09-05/campaign_006_ramped_targets/README.md).
