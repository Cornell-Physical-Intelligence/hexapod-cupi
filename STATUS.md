...
Last reviewed: 2026-09-06 02:08 UTC. **No physical-model PPO is running or has started.** Campaign 007 stopped during nominal validation when the shared coordination file changed to request sharing. The frozen launcher treats any byte change as a yield request; validation exits immediately, while training would request a checkpoint. The weather request asked to keep hexapod running concurrently, but neither launcher implements shared execution. This handoff stopped the hexapod instead of establishing concurrency. Its exact owned container was removed and its reservation released at 2026-09-05 22:19:23 UTC. No nominal final report or new physical failure was recorded.

The last flushed nominal sample is standing step 600/1000 at 22:18:24 UTC, before any driven sequence. The fresh probe passed; full motion/refined checks and PPO did not run. The user then explicitly granted full Spark takeover. The exact weather GPU queue was stopped gracefully with outputs preserved; by 02:08 UTC its parent and GPU processes were gone. Guard 1711343 holds the scheduler lock, bounded until 12:06:50 UTC. The next release will distinguish explicit sharing-control changes from harmless prose updates while preserving actual request/yield and resource-isolation checks. Exclusive hexapod execution is being restored; concurrency/MPS is not enabled.
...
Source **fd34f66** adds exact placement matching and raw-demand comparison before admission, preventing equal torque caps from hiding different controller requests. **898 CPU tests and GitHub CI passed.** Interrupted campaign `fourbar-campaign-20260905T220944Z-78b3e50d` ran from `/home/orionh/HEXAPOD_runs/mkii_placement_convergence_v1/source`. Its planned scratch and full PPO stages did not start. Reservation 1667860 released on the interruption; its former maximum expiry is not an active allocation. A fresh complete validation remains required before any learning.
