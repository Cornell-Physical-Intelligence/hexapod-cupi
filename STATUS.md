...
Last reviewed: 2026-09-06 03:18 UTC. **Campaign 008 completed nominal validation and failed; physical-model PPO has not started.** All 32 environments completed 1,000 standing plus 2,400 driven controls without resets. Standing remained stable, but motion exceeded the 0.1 mm closure bound (**0.213821 mm**), included zero-support samples, and failed the individual LM/LR pushlever and group LF/RM pushlever direction checks. Applied torque stayed within its 5.5 N m envelope; raw demand reached **85.2076 N m**. There were no non-foot ground contacts. Peak passive velocity residual was **221.327 rad/s**, so the passing standing result did not establish stable motion dynamics.

An exact 32-environment replay through the LR pushlever test is being prepared to identify the affected environments, phase responses and coincident joint/contact/torque events. It preserves the original 1,000-step standing history and preceding individual motions, records every physics-step physical metric, and adds detailed traces around the three left pushlevers. The existing aggregate report cannot establish a root cause. Recovery state: `/home/orionh/HEXAPOD_runs/mkii_motion_recovery_v1/recovery.json`. No diagnostic GPU job has been launched at this snapshot.

Under the user's explicit instruction to remove competing processes and continue toward a finished PPO tonight, the verified weather CPU batch/continuation workers, score watchers and corrdiff materializer were stopped. All targeted processes were absent by **03:14 UTC**; output files were preserved. Spark had **118 GiB available memory** and no CUDA workload after cleanup. Successor guard **1772925** holds `/opt/wx/gpu.lock`, bounded until **13:10:41 UTC**, at `/home/orionh/HEXAPOD_runs/mkii_motion_recovery_v1/priority_20260906T031041Z/`. Prior guard 1711343 released when campaign 008 failed. The shared note records continued exclusive priority. Tonight is the requested completion target, not a promise that unresolved physics or a useful policy can meet it.
