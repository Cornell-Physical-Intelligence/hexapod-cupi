...
Last reviewed: 2026-09-05 22:10 UTC. **The full validation-to-training campaign is running; physical-model PPO has not started yet.** A fresh probe and complete nominal/refined standing-and-motion checks precede scratch PPO and the full resumed job. The earlier first full nominal pass failed its refined repeat, so no prior short or failed result is being used as admission.
...
Source **fd34f66** adds exact placement matching and raw-demand comparison before admission, preventing equal torque caps from hiding different controller requests. **898 CPU tests and GitHub CI passed.** Campaign `fourbar-campaign-20260905T220944Z-78b3e50d` runs from `/home/orionh/HEXAPOD_runs/mkii_placement_convergence_v1/source`. It proceeds to 64 environments × 3 scratch updates and a separate 512-environment × 1,000-update resume only after all full validation and checkpoint gates pass. The new bounded reservation holds the weather scheduler lock until campaign completion/failure/pause, explicit release, or 2026-09-06 08:09:43 UTC at the latest.
