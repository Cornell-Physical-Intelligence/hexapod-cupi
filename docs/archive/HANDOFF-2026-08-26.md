> **Frozen historical snapshot — do not update.**
> This is the `HANDOFF.md` that stood at `2026-08-26T17:46:47Z`, moved here
> verbatim. It is superseded by `STATUS.md` (current state) and `docs/`
> (durable design, operations, and incident records). Nothing in this file is
> maintained; where it disagrees with `STATUS.md` or `docs/`, those win.
> Absolute Mac paths beginning with `/Users/andreboufama` refer to the machine
> this snapshot was written on, not to any current checkout.
