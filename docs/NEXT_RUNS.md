...
Before the next retry, bind the complete external-automation block: all identified loaded masks, preserved prior units, the reconstruction entry gate and the exact live queue-lock owner. Reuse the unchanged source005/host007 and authentic native6 prerequisite under a fresh output identity. This operational guard change does not require substituting geometry or waiving any standing gate. Preserve startup and contention interruptions separately, then enable the learner only from a complete same-source32 pass.

## 2026-09-11 — after an uninterrupted batch rejection

Analyze the [completed source005 batch](../artifacts/mkii_updated_2026-09-10/native_standing32_rejected_005/README.md) before another native allocation. Run the reviewed CPU analyzer against immutable remote raw files, pinning the source and authentic terminal audit. Compare each support-failure sample and its neighboring steps across detailed patches, floor-filtered force matrices, link COM velocities, joint angles and SDK rates. Preserve sensor/body mapping, all-step occupancy counts and input hashes before and after analysis. Do not download the five-gigabyte trace to a nearly full laptop or treat unavailable native buffer metadata as proof of completeness.

Select any subsequent implementation or numerical comparison from those observations, with a new source identity when behavior changes and fresh one-robot then matching-source batch admission. Keep the prepared learner disabled until that actual acceptance exists. Maintain every external automation mask and the reconstruction queue/entry blocks during CPU diagnosis and between native jobs.
