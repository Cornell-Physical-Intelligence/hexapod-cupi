...
### 2026-09-10 — carry historical jitter lessons into canonical native admission

The [completed CAPS500 comparison](../artifacts/project_review_2026-09-10/direct_omni_extended500_interpretation_001/README.md) is historical C-study evidence. A larger budget improves some tracking and drift metrics without quiet acceptance; source versions and budgets differ, so this is not a matched-budget causal study. Preserve both original-policy and CAPS50 comparison baselines, reset-contaminated excursions and all failed trials. Fable maximum-effort review supports admission first; its baseline/termination conflations were independently corrected without altering the original analysis.

Prioritize the approved detailed model's bounded native import/SDF cooking/identity inspection. Use no learned actor, targets, invented drive gains or unsupported ground stance in that first inspection. Read actual body/joint order, frames, masses/inertias, limits and cooked collision representation from Isaac. Explicitly record SDK initialization/warm-up steps and any constraint/self-contact motion; zero gravity does not prove static physical correctness. Keep source collision settings and preserve a failed cooking attempt rather than quietly reducing detail. This phase cannot admit learning or ground support by itself.

### 2026-09-10 — bind diagnostic APIs to the installed runtime

The [first canonical inspection](../artifacts/mkii_updated_2026-09-10/native_inspection_001/README.md) exposed an obsolete private tensor-module path before loading the robot. Bind the successor to the actual installed public tensor provider and its source hash. Preserve the failed attempt and its cleanup evidence. No geometry, inertia, joint-limit or collision-detail change follows from this infrastructure failure.

The cooking observation must distinguish simulation initialization returning from an unavailable legacy queue counter. NVIDIA documents that startup blocks for required cooked collision data. Require actual native views and exact SDF paths after that barrier; record an unavailable counter as unknown, never as zero. This is representation inspection, not signed-distance accuracy, supported contact or training admission. See [NVIDIA's collider cooking documentation](https://docs.omniverse.nvidia.com/kit/docs/omni_physics/latest/dev_guide/rigid_bodies_articulations/collision.html#generate-mesh-colliders-cooking).

Then introduce the separately reviewed direct-drive coordinate/actuator/runtime contract and test supported quiet behavior before learning. Log raw actor inputs, requested/executed targets, actuator/limiter state, actual angles/SDK rates and requested/applied torques on a common physical clock with reset boundaries. The historical4.4Hz oscillation is a clue, not a prescribed new-model excitation or permission to transfer an old controller. Dynamic action-pipeline state should be available to the future policy or handled by an explicitly versioned stateful controller. The model change must preserve named observation/action semantics and every numeric/visual admission gate.

### 2026-09-10 — adopt bounded canonical actuation preparation

Use the [completed native003 import](../artifacts/mkii_updated_2026-09-10/native_inspection_003/README.md) as the exact input to the [corrected actuation proposal](../artifacts/mkii_updated_2026-09-10/actuation_design_001/README.md). First measure named coordinate resets and small explicit torque responses without gravity, floor or position drives. The selected0.005Nm20ms pulse plus20ms coast fits the unchanged diagnostic motion bounds under the full floating-base planning model; retain the rejected larger-pulse draft. Treat the measured native actuation input separately from constraint reaction and the implicit-drive force ceiling.

Then qualify one explicitly provisional simulation servo and the actual distal+X toe support region before a fresh32×24×2 PPO smoke. The neutral stance/static-load and inertia-derived gain calculations are candidates, not native support evidence. Version the external-force integration setting, record400Hz demand/command/angle/rate channels, measure1-to32 scaling and retain all original Stage2 quality gates. Hardware identification gates transfer; declared provisional simulation characterization can proceed under its own matching native admission. Do not describe the48V curve, symmetric braking,1.6Nm ceiling or zero added friction/latency as calibrated hardware behavior.


### 10 September 2026 — direct-drive response interpretation and support boundary

The [canonical effort diagnostic](../artifacts/mkii_updated_2026-09-10/native_actuation_001/README.md) establishes the small coordinate/sign/input response with the detailed source inertia. Its native generalized matrix matches root-COM velocity coordinates. Preserve that frame convention in the new runtime and keep native input buffers distinct from constraint reaction and hardware torque. Near-equality of aggregate SDK and interval-angle rate maxima in this zero-gravity experiment does not qualify contact feedback or diagnose the historical C-study oscillation.

Proceed through explicit source-shape/contact checks and separately bound provisional standing, then fresh PPO. The initial plate-frame height floor is 0.055 m relative to the authored flat ground; nominal stance is 0.076611 m. This is a declared new-model geometry mapping, with unchanged quiet and actuator gates and actual nonspherical toe classification required. Measured hardware gains, bus voltage, payload and thermal behavior remain a separate calibration boundary.


### 10 September 2026 — measured SDF tuple layout

The [actual source-shape query](../artifacts/mkii_updated_2026-09-10/native_sdf_query_001/README.md) identifies the installed native tuple as `(gradient_x, gradient_y, gradient_z, distance)` in source-shape coordinates/metres. Use that measured binding for future diagnostic decoding; the Python provider's distance-first prose is contradicted by actual data. Keep the original failed interpretation and the explicit corrected CPU interpretation separate. No CAD, collider or diagnostic threshold changes are justified by a field-order documentation mismatch. Source-shape agreement still precedes independent loaded contact/standing validation.

The canonical standing preparation intentionally strengthens nonfoot-contact and non-toe floor-clearance checks to cover every controlled substep. Historical source009 applied nonfoot/support scoring after settling; do not rewrite that historical contract or label this added canonical requirement as inherited. Clip source mesh triangles at the actual distal-cap boundary, retaining edge intersections, when evaluating non-toe clearance. Filtering existing vertices alone can omit the crossing shaft surface.


### 10 September 2026 — preserve exact zero-force native contact records
...
### 10 September 2026 — separate solver convergence from admission and operating time

The [completed detailed standing32 report](../artifacts/mkii_updated_2026-09-10/native_standing32_rejected_002/README.md) motivates one controlled numerical comparison. Preserve its support/quiet rejection and the separate host timeout; neither can be reclassified to admit learning. The earlier optimized one-robot result remains evidence for its exact source. Do not infer a solver cause from brief support loss or raw SDK joint-rate RMS alone.

Use a new source identity to test 32 position / 4 velocity iterations. Before any authoring, require every articulation's installed public USD schema readback to be **unauthored 32/1**. The documented fallback is an explicit precondition, not an already observed value or independent internal-solver count. Reject an unexpected precondition rather than silently substituting settings. Keep the position count, detailed source geometry, SDFs, masses/inertias, 400 Hz servo, external-force setting, 50 Hz targets, actuator envelope and all acceptance gates unchanged. Seal per-root values before authoring, after authoring, after reset and at the end.

Require a fresh one-robot screen and then matching-source 32-replica acceptance before learner integration. A separately versioned 1,200-second phase budget, with unchanged 90-second application-readiness bound and exact cleanup, is an operational response to measured acquisition/finalization time. It does not relax the simulated duration, support, quiet, torque or Stage 2 gates. This supersedes the earlier deadline-preservation instruction only for the declared successor; all prior timeout outcomes stay immutable. Additional velocity iterations are a hypothesis to test, not a promised repair. Current dispatch and outcomes belong only in STATUS; future execution order belongs in NEXT_RUNS.


### 10 September 2026 — distinguish constrained velocity reporting before further solver scaling

After the bounded 32/4 comparison, use 32/0 as the next single-variable solver experiment, preserving 32 position iterations and all physical/controller/gate contracts. NVIDIA's [simulation documentation](https://nvidia-omniverse.github.io/PhysX/physx/5.8.0/docs/Simulation.html) explains that constrained reported velocity and pose increments can differ because position and velocity stages solve different errors. Its [Omni Physics 110 limitations](https://docs.omniverse.nvidia.com/kit/docs/omni_physics/110.0/dev_guide/guides/current_limitations.html) list zero or few velocity iterations as a diagnostic alternative. This supports a bounded comparison; it does not establish the cause of our failure.

Add read-only legacy friction coefficients, link velocities and a direct floor-force aggregate. [PhysX issue 498](https://github.com/NVIDIA-Omniverse/PhysX/issues/498) reports that zero modern friction parameters can coexist with legacy friction on a specific CPU build. Treat that report as an unconfirmed hypothesis for our installed GPU/parser, rather than assuming the modern getter proves every friction path zero. Preserve any unexpected readback without changing it. SDK and interval-angle rates remain separate observations; keep the original quiet gate and require fresh one-robot then 32-robot admission before PPO. Current outcomes belong in STATUS and immutable artifacts.


### 11 September 2026 — preserve standing identity through learner integration

Carry the admitted source005 solver and read-only diagnostics into the fresh learner by an explicit composition boundary. Preserve the complete 8,000-step neutral prefix, verify and seal its 32/0 solver readback before the learner callback, then retain the original final solver check after the additional 384 policy substeps. Bind the prefix's 8,000-step diagnostic coverage separately from the final 8,384-step coverage, with immutable legacy-friction and solver records. Preserve one initial reset, actor/action/limiter contracts and every standing predicate. These checks establish integration scope, not a walking objective or Stage 2 quality.

Treat pre-AppReady failures as infrastructure outcomes. Retain the original unfinalized native state and actual logs, verify exact owned cleanup, and use a fresh output identity for a same-source retry. Do not infer a physics result or claim a traceback that the inventory does not contain. A startup retry does not alter the 90-second readiness bound or admit learning from single-robot evidence alone.


### 11 September 2026 — block all external Spark automation

The user explicitly requires complete shutdown of external automated Spark jobs and exclusive priority for HEXAPOD. Replace timer-only deferral with persistent masks for all identified forecast families and the competing inference service. Block reconstruction startup at its known entry point and hold its queue lock across runs; preserve exact prior files, enable states and queued/completed work for a later user-authorized release. Audit actual GitHub and local scheduler inventories rather than inferring a remote trigger from the presence of a repository. Keep HEXAPOD's own continuation and publication jobs enabled.

These are versioned operational controls. They do not change the admitted detailed geometry, solver, motor envelope or quality gates. Bind masks, loaded states and the live queue-lock owner before the next native allocation. Preserve any interrupted acquisition with its original unfinalized state; an external interruption never supplies the missing 32-replica admission. Current control receipts and execution belong in STATUS and immutable artifacts.

### 11 September 2026 — diagnose the completed scaling rejection before another physics change

The [uninterrupted source005 batch](../artifacts/mkii_updated_2026-09-10/native_standing32_rejected_005/README.md) separates the remaining standing rejection from the earlier compute interruptions. Preserve its original support and quiet verdicts. Before changing solver settings again, compare the saved detailed patch forces with the same-step floor-force matrix, link velocities, actual joint-angle increments and raw SDK rates. Map sensor order to articulation order explicitly and retain the exact floating-point aggregation used by the classifier.

Use a bounded, read-only CPU replay on Spark's retained traces. Inspect each failed support sample and its adjacent steps, with buffer occupancy across the complete acquisition. Reconstructed record counts cannot prove completeness of native buffers whose original count/start arrays were not saved. Agreement or disagreement between channels informs the next experiment; neither waives a support or quiet gate. Matching-source 32-replica admission still precedes the prepared PPO integration.


### 11 September 2026 — pause autonomous execution for James’s handoff

The latest user instruction supersedes the earlier unattended continuation and acceleration directives. Preserve the full roadmap and evidence while stopping new research and GPU allocations. Pause the Codex continuation, retain external-job reservation controls, publish previously local preparations as clearly labeled snapshots, and provide James with a single starting guide. Pausing is not Stage 2/3 completion, native admission or a release of competing compute. A future resume must review the canonical model and actual rejection before selecting a new bounded experiment.
