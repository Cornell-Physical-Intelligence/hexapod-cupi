...
**Execution state: corrected serial model under live validation.** The user authorized Spark execution after the offline repairs and animated CAD inspection. The versioned standing validator now runs on Spark; physical four-bar dynamics and driven joint/direction qualification remain open. No training or automatic restart is queued. See [step 1](MKII_STEP1.md), [live evidence](../artifacts/mkii_step2_2026-09-04/README.md), [the run sequence](NEXT_RUNS.md) and [current status](../STATUS.md).
...
The [RS05 specification review](RS05_SPEC_REVIEW.md) identifies a critical actuator gate: the URDF/config already specify 5.5 N·m peak, but the inherited actuator clips delivered effort at 1.6 N·m and has no qualified torque-speed/thermal model. Vendor continuous stall is 1.2 N·m; the 1.6 and 1.8 N·m rotating ratings use different cooling plates. These short capped standing results do not validate bursts or endurance. Before physical-task training, bind a versioned motor contract and resolved configuration identity to the simulation/runtime manifest, with explicit torque-speed, voltage, thermal, burst/recovery and phase-current limits. Raising the effort cap to an indefinite 5.5 N·m is insufficient. The current manifest omits motor parameters, and hardware CAN/current/temperature protection remains to implement.
